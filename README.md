...
- **全工具 Tour 引导**：字幕翻译、字幕转换、字幕语言提取、文件名翻译页面均内置分步引导，首次进入自动弹出，也可从页头随时重新查看。
- **字幕翻译历史任务恢复**：支持扫描当前输出目录、选择目录扫描或导入单个 `*.fusionkit.resume.json` 恢复清单，自动发现未完成翻译任务并续跑。
- **恢复清单自包含续跑**：源文件缺失或变化时，可使用恢复清单中保存的原始分片继续翻译，降低长任务中断后的返工成本。
- **文件名 / 文件夹名翻译工具**：支持选择文件或文件夹生成 dry-run 预览，确认后再安全应用批量重命名，并提供 journal 辅助回滚。
- **Agent 工作流增强**：HomeAgent 支持拖拽文件 / 文件夹到输入框、跨页面保留输入草稿，并可通过自然语言执行字幕恢复和名称翻译预览。
