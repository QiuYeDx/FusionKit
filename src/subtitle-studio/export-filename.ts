...
import { stripTrailingMediaExtension } from '../lib/media-file-name';
...
export function subtitleExportFileName(doc: { origin: { displayName: string }; translationTracks: readonly { id: string; language: string }[] }, options: ExportOptions): string {
  ...
  if (options.stripMediaExt !== false) base = stripTrailingMediaExtension(base);
  base = base.replace(/[<>:"/\\|?*\x00-\x1f\x7f-\x9f]/g, '_').replace(/[. ]+$/, '').slice(0, 160) || 'subtitle';
