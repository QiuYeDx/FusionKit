...
import { AlertCircle, BookOpen, CheckCheck, LoaderCircle } from 'lucide-react';
...
const targetLanguage = (language: string) => language === 'zh' ? 'zh-Hans' : languages.includes(language as typeof languages[number]) ? language : 'zh-Hans';
...
export function StudioKnowledgeBatch({ documents, config, apiKey, disabled, onStarted, onAdmissionChange }: Props) {
  const { t, i18n } = useTranslation();
  const formId = useId();
  const trigger = useRef<HTMLButtonElement>(null);
  const [open, setOpen] = useState(false);
  const [library, setLibrary] = useState<LibrarySnapshot | null>(null);
  const [selection, setSelection] = useState<BatchKnowledgeSelection>({ version: 1, languagePair: { source: 'ja', target: targetLanguage(config.language) }, collectionIds: [], disabledEntryIds: [], instructions: config.instructions || undefined });
  ...
  useEffect(() => {
    ...
    if (configRef.current.instructions !== config.instructions || configRef.current.language !== config.language) {
      setSelection(value => ({ ...value, instructions: config.instructions || undefined, languagePair: { ...value.languagePair, target: targetLanguage(config.language) } }));
    }
  ...
  const check = async () => {
    if (pendingRef.current || !active.current || overLimit || !documents.length) return;
  ...
  const start = async () => {
    if (pendingRef.current || !active.current || !preview?.readyCount || !apiKey.trim() || overLimit) return;
  ...
  return <>
    <div className="min-w-0 space-y-2">
      <Button ref={trigger} data-testid="studio-knowledge-batch" size="sm" variant="outline" disabled={disabled || busy || overLimit || !documents.length} onClick={() => void begin()}><BookOpen />{t('knowledge:batch.open')}</Button>
    ...
    <ScrollableDialog open={open} onOpenChange={next => { if (!next) close(); }} maxWidth="sm:max-w-3xl" contentClassName="grid-rows-[auto_minmax(0,1fr)_auto] [&>button]:hidden" onCloseAutoFocus={event => { event.preventDefault(); if (active.current === false && trigger.current?.isConnected) trigger.current.focus({ preventScroll: true }); }}>
        ...
        <div data-testid="knowledge-batch-content" aria-busy={busy} className="min-w-0 space-y-4 [overflow-wrap:anywhere]">
          ...
          <fieldset disabled={busy || !library} className="min-w-0 space-y-4">
            ...
            <Choice testId="knowledge-batch-recipe" disabled={busy || !library} label={t('knowledge:trial.recipe')} value={selection.recipeId ?? 'none'} options={[{ id: 'none', name: t('knowledge:trial.no_recipe') }, ...availableRecipes, ...(selection.recipeId && !availableRecipes.some(item => item.id === selection.recipeId) ? [{ id: selection.recipeId, name: t('knowledge:batch.unavailable_recipe') }] : [])]} onChange={id => {
              ...
              update({ recipeId: chosen?.id, ...(chosen ? { languagePair: chosen.languagePair } : {}) });
            }} />
            ...
            <details data-testid="knowledge-batch-topics" className="min-w-0 rounded-md border p-3" open>
            ...
            <p data-testid="knowledge-batch-scope-help" className="text-xs leading-5 text-muted-foreground">{t('knowledge:batch.scope_help')}</p>
      ...
      <ScrollableDialogFooter className="flex flex-wrap items-center justify-end gap-2 p-3">
        ...
        <Button data-testid="knowledge-batch-check" size="sm" variant="outline" disabled={busy || overLimit || !documents.length} onClick={() => void check()}>{pending === 'check' && <LoaderCircle className="animate-spin" />}{t('knowledge:batch.check')}</Button>
