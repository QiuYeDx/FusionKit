    ...
    <ScrollableDialog open={open} onOpenChange={changeOpen} maxWidth="sm:max-w-[600px]" contentClassName="studio-export-dialog" onOpenAutoFocus={event => {
      ...
      <ScrollableDialogContent className="studio-export-content" fadeMaskHeight={16}>
        <div className="studio-export-form">
          ...
          {batchResult && <section aria-live="polite" data-testid="studio-batch-result"><p className="studio-batch-note">{t('studio:batch.export_result', { count: batchResult.items.filter(item => item.ok).length, failed: batchResult.items.filter(item => !item.ok).length })}</p><ul className="studio-batch-items">{batchResult.items.map(item => <li key={item.documentId} data-document-id={item.documentId} data-state={item.ok ? 'success' : 'failed'}><StudioFileName name={item.displayName} focusable /><span>{item.ok ? 'result' in item ? item.result.fileName : item.fileName : t(errorKeys[item.error])}</span></li>)}</ul></section>}
        </div>
