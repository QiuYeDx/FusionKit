...
const automaticLabelKeys = {
  pending: { label: 'studio:transcription.auto_translation_pending_short', description: 'studio:transcription.auto_translation_pending' },
  admitted: { label: 'studio:transcription.auto_translation_admitted_short', description: 'studio:transcription.auto_translation_admitted' },
  needs_configuration: { label: 'studio:transcription.auto_translation_recovery_short', description: 'studio:transcription.auto_translation_recovery' },
} as const;
function AutomaticTranslationLabel({ status }: { status: NonNullable<TranscriptionTaskSummary['automaticTranslation']>['status'] }) {
  const { t } = useTranslation();
  const keys = automaticLabelKeys[status];
  return <Tooltip delayDuration={350}>
    <TooltipTrigger asChild><span tabIndex={0} className="studio-transcription-auto-label" data-state={status}><Languages aria-hidden="true" />{t(keys.label)}</span></TooltipTrigger>
    <TooltipContent sideOffset={6} className="max-w-[min(20rem,calc(100vw-2rem))] whitespace-normal text-wrap">{t(keys.description)}</TooltipContent>
  </Tooltip>;
}
function TaskModelLabel({ name, backend }: { name: string; backend: string }) {
  const text = `${name} · ${backend.toUpperCase()}`;
  return <Tooltip delayDuration={350}>
    <TooltipTrigger asChild><span tabIndex={0} className="studio-transcription-task-model">{text}</span></TooltipTrigger>
    <TooltipContent sideOffset={6} className="max-w-[min(20rem,calc(100vw-2rem))] whitespace-normal text-wrap [overflow-wrap:anywhere]">{text}</TooltipContent>
  </Tooltip>;
}
...
export function StudioTranscription({ header, onOpenDocument }: { header: ReactNode; onOpenDocument: (id: string) => Promise<void> }) {
  ...
  const profiles = useModelStore(value => value.profiles);
  const assignedProfile = useModelStore(value => value.assignment.taskExecution);
    ...
    <ToolDetailLayout header={header} className="studio-transcription-layout" asideClassName="studio-transcription-aside order-2 lg:order-1" mainClassName="studio-transcription-main order-1 lg:order-2"
      ...
      <div data-testid="studio-transcription" className="studio-transcription-workspace" onDragEnter={dragEnter} onDragLeave={dragLeave} onDragOver={dragOver} onDrop={dropMedia}>
        ...
        <ToolPanel title={t('studio:transcription.selected_media')} icon={AudioLines} badge={<Badge variant="secondary" className="font-mono text-[11px]">{state.drafts.length} / 20</Badge>}
          ...
          {!!state.drafts.length && <StudioScrollFade maxHeight={290}><ul className="studio-transcription-media-list">{state.drafts.map(draft => <li key={draft.id} data-testid="studio-transcription-media-row" data-draft-id={draft.id} data-state={draft.status}>
        ...
        <ToolPanel title={t('studio:transcription.queue')} icon={ListOrdered} badge={state.tasks.length ? <Badge variant="secondary" className="font-mono text-[11px]">{state.tasks.length}</Badge> : undefined} className="studio-transcription-queue"
          ...
          {state.tasks.length ? <><StudioScrollFade maxHeight={460}><ul className="studio-transcription-task-list">{state.tasks.map(task => <li key={task.taskId} data-testid="studio-transcription-task-row" data-task-id={task.taskId} data-state={task.status}>
            ...
            <div className="studio-transcription-task-meta"><span>{state.cancellingTaskIds.includes(task.taskId) ? t('studio:transcription.cancelling') : t(taskStatusKeys[task.status])}</span><TaskModelLabel name={models.find(resource => resource.resourceId === task.modelId)?.displayName ?? task.modelId} backend={task.resolvedBackend} />{task.durationMs !== undefined && <span>{duration(task.durationMs)}</span>}{task.automaticTranslation && <AutomaticTranslationLabel status={task.automaticTranslation.status} />}{!terminal(task) && <span className="studio-transcription-task-percentage">{Math.round(task.progress)}%</span>}</div>
            {!terminal(task) && <progress max={100} value={task.progress} aria-label={`${t(taskStatusKeys[task.status])} · ${task.displayName}`} className="studio-transcription-progress studio-transcription-task-progress" />}
          ...
          </li>)}</ul></StudioScrollFade><p className="studio-transcription-session-note">{t('studio:transcription.session_only')}</p></> : <div className="studio-transcription-empty"><ListOrdered /><p>{t('studio:transcription.queue_empty')}</p><span>{t('studio:transcription.queue_hint')}</span></div>}
        </ToolPanel>
      </div>
    </ToolDetailLayout>
