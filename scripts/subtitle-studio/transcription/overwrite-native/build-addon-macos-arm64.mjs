...
const SOURCE_RELATIVE_PATH = "native/subtitle-studio-overwrite/src/addon.cc";
const DEFAULT_OUTPUT_PATH =
  "/tmp/fusionkit-subtitle-studio-overwrite-napi-v8-darwin-arm64.node";
...
export async function buildMacosArm64OverwriteAddon(options = {}) {
  ...
  const workRoot = await mkdtemp(
    path.join(outputDirectory, ".fusionkit-subtitle-studio-overwrite-native-build-"),
  );
  ...
  try {
    ...
    if (canonicalReceiptPath) {
      receiptWorkRoot = await mkdtemp(
        path.join(path.dirname(canonicalReceiptPath), ".fusionkit-subtitle-studio-overwrite-native-receipt-"),
      );
