...
const SOURCE_RELATIVE_PATH =
  "native/subtitle-studio-overwrite/src/addon-win32.cc";
const DELAY_LOAD_HOOK_RELATIVE_PATH =
  "native/subtitle-studio-overwrite/src/win-delay-load-hook.cc";
const LOGICAL_ARTIFACT_NAME = "subtitle-studio-overwrite.node";
const DEFAULT_OUTPUT_PATH = path.join(
  os.tmpdir(),
  "fusionkit-subtitle-studio-overwrite-napi-v8-win32-x64.node",
);
...
export const OVERWRITE_NATIVE_WINDOWS_BUILD_CONTRACT = deepFreeze({
  ...
  component: "subtitle-studio-overwrite",
...
const FIXED_COMPILE_FLAGS = Object.freeze([
  ...
  "-DNODE_GYP_MODULE_NAME=subtitle_studio_overwrite",
  "-DUNICODE",
  "-D_UNICODE",
...
export async function buildWindowsX64OverwriteAddon(options = {}) {
  ...
  const workRoot = await mkdtemp(
    path.join(outputDirectory, ".fusionkit-subtitle-studio-overwrite-win-build-"),
  );
  let receiptWorkRoot;
  let primaryFailure;
  try {
    ...
    const receipt = deepFreeze({
      ...
      build: {
        recipe:
          "scripts/subtitle-studio/transcription/overwrite-native/build-addon-windows-x64.mjs",
        source: SOURCE_RELATIVE_PATH,
        delayLoadHook: DELAY_LOAD_HOOK_RELATIVE_PATH,
        nodeVersion,
    ...
    if (receiptPath) {
      receiptWorkRoot = await mkdtemp(
        path.join(path.dirname(receiptPath), ".fusionkit-subtitle-studio-overwrite-win-receipt-"),
      );
    ...
    await cleanupWindowsBuildWorkspaces([workRoot, receiptWorkRoot].filter(Boolean), primaryFailure);
  }
}

export async function cleanupWindowsBuildWorkspaces(directories, primaryFailure, remove = rm) {
  const results = await Promise.allSettled(directories.map(directory => Promise.resolve().then(() =>
    remove(directory, { recursive: true, force: true, maxRetries: 3, retryDelay: 20 }))));
  const failures = results.filter(result => result.status === "rejected").map(result => result.reason);
  if (failures.length) {
    const error = new AggregateError(primaryFailure === undefined ? failures : [primaryFailure, ...failures],
      "Windows native build workspace cleanup failed.", primaryFailure === undefined ? undefined : { cause: primaryFailure });
    error.code = primaryFailure?.code ?? "build_cleanup_failed";
    throw error;
  }
}
