...
test(
  ...
  async () => {
    const tempRoot = await mkdtemp(
      path.join(os.tmpdir(), "fusionkit-subtitle-studio-overwrite-real-staging-"),
    );
    const runtimeRoot = path.join(tempRoot, "subtitle-studio/transcription");
    const addonPath = path.join(tempRoot, "subtitle-studio-overwrite.node");
...
test("stages signed macOS final bytes under a content-addressed atomic subtree", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      `subtitle-studio-overwrite.${manifest.artifact.sha256}.node`,
    );
    assert.equal(
...
test("rejects stale receipt bytes and preserves a prior no-clobber generation", async () => {
  ...
  try {
    ...
    const manifestBefore = await readFile(
      path.join(existing.root, "overwrite/v1/subtitle-studio-overwrite.v1.json"),
    );
    await assert.rejects(
...
test("shares the runtime 256 KiB overwrite manifest limit", async () => {
  ...
  try {
    ...
    const manifestPath = path.join(
      fixture.root,
      "overwrite/v1/subtitle-studio-overwrite.v1.json",
    );
...
async function createFixture(platform, arch, options = {}) {
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "fusionkit-subtitle-studio-overwrite-staging-test-"),
  );
  const root = path.join(tempRoot, "subtitle-studio/transcription");
...
function macReceipt(bytes) {
  return {
    ...
    build: {
      recipe:
        "scripts/subtitle-studio/transcription/overwrite-native/build-addon-macos-arm64.mjs",
      source: "native/subtitle-studio-overwrite/src/addon.cc",
      nodeVersion: "20.19.5",
      napiVersion: 8,
    ...
    artifact: {
      logicalFileName: "subtitle-studio-overwrite.node",
...
function windowsReceipt(bytes) {
  return {
    ...
    build: {
      ...
      source: "native/subtitle-studio-overwrite/src/addon-win32.cc",
      delayLoadHook:
...
async function readManifest(root) {
  return JSON.parse(
    await readFile(
      path.join(root, "overwrite/v1/subtitle-studio-overwrite.v1.json"),
      "utf8",
    ),
  );
}
