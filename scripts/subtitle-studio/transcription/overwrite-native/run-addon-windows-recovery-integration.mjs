...
export async function runWindowsOverwriteRecoveryIntegration(options = {}) {
  ...
  const workRoot = await mkdtemp(
    path.join(options.tempRoot ?? os.tmpdir(), "fusionkit-subtitle-studio-overwrite-win-rec-"),
  );
  try {
...
function spawnChild(
  ...
  return spawnSync(process.execPath, childArguments, {
    ...
    windowsHide: true,
    maxBuffer: 1024 * 1024,
    env: createWindowsRecoveryChildEnvironment(faultAction, faultPoint),
  });
}

export function createWindowsRecoveryChildEnvironment(faultAction, faultPoint, hostEnvironment = process.env) {
  const systemRoot = hostEnvironment.SystemRoot ?? "C:\\Windows";
  return {
    ...
    TEMP: os.tmpdir(),
    TMP: os.tmpdir(),
    ...(hostEnvironment.ELECTRON_RUN_AS_NODE === "1" ? { ELECTRON_RUN_AS_NODE: "1" } : {}),
    ...(faultAction ? { FUSIONKIT_OVERWRITE_TEST_FAULT_ACTION: faultAction, FUSIONKIT_OVERWRITE_TEST_FAULT_POINT: faultPoint } : {}),
  };
}
...
function journalPath(fixture, state) {
  return path.join(
    ...
    `${fixture.partialLeaf}.fusionkit-subtitle-studio-overwrite.${state}`,
  );
}
