...
export async function runOverwriteNativeIntegration(options = {}) {
  ...
  const workRoot = await mkdtemp(
    path.join(tempRoot, "fusionkit-subtitle-studio-overwrite-native-integration-"),
  );
  try {
    const cases = [
    ...
    runJournalValidationCase(addon, workRoot, {
      ...
      mutate(fixture) {
        const rollbackPath = path.join(
          fixture.directoryPath,
          `${fixture.request.partialLeaf}.fusionkit-subtitle-studio-overwrite.rollback`,
        );
...
function runTerminalDecisionConflictCases(addon, workRoot) {
  return ["rollback", "finalize"].map((terminalDecision) => {
    ...
    const terminalJournalPath = path.join(
      fixture.directoryPath,
      `${fixture.partialLeaf}.fusionkit-subtitle-studio-overwrite.${terminalDecision}`,
    );
...
function partialLeafFor(transactionId) {
  assert.match(transactionId, /^[A-Za-z0-9-]{1,80}$/u);
  return `.fusionkit-subtitle-studio-${transactionId}.partial`;
}

function transactionIdFromPartialLeaf(partialLeaf) {
  const match = /^\.fusionkit-subtitle-studio-([A-Za-z0-9-]{1,80})\.partial$/u.exec(
    partialLeaf,
  );
  assert.ok(match);
  return match[1];
