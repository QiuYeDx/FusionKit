...
test("freezes a shell-free N-API v8 macOS arm64 build descriptor", () => {
  ...
  for (const required of [
    ...
    "-undefined",
    "dynamic_lookup",
    "native/subtitle-studio-overwrite/src/addon.cc",
  ]) {
  ...
  assert.equal(
    compile.args.at(-1),
    "<temporary-output>/subtitle-studio-overwrite.node",
  );
...
test("accepts only absolute typed artifact paths and no positional arguments", () => {
  const parsed = parseBuildArguments([
    "--output",
    "/tmp/subtitle-studio-overwrite.node",
    "--receipt",
    "/tmp/subtitle-studio-overwrite-build.json",
    "--dry-run",
  ]);
  assert.deepEqual(parsed, {
    outputPath: "/tmp/subtitle-studio-overwrite.node",
    receiptPath: "/tmp/subtitle-studio-overwrite-build.json",
    dryRun: true,
  });
...
test(
  ...
  { skip: !MACOS_ARM64 },
  async () => {
    const outputRoot = await mkdtemp("/tmp/fusionkit-subtitle-studio-overwrite-build-failure-");
    const outputPath = path.join(outputRoot, "compile-failure.node");
    try {
      const commandRunner = (command, args, options) => {
        if (args.includes("native/subtitle-studio-overwrite/src/addon.cc")) {
          return {
            exitCode: 1,
            stdout: "",
...
test(
  ...
  { skip: !MACOS_ARM64 },
  async () => {
    const outputRoot = await mkdtemp("/tmp/fusionkit-subtitle-studio-overwrite-build-load-");
...
test(
  ...
  { skip: !MACOS_ARM64 },
  async () => {
    const outputRoot = await mkdtemp("/tmp/fusionkit-subtitle-studio-overwrite-recovery-");
