...
test("allows explicit negative Git probes only after command cleanup is complete", async () => {
  const result = await runBoundedBuildCommand(process.execPath, ["-e", "process.stderr.write('not a repository');process.exitCode=128"], {
    cwd: os.tmpdir(), env: process.env, timeoutMs: 5_000, allowFailure: true,
  });
  assert.equal(result.exitCode, 128);
  assert.equal(result.stderr, "not a repository");
});
