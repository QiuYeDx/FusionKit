...
const MODEL_MANIFEST_PATH = path.join(
  PROJECT_ROOT,
  "resources/subtitle-studio/transcription/manifests/subtitle-studio-models.v1.json",
);
const MAX_HEALTH_BYTES = 4 * 1024;
const MAX_DIAGNOSTIC_CAPTURE_BYTES = 64 * 1024;

export async function runMacosRuntimeSmoke(options) {
  ...
  const workRoot = await mkdtemp(path.join(os.tmpdir(), "fusionkit-studio-macos-smoke-"));
  try {
    ...
    return {
      schemaVersion: 1,
      workPackage: "STUDIO-MACOS-RUNTIME",
...
async function runCli(argv = process.argv.slice(2)) {
  ...
  if (options.help) {
    process.stdout.write(
      "Usage: node run-macos-runtime-smoke.mjs --runtime <canonical-root> " +
        "--model <pinned-large-v3-q5_0.bin> [--backend cpu|metal] " +
        "[--timeout-ms <milliseconds>] [--report <ignored-json>]\n",
    );
    return;
  }
  const report = await runMacosRuntimeSmoke(options);
