...
async function verifyOnWindows(options) {
  const workRoot = await mkdtemp(
    path.join(os.tmpdir(), "fusionkit-studio-ffmpeg-gpgv-windows-"),
  );
  try {
...
async function verifyWithPrivateHome(options) {
  const gpgHome = await mkdtemp(path.join(os.tmpdir(), "fusionkit-studio-ffmpeg-gpg-"));
  ...
  try {
    ...
    await rm(gpgHome, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
  }
}
