...
test("loads the immutable canonical staging roots and artifact naming contract", () => {
  ...
  assert.equal(DEVELOPMENT_RUNTIME_ROOT, "build/subtitle-studio-resources/transcription");
  assert.equal(PACKAGED_RUNTIME_ROOT, "subtitle-studio/transcription");
  assert.equal(
    STAGING_RUNTIME_MANIFEST_RELATIVE_PATH,
    "manifests/subtitle-studio-runtime.v1.json",
  );
  assert.equal(
    STAGING_ARTIFACT_NAME_PATTERN,
...
test("rejects unsupported targets and resolves only the canonical development root", () => {
  ...
  assert.equal(
    runtimeRoot,
    path.join(projectRoot, "build", "subtitle-studio-resources", "transcription"),
  );
  assert.equal(
    resolveRuntimeStagingOutputParent(projectRoot),
    path.join(projectRoot, "build", "subtitle-studio-resources"),
  );
});
