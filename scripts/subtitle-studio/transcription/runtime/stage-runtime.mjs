...
const EVIDENCE_ROOT = path.join(
  PROJECT_ROOT,
  "resources/subtitle-studio/transcription/licenses",
);
...
export async function stageMacosArm64Runtime(options) {
  ...
  const partialRoot = path.join(
    outputParent,
    `subtitle-studio-transcription.partial-${process.pid}-${Date.now()}`,
  );
  ...
  try {
    ...
    const stagedInputs = [
      {
        ...
        codeIdentifier: "com.fusionkit.subtitle-studio.whisper-server",
      ...
      },
      {
        ...
        codeIdentifier: "com.fusionkit.subtitle-studio.ffmpeg",
      ...
      },
      {
        ...
        licenseRef: "ffmpeg-lgpl-2.1-or-later",
        sourceRef: "ffmpeg-8.1.2",
        codeIdentifier: "com.fusionkit.subtitle-studio.ffprobe",
    ...
    const manifest = {
      ...
      manifestId: "subtitle-studio-runtime-darwin-arm64-v1",
...
export async function validateFfmpegBuildReceipt(receipt, inputs) {
  if (
    ...
    receipt.source?.signatureSha256 !== FFMPEG_BUILD_CONTRACT.signatureSha256 ||
    receipt.source?.signatureVerification?.status !== "verified" ||
    receipt.source?.signatureVerification?.fingerprint !== FFMPEG_BUILD_CONTRACT.signingKeyFingerprint ||
    receipt.source?.isolatedFromAncestorGitMetadata !== true ||
    receipt.build?.recipe !== "scripts/subtitle-studio/transcription/runtime/build-ffmpeg-macos-arm64.mjs" ||
    receipt.build?.logicalPrefix !== FFMPEG_BUILD_CONTRACT.logicalPrefix ||
    receipt.build?.deploymentTarget !== FFMPEG_BUILD_CONTRACT.deploymentTarget ||
    JSON.stringify(receipt.build?.configureFlags) !== JSON.stringify(FFMPEG_CONFIGURE_FLAGS) ||
    ...
    !Array.isArray(receipt.build?.externalLibraries) || receipt.build.externalLibraries.length !== 0 ||
