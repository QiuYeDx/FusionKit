...
export class BilingualService {
  ...
  async removeTrack(documentId: string, revision: number, trackId: string, guard: () => void = () => {}) {
    const snapshot = await this.repository.transact(documentId, revision, value => {
      const track = value.document.translationTracks.find(track => track.id === trackId);
      ...
      const recordId = track.executionRef?.id;
      if (recordId && !value.document.translationTracks.some(item => item.executionRef?.id === recordId)
        && !value.tasks.some(task => task.translation?.checkpoint?.version === 2 && task.translation.checkpoint.executionRef.id === recordId)) {
        delete value.executionRecords?.[recordId];
      }
