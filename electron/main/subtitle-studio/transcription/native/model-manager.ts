...
import type { LocalSubtitleManagedResourceSummary } from "../../../../../src/subtitle-studio/transcription/ipc-contract";
...
export class LocalSubtitleModelManager {
  ...
  async #removeVerifiedSource(source: SourceReceipt, model: LocalSubtitleModelManifestEntry): Promise<void> {
    ...
    const quarantinePath = path.join(
      source.parentPath,
      `.fusionkit-studio-model-move-${sanitizeStagingId(this.#stagingIdFactory())}`,
    );
    try {
...
async function resolveOwnedSourceQuarantine(
  ...
  const matches: string[] = [];
  for (const name of names) {
    if (!name.startsWith(".fusionkit-studio-model-move-")) continue;
