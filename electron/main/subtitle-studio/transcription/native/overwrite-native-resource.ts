...
import rawOverwriteStagingContract from "../../../../../resources/subtitle-studio/transcription/manifests/subtitle-studio-overwrite-staging.v1.json";
import {
...
export const LOCAL_SUBTITLE_OVERWRITE_NATIVE_MANIFEST_LEAF =
  "subtitle-studio-overwrite.v1.json" as const;
...
const PROOF_BRAND: unique symbol = Symbol(
  "fusionkit.subtitle-studio.transcription.verified-overwrite-native-addon",
);
...
export interface LocalSubtitleOverwriteNativeManifest {
  ...
  readonly component: "subtitle-studio-overwrite";
  readonly target: {
...
export async function verifyLocalSubtitleOverwriteNativeAddon(
  ...
  const addonGeneration = sha256(Buffer.from(
    `fusionkit.subtitle-studio.transcription-overwrite\0${manifestRead.sha256}`,
    "utf8",
  ));
  const proof = {
    schemaVersion: 1,
...
export function parseLocalSubtitleOverwriteNativeManifest(
  ...
  if (
    ...
    input.component !== "subtitle-studio-overwrite" ||
    ...
    input.artifact.relativePath !==
      `${target.artifactRelativeDirectory}/subtitle-studio-overwrite.${input.artifact.sha256}.node` ||
...
async function validateBuildReceipt(
  ...
  if (
    ...
    value.component !== "subtitle-studio-overwrite" ||
...
function isReceiptArtifact(value: unknown, target: TargetPolicy): boolean {
  ...
  if (!isExactRecord(value, keys)) return false;
  if (
    value.logicalFileName !== "subtitle-studio-overwrite.node" ||
...
function parseOverwriteStagingContract(input: unknown): readonly TargetPolicy[] {
  if (
    ...
    input.component !== "subtitle-studio-overwrite" ||
    input.developmentRuntimeRoot !== "build/subtitle-studio-resources/transcription" ||
    input.packagedRuntimeRoot !== "subtitle-studio/transcription" ||
    ...
    input.artifactLeafPattern !== "subtitle-studio-overwrite.<sha256>.node" ||
