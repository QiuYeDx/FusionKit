import { hasVariantOverlapBudget, planVariantOverlapReview } from "./native/cue-variant-overlap-resolver";
import { planOverlapGroupReview } from "./native/overlap-group-review";
import { overlapGroupSourceOptions, selectOverlapGroupSource } from "./native/overlap-group-selection";
import { summarizeLocalSubtitleCues } from "./native/cue-summary";
import { restoreFinalLocalSubtitleSeparators } from "./native/cue-display-separators";
import { inspectVariantOverlapTiming } from "./native/cue-variant-overlap-evidence";
import { inspectShortOnsetSource, inspectShortOnsetView, inspectShortOnsetConsensus, type ShortOnsetAudio, type ShortOnsetView } from "./native/cue-short-onset-evidence";
import { hasContainedOverlapBudget, planContainedOverlapReview } from "./native/cue-contained-overlap-resolver";
import { inspectContainedOverlapTiming } from "./native/cue-contained-overlap-evidence";
import { randomUUID } from "node:crypto";
import path from "node:path";
import { shouldRetryUnconditionedAudio } from "./native/quiet-audio";
import { planLocalSubtitleOverlapReview, resolveLocalSubtitleOverlap } from "./native/cue-overlap-resolver";
import { hasPrefixOverlapBudget, planPrefixOverlapReview } from "./native/cue-prefix-overlap-resolver";
import { hasMultiOverlapBudget, planMultiOverlapReview } from "./native/cue-multi-overlap-resolver";
import { inspectMultiOverlapTiming } from "./native/cue-multi-overlap-evidence";
import { inspectPrefixOverlapTiming, type PrefixOverlapView } from "./native/cue-prefix-overlap-evidence";
import { needsLocalSubtitleSeparators, needsLocalSubtitleDtwRefinement, restoreLocalSubtitleCueSeparators, refineLocalSubtitleCueWithDtw } from "./native/cue-separator-restorer";
import {
...
} from "../../../../src/subtitle-studio/transcription/domain";
import type {
  LocalSubtitleJobBatchExecutionContext as NativeBatchExecutionContext,
  LocalSubtitleJobBatchRuntime,
  LocalSubtitleJobTaskExecutionContext as NativeTaskExecutionContext,
} from "./native/job-manager";
import {
...
} from "./native/server-supervisor";
import {
  sameLocalSubtitleFileIdentity,
} from "./native/filesystem-object-identity";
import {
...
} from "./native/subtitle-post-processor";
...
interface ProductionBatchRuntimeRecord {
  ...
  readonly owner: TranscriptionBatchExecutionContext["owner"];
  readonly batchId: string;
  readonly config: TranscriptionBatchExecutionContext["config"];
  readonly managedModel: TranscriptionBatchExecutionContext["managedModel"];
  readonly managedVad?: TranscriptionBatchExecutionContext["managedVad"];
...
export type TranscriptionBatchExecutionContext = Omit<NativeBatchExecutionContext, "config"> & {
  readonly config: Omit<NativeBatchExecutionContext["config"], "output" | "postAction">;
};
export type TranscriptionTaskExecutionContext = Omit<NativeTaskExecutionContext, "config"> & {
  readonly config: TranscriptionBatchExecutionContext["config"];
};
...
export class TranscriptionExecutor {
  ...
  constructor(options: TranscriptionExecutorOptions) {
  ...
  beginBatchSlice(
    context: TranscriptionBatchExecutionContext,
  ): LocalSubtitleJobBatchRuntime {
    if (
  ...
  async execute(
    context: TranscriptionTaskExecutionContext,
  ): Promise<TranscriptExecutionResult> {
    ...
    try {
      ...
      const acceptedPrimaryCounts = new Map<string, number>();
      ...
      const dispatchWindow = async (
        ...
        while (true) {
          ...
          let unconditionedFallback = false;
          if (outcome.quietAudioConditioned && shouldRetryUnconditionedAudio(outcome.assessment)) {
            ...
            unconditionedFallback = true;
          }
          const { attempt, assessment, decision } = outcome;
          if (decision.action === "accept") {
            retainAttempt(attempt);
            const count = requestsPerRoot.get(window.rootWindowKey);
            if (!window.retryDepth && !qualityRecoveryAttempts &&
                count === (unconditionedFallback ? 2 : 1)) acceptedPrimaryCounts.set(window.rootWindowKey, count);
            return;
          }
      ...
      if (runtime.target.platform === "win32" && context.config.resolvedBackend === "cuda" &&
        ...
        // A new seam is considered only after every established optional request.
        if (context.config.model.modelId === "large-v3") {
          const review = rootPlan.windows.slice(1).flatMap((rightWindow, index) => {
            const leftWindow = rootPlan.windows[index]!;
            if (!hasPrefixOverlapBudget([leftWindow.rootWindowKey, rightWindow.rootWindowKey],
              rootPlan.windows.length, acceptedPrimaryCounts, requestsPerRoot)) return [];
            const left = attempts.find(a => a.window.windowKey === leftWindow.windowKey);
            const right = attempts.find(a => a.window.windowKey === rightWindow.windowKey);
            if (!left || !right || ![left, right].every(a => ["ja", "japanese"].includes(a.response.result.language.toLowerCase()))) return [];
            const proposal = planPrefixOverlapReview({ sourceIdentity: normalized!.normalizationId, durationMs: normalized!.durationMs,
              leftWindow, rightWindow, leftRaw: left.response.result.segments, rightRaw: right.response.result.segments, cues: segments });
            return proposal && !replacements.has(proposal.leftIndex) && !replacements.has(proposal.rightIndex) ? [proposal] : [];
          })[0];
          if (review) {
        ...
        // Keep prefix repairs first; admit at most one additional complete multi-parent group.
        if (context.config.model.modelId === "large-v3") {
          const review = rootPlan.windows.slice(1).flatMap((rightWindow, index) => {
            const leftWindow = rootPlan.windows[index]!;
            if (!hasMultiOverlapBudget([leftWindow.rootWindowKey, rightWindow.rootWindowKey],
              rootPlan.windows.length, acceptedPrimaryCounts, requestsPerRoot)) return [];
            const left = attempts.find(a => a.window.windowKey === leftWindow.windowKey);
            const right = attempts.find(a => a.window.windowKey === rightWindow.windowKey);
            if (!left || !right || ![left, right].every(a => ["ja", "japanese"].includes(a.response.result.language.toLowerCase()))) return [];
            const proposal = planMultiOverlapReview({ sourceIdentity: normalized!.normalizationId, durationMs: normalized!.durationMs,
              leftWindow, rightWindow, leftRaw: left.response.result.segments, rightRaw: right.response.result.segments, cues: segments });
...
function snapshotPinnedServerRuntimeIdentity(
  ...
  managedModel: TranscriptionBatchExecutionContext["managedModel"],
  managedVad: TranscriptionBatchExecutionContext["managedVad"],
  acceleratorPack: LocalSubtitleVerifiedAcceleratorPack | undefined,
...
function sameManagedResourceIdentity(
  current: TranscriptionBatchExecutionContext["managedModel"],
  requested: TranscriptionBatchExecutionContext["managedModel"],
): boolean {
  return (
...
function sameOptionalManagedResourceIdentity(
  current: TranscriptionBatchExecutionContext["managedVad"],
  requested: TranscriptionBatchExecutionContext["managedVad"],
): boolean {
...
function isSupportedProductionTaskMode(
  taskMode: TranscriptionTaskExecutionContext["config"]["taskMode"],
): boolean {
...
function isSupportedManagedVadContext(
  context: Pick<
    TranscriptionBatchExecutionContext,
    "config" | "managedVad"
  >,
): boolean {
