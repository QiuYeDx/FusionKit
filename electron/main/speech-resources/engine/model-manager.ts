import { SpeechResourceSmokeError, type SpeechResourceModelSmoke, type SpeechResourceVadSmoke } from "../../../../src/speech-resources/contracts";
...
} from "../../../../src/speech-resources/contracts";
import type { LocalSubtitleManagedResourceSummary } from "../../../../src/speech-resources/schemas";
import type { LocalSubtitleOwnerKey } from "../../../../src/speech-resources/contracts";
import {
...
export interface LocalSubtitleModelManagerOptions {
  ...
  readonly smokeVad: SpeechResourceVadSmoke;
...
export class LocalSubtitleModelManager {
  ...
  constructor(options: LocalSubtitleModelManagerOptions) {
    if (!options || typeof options.smokeModel !== "function" || typeof options.smokeVad !== "function") {
    ...
    this.#smokeModel = options.smokeModel;
    ...
    this.#vadManager = options.vadManager === false
      ? undefined
  ...
  async #executeDownload(
    ...
    try {
      ...
      try {
        ...
        throwIfCancelled(context);
        await this.#smokeModel({ owner, model: {
...
function toResourceJobError(error: unknown): LocalSubtitleError {
  ...
  if (error instanceof SpeechResourceSmokeError) {
    return createLocalSubtitleError(error.code, "The speech resource load smoke failed.");
  }
