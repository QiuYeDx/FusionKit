import { type SpeechResourceVadSmoke } from "../../../../src/speech-resources/contracts";
...
import type { LocalSubtitleServerManagedResourceIdentity } from "../../../../src/speech-resources/contracts";
import {
...
export class LocalSubtitleVadManager {
  ...
  constructor(options: LocalSubtitleVadManagerOptions) {
    if (
      ...
      typeof options.smokeVad !== "function" ||
    ...
    this.#smokeVad = options.smokeVad;
  ...
  async #executeInstall(
    ...
    try {
      ...
      throwIfCancelled(context);
      await this.#smokeVad({ owner, model: smokeModel, vad: {
