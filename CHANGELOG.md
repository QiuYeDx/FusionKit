...
- 新增长文本翻译 Beta 工具，支持单个或多个 TXT 文件、快速并发模式、有序项目和批量独立任务队列
- 新增长文本连贯串行模式，支持语义记忆、跨文件记忆延续、文件前记忆重置和费用提示
- 新增长文本任务工作区、事件日志、分片结果落盘、暂停/取消、部分完成、恢复、重启和删除能力
- 新增长文本 TXT 输出模式：仅译文、双语简洁模式、`[Original]` / `[Translation]` 标签模式
- 新增 Markdown parser、保护占位符、Markdown 仅译文组装和 blockquote 双语组装能力
- 长文本翻译页面新增上下文预算校验、执行模式说明、恢复弹窗、项目排序和独立任务队列视图
...
- 端到端 Markdown 翻译入口仍未开放；当前已完成 parser 和输出组装能力
- 长文本翻译仍需真实模型与跨平台手工验收后再移出 Beta
