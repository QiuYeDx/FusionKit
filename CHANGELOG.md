...
- 新增独立的字幕工作台，将字幕文档管理、本地音视频转写、AI 翻译和字幕导出整合到同一工具中；文档与译文保存在本地，支持关闭应用后重新打开
- 字幕工作台支持 SRT、LRC、VTT、ASS 导入与导出，支持拖入字幕文件、批量导入、双语字幕拆分整理、多个译文轨及原文/译文对照预览
- 新增文档搜索、筛选、排序、分页和跨页多选，支持批量翻译、导出、继续、取消及删除；右键已选文档可操作整个选区，右键未选文档只操作该文档
- 新增工作台转写视图：支持拖入音频或视频、音轨选择、模型与 VAD 配置、CPU/CUDA 后端、队列进度和任务取消；转写设置可在重启后恢复，并可选择转写完成后自动翻译，默认关闭
- 新增翻译任务总览，集中显示进行中、已完成和需处理任务、批次进度及模型 API 返回的实际 Token 用量；支持查看任务详情、取消和恢复中断任务
- 字幕预览新增复制菜单，可明确选择原文、译文或双语，并按需附带时间戳；文档预览及转写队列支持打开来源文件夹

### 优化

- 字幕工作台与本地字幕转写工具共用模型、VAD 和兼容的 CUDA 资源，接管已有安装并统一维护下载、取消、删除和占用状态，避免重复下载
- 完善单份与批量导出：默认保存到源文件所在目录，转写文档使用源媒体目录；文件名默认不附加 `.bilingual` 等内容后缀，支持预设或自定义后缀，同名文件默认自动编号，也可显式选择覆盖
- 字幕工作台导出新增“移除媒体扩展名”开关，默认开启，与字幕格式转换工具共用命名规则；例如 `song.wav.srt` 导出为 LRC 时默认命名为 `song.lrc`，关闭后保留为 `song.wav.lrc`
- 导出方式根据实际译文内容调整：全部文档均无译文时默认并限定为仅原文；有译文时默认双语及“未完成处使用原文”，仅译文导出可用原文补齐，双语缺译行仅输出一遍原文；空译文轨不再被视为已有译文
- 导出检查改为独立确认步骤，优先展示可导出数量和格式变化概览，文件详情按需展开；成功、部分成功和失败提示统一为紧凑的结果弹窗，状态图标、标题与说明居中，保留底部完成操作
- 统一导出、翻译和结果弹窗中的文档清单、数量徽标、折叠样式与滚动渐变，压缩侧栏、队列、表单及预览标题的冗余留白，为文档和字幕内容提供更多可视空间
- 转写队列新增批量清理，并区分清理成功任务与清理所有已结束任务；行内操作统一为小号图标按钮，简化 Tooltip，改善按钮和进度条在行悬停时的辨识度
- 本地字幕转写默认采用结合停顿信息的分片策略并补充 VAD 保护，改进句子边界、短语起点、窗口接缝重复和字幕分隔；本地转写预览与任务详情采用更紧凑的布局

### 修复

- 修复字幕工作台无法识别本地字幕转写工具已下载资源、仍提示重新准备运行环境的问题
- 修复翻译进度更新、全选文档及切换预览时触发整页加载和短暂禁用的问题；文档点击立即更新选中状态，预览独立显示平滑加载效果，快速切换时以最后一次选择为准
- 修复工具列表与字幕工作台之间进出时的内容位移，以及文档/转写视图顶部间距不一致引起的跳动
- 修复字幕和媒体拖入流程中的文件授权及排队处理问题，并完善 Windows 保存对话框默认路径大小写识别、来源目录定位和同名文件保护
- 修复结果弹窗关闭过程中内容提前清空或闪回配置表单的问题，保留完整退出过渡及键盘焦点恢复

### 测试与文档

- 补充字幕工作台架构、需求、实施记录和人工验收跟踪，覆盖文档持久化、翻译恢复、共享资源、转写交接及批量工作流
- 扩展四种字幕格式、缺译/空译文/过期译文回退、同名文件策略、资源共享及异步预览的自动化回归，并增加实际 Electron 的拖拽、右键菜单、单份/批量导出和深浅主题检查
- 完成 Windows CPU/CUDA 固定样本对照及共享资源实测；安装包演练、跨平台发布和整体验收仍按独立记录跟进，本次代码合并不代表这些发布检查已完成
...
- 本地字幕转写页新增首次使用引导，覆盖模型准备、转写配置、源语言、媒体添加、任务队列和开始操作；明确提示使用前须在本地环境下载托管模型或导入兼容的 GGML 模型，完成或跳过后不再自动弹出，仍可从页头随时重新打开
...
- 完善工具页配置持久化：本地字幕转写、字幕 AI 翻译、字幕格式转换和文件名翻译会在应用重启后恢复上次的可复用设置，同时继续清理任务、文件/目录授权、结果和错误等临时状态
- 调整 macOS 原生红黄绿窗口按钮的位置，使按钮组与 40px 标题栏中的玻璃胶囊横纵居中对齐

### 修复

- 修复字幕 AI 翻译并发分片刷新 `fusionkit.remaining.*` 等恢复产物时在 Windows 网络盘发生内部 `rename` 竞争并以 `EPERM` 中断整个任务的问题；同一任务的恢复产物改为快照化串行发布，对 `EPERM`/`EBUSY`/`EACCES` 执行有界退避与提交回读确认，辅助产物最终写入失败仅记录警告而不再推翻已成功的翻译结果
- 修复 macOS/APFS 上 Finder 或文件提供器仅更新媒体文件元数据 `ctime` 时，本地字幕任务在 `preparing_media` 阶段误报 `media_changed` 并失败的问题；Darwin 源输入改为按文件对象身份、大小和 `mtime` 判断是否替换，私有快照、PCM 和 Windows 等其他严格身份校验保持不变
- 修复字幕 AI 翻译成功后仍在输出目录保留 `fusionkit-task-*.fusionkit.completed.json` 等恢复临时文件的问题；成功终态现在会清理全部任务恢复产物，并延长 Windows 短暂文件占用时的有界清理重试窗口
- 修复字幕 AI 翻译在模型服务短时不可用、兼容网关返回临时失败或 Responses API 返回 `status: failed` 时过早进入终态失败的问题；单分片改为 7 次有界尝试、可取消的指数退避与随机抖动，并保留鉴权、参数、额度等确定性错误的快速失败
- 修复 Windows 超长路径从资源管理器拖入时 Chromium 将源文件物化为 `%TEMP%` shell 临时代理，导致字幕格式转换/语言提取/文本翻译把源目录误判为临时目录、字幕 AI 翻译及音频输入授权失败的问题；受影响的字幕、文本和音频文件入口统一通过共享安全桥恢复并校验原始文件身份，源目录输出不再落入临时目录
...
- 新增 macOS 输入文件 `ctime` 漂移、Windows 严格文件身份、本地字幕 Tour 持久化与目标节点，以及 macOS 窗口按钮定位的回归测试
