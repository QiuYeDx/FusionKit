...
详情页采用与其它工具详情页一致的两列布局，避免常驻第三列挤压主要操作区：
...
2. 右侧任务区
   ...
   - 当前阶段、文件和分片进度。
   - token / 费用。
   - 准备、开始、取消、恢复、打开输出、打开工作区。
   - 条件化警告和错误详情。

响应式断点应使用 `lg:grid-cols-[320px_minmax(0,1fr)]` 这一类和其它工具页一致的口径；默认初始化视口下不应因为过晚的 `xl` breakpoint 退化为单列。模型可用性保留在页头 badge 和主任务区缺省提示中，不需要单独常驻右侧信息列。
...
3. 用户界面不再把长文本翻译标记为 Beta。页面只展示可操作状态、模型可用性、Markdown 结构保护和资源限制：
   - Markdown 会保护复杂结构。
   - 极复杂 Markdown 建议检查输出。
   - 串行模式更慢且费用更高。
4. 如工作区 schema 升级：
