...
> 2026-07-22 FS-TXN-001 overwrite transaction checkpoint：新增 exact/deep-frozen request、不可结构伪造的 branded Coordinator 与同步 `begin/finalize/rollback` receipt；pending/resolved/rejected thenable 立即拒绝并吸收晚 rejection，terminal 同/跨方法重入被 fencing。Exporter 在任何目录解析或 partial 写入前对缺失 backend fail closed；配置 transaction 后同步执行 `begin → Registry activate → finalize`，activation 失败 rollback，finalize 失败先 revoke artifact 再 rollback，只有 revoke/rollback/partial cleanup 未收敛才映射 `cleanup_failed/cancel_failed`。真实 Registry 连续 read、existing/absent victim、victim 恢复、late cancel 与故障注入已覆盖。聚焦 3 files / 138 passed；local-subtitle 34 passed + 2 skipped files / 768 passed + 2 skipped tests；全量 135 passed + 2 skipped files / 1721 passed + 2 skipped tests；TypeScript、三段 Vite test build、manifest 0/0、validator 17/17 与 diff check 通过。当前只有 TypeScript 编排合同与 path-based test backend；native macOS arm64 / Windows x64 backend、hostile parent replacement、rollback failure recovery、victim recovery 和 packaged validation 均未完成，main 未注入 transaction，Job Manager/Executor 保持 index-only，因此 `FS-TXN-001` 与 `BE-002` 均继续进行中。
...
`FS-TXN-001` 当前只冻结了 main-private 的同步事务编排边界：branded Coordinator 向 native backend 传入目录路径、预期目录 identity、partial/final leaf、partial identity 与 byte size；backend 必须在同一目录 handle 下完成 no-follow victim receipt 与替换，并返回持有该 handle 的 receipt。Exporter 不在 `begin` 与 Registry activation、activation 与 `finalize/rollback` 之间插入 `await` 或 cancellation check；rollback 必须恢复 victim（或原先不存在）并把 exact new inode 放回 partial leaf，供 Exporter 做 identity-bound cleanup。`begin` 抛错或返回非法值前必须已恢复 victim/partial 并释放所有 handle/backup，`finalize/rollback` 抛错必须保留可重试恢复 authority。当前 fake/path-based backend 测试只证明 JS 调用顺序和错误结算，不证明任何 native syscall、目录替换安全或 crash recovery；两平台 native 与 packaged 证据完成前 production 双重 gate 不变。
...
- 已新增 main-private strict/deep-frozen transaction request、WeakSet branded Coordinator 与同步 receipt 状态机。Coordinator 捕获已验证 backend/receipt 方法，拒绝结构对象、原型伪造、subclass、expanded payload、pending/resolved/rejected thenable 和 terminal 重入；失败 receipt 保持 open。
- Exporter 对未配置 backend 的 overwrite 在目录解析/partial 创建前 fail closed；native transaction 与 legacy test adapter 互斥。transaction 路径在同一同步段执行 `begin → Artifact Registry activate → finalize`，activation 失败 rollback，finalize 失败先 revoke ref 再 rollback。已可靠恢复的 finalize failure 为普通 `output_write_failed`；只有 Registry/rollback/partial cleanup 未收敛才使用 `cleanup_failed/cancel_failed`。
- component/real Registry 回归覆盖 existing/absent victim、连续 read、activation/finalize/revoke/rollback 故障、victim 恢复、partial cleanup 和 late cancel；Production Executor 组合回归证明前一格式已 commit 时后一格式 cleanup/cancel failure 不删除可读 artifact。
- 本轮没有 native backend。production backend 必须跨 Registry activation 持有同一个目录 handle，在该 handle 下做 no-follow child 验证、victim receipt、atomic replace、finalize/rollback；begin 失败必须先恢复 victim/partial，rollback 必须恢复 victim/absence 并把 exact new inode 放回 partial leaf。rollback failure 的 recovery authority、Windows exact file identity 与 crash recovery 也必须在接线前冻结。
- 验证为 3 files / 138 focused、local-subtitle 768 passed + 2 skipped、全量 1721 passed + 2 skipped、TypeScript、三段 Vite、manifest 0/0、validator 17/17 和 diff check。fake/path-based test backend 不构成 native、hostile filesystem 或 packaged 证据；main 未注入 transaction，Job Manager/Executor 仍只允许 index。
...
- 未完成：production overwrite 的 macOS arm64 / Windows x64 native backend、hostile parent replacement、rollback/victim recovery 与 packaged validation，以及 CUDA/Metal、VAD、translate、translation handoff、FE 与真实 native/packaged E2E。当前 source output、batch pin、多格式与 overwrite receipt 证据仍是 unit/contract harness，不构成真实 FFmpeg + official server + PRE-006 模型、native filesystem transaction 或 target packaged runtime 证据，完整范围闭环前 `FS-TXN-001` 与 `BE-002` 保持进行中。
...
1. `BE-002` 的 Job Manager foundation、最多 100 文件的 custom/source CPU/no-VAD/index-only 批次、SRT/LRC 保序单/双格式、full/ordinary partial/cancellation partial、逐文件失败隔离、exact-identity batch runtime pin 与可信多父目录 source output 已实现；`FS-TXN-001` 已完成同步 receipt 与 Exporter 接入。下一步实现两平台 plain Node-API directory-handle-relative backend、hostile parent replacement、rollback/victim recovery、resource staging 与 packaged validation，证据齐全后才接通 production overwrite；随后按依赖可用性扩展 CUDA/Metal/VAD/translate。translation handoff、FE 与真实 packaged/native E2E 仍分别由 LINK、FE、`NATIVE-002`/QA 工作包完成，完整验收前不得把 `FS-TXN-001`、`BE-002` 或 M2 标记为已完成。
