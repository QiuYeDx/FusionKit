...
>
> 状态：39个顶层工作包中31个已完成、6个未开始、2个进行中。`LINK-001`～`LINK-008`已完成安全配置迁移、稳定task identity、path-free目录/恢复引用、one-shot artifact handoff、不可变导入回执与三种逐文件后处理模式；`FE-002`已接通完整配置、managed VAD identity/门禁、device-aware preview与本地转英文模式，`FE-004`已完成按格式full/partial结果、预览/reveal/结构化错误详情、自动与手动翻译交接入口。默认仍为`export_only`，自动模式只启动当前回执的`addedTaskIds`，取消后的partial commit不自动交接，可重试转写失败保留批次翻译快照，已删除翻译任务可显式创建新快照重新交接。顶层`MODEL-002`仍等待真实大文件下载与目标环境证据，`FE-002`仍等待VAD/CUDA/Metal真实目标机与packaged产品证据；`QA-001`～`QA-005`和`DOC-001`尚未开始，不提前声明M2/M3、packaged或目标机验收完成
>
...
> 2026-07-27 FS-TXN-001G verified native-addon staging/load + builder contract checkpoint 已完成：独立staging schema与Node/main双解析合同固定两平台N-API/protocol/journal、build receipt、target和签名/hash phase；final bytes以SHA-256命名并no-clobber发布到`overwrite/v1`。main只接受不可结构伪造、generation-bound的opaque proof并在load前后重验root/manifest/receipt/artifact identity；正式builder只消费一个canonical runtime mapping，beforePack以`launch:false`执行official runtime与addon双gate并固定macOS nested-sign hash exclusion。staging/builder 21/21、overwrite-native 29 passed + 1真实Windows skipped、loader focused 79 passed、相关160 passed、local-subtitle 922 passed + 2 skipped、全量1875 passed + 2 skipped、TypeScript、三段Vite、manifest/validator与diff通过；真实macOS production build→ad-hoc sign→stage→fresh load通过。runtime Node为58 passed + 1 skipped + 1个既有fabricated Windows `where.exe` fixture失败，不记为全绿。production main/reauthorization、真实Windows与packaged范围不变。

> 2026-07-27 FS-TXN-001H production overwrite main composition checkpoint 已完成：production bootstrap只消费canonical verified addon proof，固定repository为`<userData>/local-subtitle/recovery/overwrite-recovery.v2.json`；已知resource/backend错误映射`unavailable`，已知repository/recovery错误映射`blocked`且不重写invalid state，未知错误rethrow。ready分支把同一native runtime/recovery owner成对注入Exporter与SessionLifecycle；非ready分支在`app_quit/fatal`时noop、`update`时返回`recovery_pending`。focused 11 files / 313 passed、local-subtitle 40 passed + 2 skipped files / 931 passed + 2 skipped tests、全量141 passed + 2 skipped files / 1884 passed + 2 skipped tests，overwrite-native 29 passed + 1 skipped，manifest 0/0、validator 17/17、TypeScript、三段Vite与diff通过。Job Manager/Executor双重`index-only`和IPC/UI不变；下一步001I接入reauthorization IPC/UI，并先关闭新的reauthorization admission再等待已有recovery tails收敛；真实Windows与packaged范围仍未完成。

> 2026-07-27 FS-TXN-001I reauthorization IPC/UI checkpoint 已完成：strict app-scoped list/recover schema与preload/main bridge、100项tuple-cursor分页、renderer `64 x 100 = 6400` 项上限、stable 12-hex `displayCode`、跨窗口per-recovery picker admission coordinator、目录锁内TTL与temporary authority回收已接通。pending `not_found`继续保留record/fence；begin throw形成且无法证明journal状态的孤立preclaim未来只能通过显式validated discard合同清除，不得自动删除。App全局四语言prompt提供可辨识path-free列表、task failure/focus/visibility刷新、自动聚焦与safe Date fallback。focused 9 files / 175 passed；local-subtitle 42 passed + 2 skipped files / 951 passed + 2 skipped tests；全量145 passed + 2 skipped files / 1921 passed + 2 skipped tests；overwrite-native 29 passed + 1 skipped；manifest 0/0、validator 17/17、四语言各1522 keys、TypeScript、三段Vite与diff通过。001A～001I仍只是component checkpoints，双重`index-only`与顶层状态不变。

> 2026-07-29 FS-TXN-001J Windows x64 protocol v4 real-matrix checkpoint 已完成：固定LLVM-MinGW 20260407、Node 24.14.0 headers与Windows x64 `node.lib`，真实编译/加载N-API v8 production/test-only addon。修复NT-specific journal HANDLE传给`FlushFileBuffers`导致error 5，以及POSIX delete后保留delete-pending HANDLE又按名称复查导致error 5；现在分别使用`NtFlushBuffersFile`和zero-link proof后显式close。production 4/4 terminal、9/9 recovery/open-decision、6/6 rejection；fresh-process 4/4 abandoned-open、4/4 begin crash、4/4 open-recovery arm crash、12/12 rollback crash、12/12 rollback error retry、7/7 finalize error retry、7/7 finalize crash、4/4 acknowledgement crash、4/4 acknowledgement error retry、2/2 conflict。production/test artifact分别为847,360 bytes / `40c666c...c5b2`与849,920 bytes / `fd4eeee8...f82`；报告保持production gate关闭且不声明power-loss safety。

> 2026-07-28 NATIVE-002A macOS arm64 official runtime assembly checkpoint 已完成：新增isolated completely-clean exact-checkout source build/receipt、private-path scan与构建子进程TERM→KILL有界清理，raw server为3,641,216 bytes / `aba72ffc...b1be5d`；nested ad-hoc final server为3,638,160 bytes / `b5227f8b...23b65e`，FFmpeg/ffprobe为`55f36865...bacc53` / `8dfe0a7a...60d09`，runtime manifest为`19271b1f...22530`，protocol-v4 addon为`f54671ef...73ecc2`、generation为`cce55b2e...0530d5`。canonical beforePack static双门禁、CPU + production `large-v3-q5_0` private `/health`、media PCM16 decode与no-PATH通过；artifact为thin arm64、minOS 11、8/8 system-only dependencies并内嵌Metal。当时restricted sandbox中的Metal health因7.33 MiB allocation后SIGSEGV未通过，该环境性失败已由下方002A1 unrestricted复验收敛；Windows、packaged app、Developer ID/公证/Gatekeeper均未验证，因此`NATIVE-002`与双重`index-only`状态不变。

> 2026-07-28 NATIVE-002A1 macOS arm64 Metal production-model verification checkpoint 已完成：002A的Metal失败被定位为restricted sandbox仅7.33 MiB allocation并在private health前触发SIGSEGV；对同一runtime manifest `19271b1f...22530`与production模型`d75795ec...ad1`进行unrestricted hardened复验后，Metal private health/media decode/no-PATH约8秒通过。独立backend probe记录model load 531～547 ms、peak RSS 1,516,158,976～1,535,049,728 bytes、initialization/device=true、failure=false且`backendVerified=true`，CPU hardened smoke通过。只撤销Metal blocker；Windows、两平台packaged与双重`index-only`状态不变。
>
> 2026-07-29 NATIVE-002B Windows x64 official runtime assembly checkpoint 已完成：official CPU server + 12 DLL、FFmpeg/ffprobe、6份evidence与protocol-v4 addon进入canonical base root，manifest为`f8058656...d8798a`；独立CUDA 12.4 candidate固定20个x64 PE / 1,199,083,008 bytes，manifest为`67215097...dbabe`，不进入默认installer/`extraResources`。CPU/CUDA使用production模型`d75795ec...ad1`通过private health、bundled media decode/no-PATH和cleanup；CUDA exact-PID peak VRAM为1,815,437,312 bytes。静态pack合同不自证target smoke，独立target report记录通过。剩余两平台packaged矩阵；CUDA下载/install/update/rollback归`MODEL-002`，NVIDIA分发closure归`QA-005`，双重`index-only`状态不变。

> 2026-07-30 NATIVE-002C macOS arm64 packaged consumption checkpoint 已完成：正式`electron-builder.json`与runtime/addon双`beforePack`门禁生成真实`.app`，只从`Contents/Resources/local-subtitle`消费official runtime和content-addressed protocol-v4 addon。外层ad-hoc签名前后同时冻结runtime manifest/artifact及addon generation/artifact/receipt，fresh-process module probe与system deep/strict codesign通过；production模型`d75795ec...ad1`的CPU和unrestricted Metal private health、bundled media/no-PATH均通过，9类missing/corrupt/wrong-arch/non-executable/launch-identity fault在enqueue前fail closed。该component不声明Developer ID、公证、Gatekeeper accepted、DMG或完整product E2E；剩余packaged范围只保留Windows x64真实app/installer目标机闭环，双重`index-only`状态不变。

> 2026-08-01 NATIVE-002D Windows x64 packaged consumption checkpoint 已完成：正式builder生成真实Windows x64 app与NSIS installer，exact packaged runtime/addon、9类fail-closed、production-model CPU/CUDA smoke、NTFS overwrite integration 15 cases与recovery全矩阵通过。NSIS配置已验证但安装/卸载/更新、数据保留和非ASCII/空格路径未执行，转由`QA-003`手工验收。

> 2026-08-03 production overwrite结项checkpoint：Exporter新增真实conflict-policy capability，Job Manager与Production Executor在同一authority上校验；verified native transaction存在时custom/source允许overwrite，不存在或recovery blocked时在draft capability消费前拒绝。聚焦4 files / 174 tests与TypeScript通过，`NATIVE-002`、`FS-TXN-001`和`BE-002`按实际职责完成；该checkpoint当时M2仍等待`FE-001`，随后状态由下方FE-001结项记录更新。
...
> 2026-08-05 MODEL-002/FE-002 managed VAD与完整配置checkpoint：Job Manager在enqueue、确认式CPU retry与执行前解析、冻结并复验固定managed VAD identity，VAD与model/runtime/backend共同进入queue-admission slice和pin identity，pending/queued/running使用都会阻止删除。Executor把冻结的真实`vadEnabled`送入official server，同时保持v1 `token_timestamps=false`、`segment_only_v1`与mapped segment timeline。renderer配置面板接通device preference、源语言、原文/转英文模式、质量预设、VAD、session-only initial prompt、6项数值高级设置、SRT/LRC、index/overwrite及模型GGML/量化摘要；preview按device preference做generation/response identity guard，VAD启用但固定资源未ready时禁止开始。既有server contract已原生支持`translate_to_english`，本轮只解除Job Manager/Executor production gate。9 files / 256 focused tests、TypeScript、四语言各1777 keys/source usage、renderer/main/preload三段Vite test build与diff check通过；真实VAD下载/native smoke、目标GPU和packaged证据仍未执行，两个顶层包继续保持进行中。
...
当前正式 `electron-builder.json` 已接入唯一canonical `build/local-subtitle-resources/local-subtitle -> local-subtitle` 的 `extraResources` mapping、固定 `beforePack`、macOS arm64 / Windows x64 target、`${arch}` artifact name与macOS runtime `signIgnore`。001G已冻结builder对versioned official runtime与overwrite addon staging的双verifier消费合同，两个verifier都固定`launch:false`并在缺件或target/mapping漂移时fail closed；这只表示正式消费门禁已接通，不表示真实official server/media artifacts已完整组装、启动或完成packaged验证。PRE-005 ignored spike与开发目录二进制仍不是发布证据，`NATIVE-002`继续负责完整artifact assembly、launch/no-PATH和目标平台packaged矩阵。
...
`FS-TXN-001F` 将当前两平台合同升级为protocol v4 / journal v3。Exporter在调用native begin前先写schema-v2 path-free `rollback_unpublished + not_started` preclaim；begin一旦标记开始，任何adoption/receipt validation失败都不得由`releaseAdoption`删除record。Registry activation成功后把durable decision改为`finalize_committed`；该写盘只允许首次尝试加一次完全相同payload的有界重试，两次都失败时保留receipt、Registry activation、record与directory fence，且不得进入native finalize或回退rollback。fresh-process recovery只按durable decision把exact `.open` 原子发布或续跑为`.finalize`/`.rollback` marker，不再让main从leaf布局猜测方向。native收敛后marker仍保留，main先持久化`nativeState=settled`，再调用receipt/module `acknowledge`，最后删除record并释放fence；任何settled持久化不确定都禁止ack。native finalizer对仍为`.open`的receipt保持方向中立，不得隐式选择rollback/finalize；它只可沿已armed的terminal方向best-effort续跑，pending acknowledgement也不得由finalizer执行ack。001I最终收紧`not_found`语义：所有recovery `not_found`都保留record/fence并写`retry_failed`，只有已有durable settled proof后的acknowledgement `not_found`可幂等完成；孤立`rollback_unpublished + not_started` preclaim未来需要显式validated discard合同。001F checkpoint时production main、IPC/UI、verified staging/builder、真实Windows protocol v4与两平台packaged validation均未完成，其中staging/load、main composition与reauthorization IPC/UI分别已由001G～001I关闭，真实Windows protocol-v4矩阵已由001J关闭。

`FS-TXN-001G` 把production addon从developer path升级为独立versioned resource：stager只接受exact build receipt，macOS先完成最终nested signing与独立签名验证，再按最终bytes的SHA-256命名并no-clobber发布到`overwrite/v1`；manifest和receipt共同绑定target、N-API v8、protocol v4、journal v3、size、hash、native format、signature profile与contained path。main verifier持有root/manifest/receipt/artifact的exact identity与addon generation，只返回WeakSet-branded、deep-frozen opaque proof；production load必须消费该proof，并在`dlopen`前后重验current state，拒绝structural copy、proxy、旧generation、symlink、replace、hash/identity drift、test-only surface，以及裸absolute path、PATH或module name authority。正式builder只消费一个canonical `extraResources` mapping，`beforePack`校验process/context/packager target一致，并以`launch:false`执行official runtime与addon双gate；macOS outer signing忽略已经nested-signed且冻结hash的runtime subtree。pre/post revalidation能发现常规漂移，但不宣称把`require(path)`与同用户恶意writer线性化；该子合同也不等于真实Windows或两平台packaged验证。

`FS-TXN-001H` 把上述proof、protocol v4 runtime与schema-v2 composite owner正式组合进Electron main。bootstrap只接受canonical resource environment产生的真实verified proof；production module不暴露dependency injection或裸addon path入口。repository固定为managed root下`recovery/overwrite-recovery.v2.json`，即`<userData>/local-subtitle/recovery/overwrite-recovery.v2.json`，并继续只保存path-free records。已知native resource/backend failure返回`unavailable/native_resource_unavailable`且不触碰repository；已知repository/recovery failure返回`blocked/recovery_state_unavailable`且不重写invalid state；未知错误、非法Coordinator或非法owner直接rethrow。ready runtime把同一个generation的transaction coordinator/recovery owner成对注入Exporter，并把同一个owner作为SessionLifecycle target；非ready runtime在`app_quit/fatal`时noop，在`update`时以`recovery_pending` fail closed。bootstrap只加载pending摘要，不扫描用户目录、不替用户选择恢复目录，也不在缺少reauthorization时执行native recovery。

`FS-TXN-001I` 在该 composition 上接通 strict path-free recovery IPC 与 app-scoped UI。public list 只返回稳定 tuple cursor 和 path-free summary，preload-private recover 才可打开 main-owned picker；跨窗口 admission 在 picker 前登记，并在 authorize/resolve 每个 await 后同时复核 admission identity 与 owner。shutdown/owner-release先关闭仍在 selecting 的 admission，已进入 recovering 的操作继续由 owner tail 收敛；temporary output draft始终在`finally`撤销。authoritative tuple在picker后重新读取，目录 authorization TTL在同 recovery tail与目录锁内复核。pending recovery的`not_found`统一写回`retry_failed`并保留record/fence；孤立`rollback_unpublished + not_started` preclaim若未来需要清理，必须新增显式可验证discard合同，不能自动删除。
...
2026-08-04完成`LINK-002`基础合同：普通、Agent与恢复任务通过受控factory生成稳定taskId，编辑接口不能换绑身份；queue/Store/UI/token estimate/renderer event/main active controller/cancel/checkpoint全部按ID定位，同名任务互不覆盖。根级模型字段已收敛为`ready | needs_configuration` execution binding，renderer与main都在执行前校验完整ready字段，main额外拒绝非法ID和重复active identity。内存import ledger将candidate binding、owner、snapshot、handoffKey与taskId绑定，queue insert和reservation同步提交；相同owner/snapshot的完整请求即使重签receiptId也返回原始不可变回执，冲突在任何新候选入队前拒绝，删除任务不清除同snapshot幂等记录。`startTasks(taskIds)`只处理请求ID并返回started/waiting/not-started/failure分区；RecoveryDialog不再调用`startAllTasks()`。该checkpoint仍保留legacy path字段，目录capability和无路径source/target合同继续由`LINK-003`～`LINK-005`迁移。

2026-08-04完成`LINK-003`底层合同：独立`subtitle-translation:*`fixed preload API注册document owner session，legacy generic bridge拒绝整个namespace；main registry分别管理draft目录token与owner/taskId/leaf/TTL绑定的task target handle，draft提升后撤销不影响task authority。`generated_task_v1`只含`generated_content` marker和`authorized_directory` opaque ref，和严格`legacy_path_v1`互斥；main登记的generated taskId不能回退legacy，token/label/fileName换绑、跨owner、终态与raw path混装均拒绝。目录canonical identity在解析、启动及最终实际leaf写入前重验，leaf额外拒绝分隔符、控制字符、设备名、超长、escape与symlink；同owner active task可原子轮换handle，失败/取消保留旧authority，过期后可重新授权。owner/reload、终态与TTL负责清理。本checkpoint只建立底层能力，普通页面/Agent生产者与RecoveryDialog/checkpoint消费者仍按`LINK-004`、`LINK-005`迁移；旧path与`outputURL`继续保留。

2026-08-04完成`LINK-007B`导入闭环：preload固定方法私有携带local与translation两个owner session，main分别验证同一sender/frame后一次性消费token；candidate identity从artifact稳定task/generation/format/size/hash与snapshot派生，不受artifactRef安全轮换影响。source target使用handoff时冻结的main-only目录proof并在候选生成时复核，custom target使用可多候选复用且独立释放的snapshot lease。main先创建不可执行candidate target，renderer ledger同步提交成功后再幂等commit；duplicate、queue异常和未转移candidate显式释放，releaseBatch只释放snapshot/lease/ledger reservation而不撤销`addedTaskIds`。协调器冻结execution binding与估算输入，构造自包含`manifest_fragments`任务，返回满足started/waiting/not-started分区不变量的不可变receipt；auto-start只调用`startTasks(addedTaskIds)`，相同handoff重试返回原receipt且不重复start。执行时剥离兼容path字段并携带opaque reference，删除/清空生成任务通过owner-bound fixed API有界重试释放handle。普通页面/Agent/Recovery producer/consumer cutover仍归`LINK-004`/`LINK-005`。

2026-08-04进入`LINK-004`并完成普通页面producer checkpoint：新增严格`authorized_task_v1` source/target ref。preload固定方法只对真实`File`私有提取path；main授权input file时冻结文件和parent directory object identity，并在两次identity复核之间读取有界UTF-8正文，renderer只得到正文、展示名和opaque token。注册时input、output mode、custom draft/source parent、output leaf与taskId原子绑定；执行请求剥离兼容path字段，main按同sender authority恢复运行时路径，source reveal和删除清理走fixed API。普通页面不再读取或写入`outputURL`来创建任务，但该旧值仍为Recovery兼容保留。Agent raw path尚未升级为authority；后续必须先实现main-owned`subtitleSelectionRef`或用户确认扫描再切换Agent，不能直接把现有`filePaths`/`scanId`签成token。

2026-08-05完成`LINK-004` Agent producer：`queue_subtitle_translate`不再接受`filePaths`、`scanId`或raw `outputDir`，而是通过固定原生多文件picker由main签发owner-bound、短TTL `subtitleSelectionRef`和逐项`itemRef`。正文读取和任务注册都要求同一receipt binding，注册后任务只保存空兼容path与`authorized_task_v1`；custom输出通过固定目录picker，每个任务取得独立target handle。未消费selection与custom draft保留opaque handle做有界撤销重试，owner结束和TTL仍负责最终清理。convert/extract和Recovery旧流程未被静默升级；legacy path与`outputURL`继续等LINK-005全消费者cutover。
...
`NATIVE-002A` 已在macOS arm64真实组装该development canonical root：official server、FFmpeg/ffprobe、6份license/source evidence与versioned overwrite addon共用唯一builder mapping。server staging必须先验证exact source-build receipt；所有nested executable签名后再冻结final bytes。CPU使用production `large-v3-q5_0`完成私有loopback `/health`，bundled media完成PCM16 decode/probe且不依赖PATH。`NATIVE-002A1`随后以同一runtime manifest `19271b1f...22530`和production模型`d75795ec...ad1`完成unrestricted hardened Metal private health/media decode/no-PATH；约8秒ready，独立backend probe证明model load 531～547 ms、peak RSS 1,516,158,976～1,535,049,728 bytes、initialization/device=true、failure=false与`backendVerified=true`，CPU hardened smoke仍通过。002A在restricted sandbox中的7.33 MiB allocation/private-health前SIGSEGV不再是Metal product blocker。

`NATIVE-002B` 已在Windows x64真实组装同一development canonical base root：official CPU server + 12 DLL、FFmpeg/ffprobe、6份license/source evidence与versioned overwrite addon均由strict verifier消费；base manifest为`f8058656...d8798a`。CUDA 12.4采用独立on-demand accelerator root，固定official archive及20个selected x64 PE，pack manifest为`67215097...dbabe`，不进入默认installer/`extraResources`。CPU/CUDA均用production模型完成private health、bundled media decode/no-PATH与cleanup；CUDA以exact-PID GPU memory counter取得1,815,437,312 bytes peak VRAM并fail closed。静态pack manifest保持`targetSmokeStatus=pending`，因为它只自证artifact integrity；独立smoke report记录外部target通过。以上两平台结果仍是target assembly/launch证据，不是packaged app证据。

`NATIVE-002C` 已把macOS arm64从canonical assembly推进到真实packaged component：正式builder生成`.app`并通过official runtime + overwrite addon双beforePack gate；packaged verifier只接受`Contents/Resources/local-subtitle`及真实`app.asar`，启动manifest中的server/FFmpeg/ffprobe并fresh-process加载content-addressed addon。外层ad-hoc签名前后冻结两类native bytes，system `codesign --verify --deep --strict`、production-model CPU/Metal health、bundled media/no-PATH与9类fail-closed fault matrix全部通过。该结果仍不等于Developer ID/公证/Gatekeeper accepted或完整UI product E2E；Windows x64 app/installer与目标机矩阵仍须由目标机闭环。
...
- `readArtifactText` / `revealArtifact` 已作为 app-scoped built-in IPC 接通；`LINK-006`随后接通`handoffArtifact`与one-shot import token，仍未读取字幕翻译Store或创建target handle。
...
- macOS arm64与Windows x64 protocol v4 backend跨Registry activation持有同一个目录handle，在该handle下做no-follow child验证、victim receipt、atomic replace、finalize/rollback；journal v3绑定exact `transactionId`与canonical partial leaf并提供`.open/.finalize/.rollback` marker。fresh-process recover只接收ID、重新授权目录identity与durable decision；module/receipt acknowledgement只在main durable settled proof后删除terminal marker。native finalizer对`.open`保持方向中立，只续跑已armed terminal且不自行ack。001G～001J已完成verified staging/load、production composition、reauthorization及真实Windows crash/retry矩阵；`NATIVE-002C`/`NATIVE-002D`已分别完成macOS/Windows packaged addon消费与回归。2026-08-03以Exporter真实transaction authority完成production admission。
- 001C验证为 native 11/11；7 files / 257 focused、local-subtitle 37 passed + 2 skipped files / 883 passed + 2 skipped tests、全量 138 passed + 2 skipped files / 1836 passed + 2 skipped tests、TypeScript、三段 Vite test build、manifest 0/0、validator 17/17 与 diff check 通过。macOS raw component仍明确 `finalizeCrashRecoveryClaimed=false`、`powerLossSafetyClaimed=false`；001C当时main未注入transaction，该缺口已由001H composition关闭，但Job Manager/Executor仍只允许index。
- 001G验证：new staging/builder 21/21；overwrite-native 29 passed + 1真实Windows skipped；loader focused 2 files / 79 passed，相关4 files / 160 passed；local-subtitle 39 passed + 2 skipped files / 922 passed + 2 skipped tests；全量Vitest 140 passed + 2 skipped files / 1875 passed + 2 skipped tests；TypeScript、renderer/main/preload三段Vite、manifest 0/0、validator 17/17与diff gate通过；真实macOS production build→ad-hoc sign→stage→fresh load通过。runtime Node为58 passed + 1 skipped + 1个既有fabricated Windows `where.exe` fixture失败，该失败不是本次引入，因此整体不记为全绿；真实Windows和两平台packaged证据仍由后续工作包单独闭环。
- 001H验证：focused 11 files / 313 passed；local-subtitle 40 passed + 2 skipped files / 931 passed + 2 skipped tests；全量Vitest 141 passed + 2 skipped files / 1884 passed + 2 skipped tests；overwrite-native 29 passed + 1 skipped；manifest 0 error / 0 warning、validator 17/17、TypeScript、renderer/main/preload三段Vite与diff gate通过。覆盖proof-only bootstrap、versioned repository、path-free pending load、已知错误分类、未知错误rethrow、production export surface与main成对注入；不替代001I reauthorization、真实Windows或两平台packaged验证。
- 001I验证：focused 9 files / 175 passed；local-subtitle 42 passed + 2 skipped files / 951 passed + 2 skipped tests；全量Vitest 145 passed + 2 skipped files / 1921 passed + 2 skipped tests；overwrite-native 29 passed + 1 skipped；manifest 0 error / 0 warning、validator 17/17、四语言各1522 keys、TypeScript、renderer/main/preload三段Vite与diff gate通过。覆盖strict分页、private recovery、picker/admission/shutdown竞态、temporary draft回收、`not_found` fail-closed、四语言UI与safe Date fallback；不替代真实Windows或两平台packaged验证。
- 001J验证：真实Windows addon build/load 7/7且0 skipped；production 4/4 terminal、9/9 recovery/open-decision、6/6 rejection；fresh-process 4/4 abandoned-open、4/4 begin crash、4/4 open-recovery arm crash、12/12 rollback crash、12/12 rollback error retry、7/7 finalize error retry、7/7 finalize crash、4/4 acknowledgement crash、4/4 acknowledgement error retry、2/2 conflict。production/test artifact分别为847,360 bytes / `40c666c...c5b2`与849,920 bytes / `fd4eeee8...f82`；overwrite focused Vitest 220/220、TypeScript、三段Vite、manifest 0/0、validator 17/17与diff check通过。报告明确production gate不变且不声明power-loss safety；不替代canonical runtime或packaged验证。

### BE-002：Job Manager、Production Executor 与会话生命周期（已完成）
...
- `LocalSubtitleProductionExecutor` 仍按单 task context 执行，但 Job Manager 可把最多100个task作为一个immutable managed-model、optional managed-VAD、transcribe或translate-to-English、CPU/Metal/CUDA、custom或source、index/conditional-overwrite batch依次调度；每个task可请求SRT-only、LRC-only或保序无重复双格式，并按冻结post-action逐文件交接。Job Manager为当前queue admission execution wave持有main-only opaque runtime token；Production Executor只有在首个实际任务完成media normalization、managed model/VAD revalidation和server runtime revalidation后才lazy acquire Supervisor batch pin，入队、等待和retry preflight都不能提前占用模型。pin绑定owner、batchId与完整exact load identity，包括runtime root/generation/target、server artifact path/hash/version/signature、managed model/VAD identity、backend和process options；后续task的fresh proof必须与pin完全兼容。每个task再从pin获取短生命周期task lease，并为每次dispatch绑定normalization、完整structural descriptor、唯一brand、前后file identity/SHA/size、root-plan-local `windowAttempt`、Supervisor `processEpoch`、实例级单调`requestGeneration`、response与admission时冻结的media runtime generation；raw gate的split children使用exact retry geometry，成功attempts才进入canonical post-processing。
- source output不创建batch directory lease。每个task的committed input lease必须同时具备`transcribe`与`derive_source_output`；main在input authorization时从canonical file path冻结parent object identity，后续重验exact owner/task/token/operation/TTL、file identity、canonical containment与相同目录对象。Executor在normalization/pin前fail-fast，丢弃raw path但保留无路径identity proof；SUB-002 exporter的每次目录请求继续重新解析且必须匹配该proof。输入文件identity失败保留`media_changed/preparing_media`，父目录availability/identity失败使用task-scope `output_write_failed/exporting`，TTL失效保留`authorization_expired/preflight`；terminal capability renewal failure不得覆盖executor已产生的稳定执行错误。renderer state、event与IPC不返回token或raw path。
- production output conflict policy由Exporter统一声明：`index`始终支持，`overwrite`只在verified native transaction或显式test adapter存在时支持。Job Manager在capability消费前查询，Production Executor在batch slice和execute再次查询；unavailable/blocked packaged runtime继续fail closed。
...
- 结项边界：Windows packaged component已由`NATIVE-002D`完成；VAD与本地转英文已由2026-08-05 checkpoint接通，translation handoff与页面分别由`LINK-008`和`FE-001`～`FE-004`闭环。NSIS生命周期、非ASCII路径与完整UI product E2E仍归QA，不能用源码级与组件测试替代目标环境证据。

### FE-001：独立工具页（已完成）

- 独立subtitle category、route、tool metadata、menu和四语言i18n已接入，与Audio provider/profile/route完全分离。
- 页面entry并行读取runtime与managed resources，不触发模型加载；只允许already-ready model进入production单文件CPU/no-VAD/SRT/index/export-only请求。
- 输入与自定义输出继续使用preload固定方法签发opaque capability；renderer不读取raw path，也不使用generic invoke。
...
2026-08-04完成`LINK-001`/`LINK-007A`配置快照checkpoint：新Store只持久化语言、输出/切片/冲突/并发偏好和path-free目录展示名，API Key/endpoint/capability/token/raw path均不进入；legacy `outputURL`及旧key继续保留。两个Store模块都在Zustand hydrate前触发幂等bootstrap并做exact readback，失败时协调器fail closed，同会话恢复写入后仍要求clean reload。协调器已冻结脱敏summary、私有model fields和可选custom lease，并覆盖TTL、release fence、并发合并与失败重试。该checkpoint当时不创建`taskId`、`handoffKey`或target handle，不调用`startAllTasks()`；后续`LINK-002`已补齐task identity与精确启动，`LINK-003`已补齐目录capability与无路径target ref，`LINK-007B`已完成真实one-shot消费、target handle所有权转移、完整receipt与exact start。

### LINK-002：稳定任务身份与精确启动（已完成）

- `SubtitleTranslatorTask`强制稳定taskId；普通、Agent与恢复生产者统一走受控factory，Store编辑不能改写ID。
- queue operation、同名任务隔离、token estimate、progress/failure/resolved事件、main active controller/cancel和checkpoint taskId均以稳定ID为准；main拒绝非法或重复active identity。
- execution binding使用`ready | needs_configuration`判别联合；单个、全部和按ID批量启动均在effect前拒绝待配置或字段不完整的ready binding。
- import ledger在内存保存脱敏receipt与owner/snapshot/handoff/candidate binding，支持原子reservation、partial add、跨receipt精确重放、冲突预检、任务删除后幂等和snapshot release；不记录正文、密钥或路径。
- RecoveryDialog自动启动只使用本次恢复批次返回的`addedTaskIds`；未来自动交接只能复用相同精确启动入口，不得调用`startAllTasks()`。
- 本包未迁移目录授权、generated source/target ref或legacy checkpoint/path消费者；这些仍严格按`LINK-003`～`LINK-005`顺序完成。

### LINK-006：Artifact ref 与 one-shot import token（已完成）

- Artifact Registry复用同会话有效ref；只有已过期ref才在完整复核directory/file identity、size、SHA、UTF-8、格式和cue后轮换。旧ref继续稳定返回`artifact_expired`，task revoke/owner release后不允许补发。
- completed session snapshot只对成功复核的过期ref发布revisioned replacement，并原子同步`artifactResults`与`completion.artifacts`；单个artifact变化或失效不阻断整个snapshot。
- `handoffArtifact`把已验证字幕复制到main内Buffer并签发owner-bound、短TTL、one-shot `translationImportToken`；公开响应只有token与expiry，私有consume才得到内容、格式、展示名、cue数和path-free artifact identity，消费阶段不重读路径。
- token registry与artifact registry保持分离；handoff service跟踪task-owned token，result clear/task remove先撤销未消费token并清零Buffer，再撤销artifact ref。owner end、TTL、消费成功或失败也通过同一dispose收敛，token不可重放。
- production JobManager、IPC与session bridge显式共享同一组artifact/token/handoff实例；本包未读取翻译Store，未创建taskId、handoffKey、target handle或translation task。
...
35. 不得在schema-v2 durable preclaim成功前调用native begin，也不得在begin已标记开始后由`releaseAdoption()`删除record/fence；terminal marker只有在main成功持久化`nativeState=settled`后才可ack。recovery返回`not_found`必须保留record/fence；只有已有durable settled proof后的acknowledgement `not_found`可幂等完成。孤立`rollback_unpublished + not_started` preclaim未来若需清除，必须新增显式、可验证的discard合同。
