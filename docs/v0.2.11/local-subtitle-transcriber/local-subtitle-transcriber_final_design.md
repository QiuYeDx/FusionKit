...
- 当前 production output conflict policy 仅放行 `index`；shared schema 与 standalone exporter 保留 `overwrite` 只表示未来合同/组件行为，不表示已接入 Job Manager 或具备 hostile filesystem replacement 安全证据。
...
- 验证：source output 聚焦 5 files / 160 passed；local-subtitle 33 passed + 2 skipped files / 681 passed + 2 skipped tests；全量 Vitest 134 passed + 2 skipped files / 1633 passed + 2 skipped tests；TypeScript、renderer/main/preload 三段 Vite test build、PRE manifest 0 error / 0 warning、validator 17/17 与 diff check 通过。Vite 只有既有 dynamic-import/chunk-size warning，2 个 skip 均为未启用的真实 native server tests。canonical runtime staging 因 Git 忽略的 canonical runtime path 缺失而按合同 fail closed，不能替代 `NATIVE-002` / QA 证据。
- 未完成：production overwrite 的目录句柄相对事务、LRC/多格式及 partial output 组合、CUDA/Metal、VAD、translate、translation handoff、FE 与真实 native/packaged E2E。当前 source output 与 batch pin 证据仍是 unit/contract harness，不构成真实 FFmpeg + official server + PRE-006 模型或 target packaged runtime 证据，完整范围闭环前 `BE-002` 保持进行中。
...
1. `BE-002` 的 Job Manager foundation、最多 100 文件的 custom/source CPU/no-VAD/SRT/index-only 批次、逐文件失败隔离、exact-identity batch runtime pin 与可信多父目录 source output 已实现；下一步补 LRC/多格式和依赖可用后的 CUDA/Metal/VAD/translate 路径。production overwrite 还需要目录句柄相对的 native transaction。translation handoff、FE 与真实 packaged/native E2E 仍分别由 LINK、FE、`NATIVE-002`/QA 工作包完成，完整验收前不得把 `BE-002` 或 M2 标记为已完成。
