...
`FS-TXN-001D` 已实现Windows x64 native parity：verified directory HANDLE跨transaction retained，所有child operation都绑定`RootDirectory`并使用`OBJ_DONT_REPARSE`；existing victim用exact hard-link backup + POSIX replace，absent victim用no-replace rename，rollback以identity-bound HANDLE收敛。Windows身份只接受固定宽度lowercase volume serial/FileId；creation time因NTFS tunneling不属于identity，128-bit FileId也不得压成JS safe-number。fresh-process rollback crash/retry矩阵已通过，但finalize crash仍只保留显式unsupported boundary。production main尚未实例化 native runtime/repository/owner，也没有 reauthorization IPC/UI；durable preclaim、abandoned/open receipt discovery、finalize-crash durable decision、Registry/authorization Windows identity composition与 packaged validation完成前，Job Manager/Executor继续只允许 index。
...
本节保留架构层面的阶段划分；可认领的工作包、依赖、状态、验证和实施记录以 `local-subtitle-transcriber_execution_plan.md` 为唯一执行台账。Execution Plan 已于 2026-07-16 建立，当前 39 个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`与`BE-002`进行中，21个未开始）。M1 的 schema、resource staging、IPC/capability、renderer session、official server contract、真实 Supervisor 生命周期、media normalization/PCM proof、canonical post-processing、标准字幕原子产物和 managed model 合同均已冻结。`BE-002` 的 index-only production 批次已形成；`FS-TXN-001A`～`FS-TXN-001D` 已完成 macOS arm64 native/cooperative-writer child-leaf/protocol v3/journal v2/composite owner/exact reauthorization recovery，以及Windows x64 HANDLE-relative/reparse/no-follow/lossless FileId/fresh-process recovery组件合同，component checkpoint不增加顶层工作包数量。总包仍等待 durable preclaim/abandoned/finalize-crash decision、Registry/authorization Windows identity composition、production main/IPC composition、generation-bound staging/load、builder与两平台 packaged validation。在这些证据完成前 production overwrite不可用。`NATIVE-002`、`MODEL-002` 与 `LINK-001` 可按各自里程碑继续推进。Windows personal distribution 的 unsigned profile已明确；Developer ID、公证和 Gatekeeper accepted只由未来 `QA-004` 验收macOS分发产物。
...
- 未完成：production overwrite 的 durable preclaim/open decision、finalize-crash durable decision、Registry/authorization/recovery selection的Windows exact identity composition、generation-bound staging/load、main/reauthorization IPC/UI injection与packaged validation，以及 CUDA/Metal、VAD、translate、translation handoff、FE与真实 native/packaged E2E。当前 source output、batch pin、多格式、overwrite receipt、composite owner与两平台raw native component证据仍不构成真实 FFmpeg + official server + PRE-006模型及target packaged runtime的完整product E2E；non-cooperative child writer safety明确不属于当前或未来本合同的保证范围。完整范围闭环前 `FS-TXN-001` 与 `BE-002` 保持进行中。
...
1. `BE-002` 的 Job Manager foundation、最多 100文件的custom/source CPU/no-VAD/index-only批次、SRT/LRC保序单/双格式、full/ordinary partial/cancellation partial、逐文件失败隔离、exact-identity batch runtime pin与可信多父目录source output已实现；`FS-TXN-001A`～`FS-TXN-001D` 已完成macOS arm64与Windows x64 developer components、strict loader、protocol v3 / journal v2、cooperative-writer child-leaf边界、test-only terminal/crash harness、durable `rollback_pending` recovery、composite owner/file repository、exact reauthorization recovery与lossless Windows volume/FileId合同。下一步回到可在Mac继续的cross-platform composition：durable prepared preclaim/open/finalize decision、Registry/authorization Windows identity联合类型、generation-bound resource staging/load、builder、production main与reauthorization IPC/UI；两平台packaged validation后才评估gate放行。完整验收前不得把`FS-TXN-001`、`BE-002`或M2标记为已完成。
