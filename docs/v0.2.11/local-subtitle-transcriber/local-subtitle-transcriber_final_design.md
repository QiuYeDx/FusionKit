...
>
> 状态：调研、Final Design、`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002` 与 `MODEL-001` 已完成。production decision record 固定 `whisper.cpp v1.9.1`、Node-managed HTTP contract v1、`large-v3-q5_0` 首发默认、跨平台 FFmpeg 8.1.2、Windows unsigned personal profile 与目标平台矩阵；shared schema、resource staging、preload/IPC/capability、renderer session、official server contract、真实 Supervisor 生命周期、media normalization/PCM proof、canonical post-processing、SRT/LRC 原子导出、Artifact Registry 与 managed model 合同已冻结。`BE-002` 已接通最多 100 文件的 CPU/transcribe/no-VAD、custom 或 source、index-only、export-only 批次，可按请求顺序生成 SRT-only、LRC-only 或无重复双格式，并稳定结算 full、普通 partial 与 commit 后取消 partial；逐文件失败隔离、owner 分区续权、exact-identity batch runtime pin 与可信多父目录 source output 已实现。`FS-TXN-001A`～`FS-TXN-001E` component checkpoints 已完成：macOS arm64 冻结 protocol v3 / journal v2、exact transaction ID、main/Registry composite recovery owner、路径无关的 file repository、Exporter component handoff、owner release/shutdown retry 与重新授权后的 exact lazy recovery；Windows x64 冻结 directory HANDLE + RootDirectory-relative NT child operations、reparse/no-follow、lossless volume/FileId、existing/absent terminal、same-receipt retry与fresh-process recovery；POSIX/Windows exact filesystem identity已贯通输出目录authorization、Artifact Registry、Exporter、recovery selection与Executor directory proof。prepared handoff 仍只提供同进程 single-claim，不是 durable preclaim；production main 尚未实例化该 runtime，verified staging/main injection、durable finalize decision 与 packaged validation也未完成。Job Manager / Production Executor 继续保持双重 `index-only` gate，`FS-TXN-001` 与 `BE-002` 均保持进行中
>
...
本 checkpoint 的跨进程 durable terminal decision 只有 `rollback_pending`：rollback 的第一次 namespace mutation 前原子发布 intent，fresh process 只重放 victim/absence 恢复与 exact new leaf cleanup。`finalize_pending` 只在当前 native/TypeScript receipt 内锁定同方向重试；Exporter 重试一次仍失败时保留已激活 Registry commit 方向，禁止 revoke 后反向 rollback，但该内存状态不是 durable cross-process commit decision。有效、request-matching 且仍存在的 open journal返回 `decision_required`；malformed、replaced或 request-mismatching journal必须拒绝。finalize crash若发生在 journal unlink后则可能返回 `not_found`，所以本 checkpoint不提供 finalize-crash recovery。不得根据当前 leaf 布局猜测用户已经选择 finalize 或 rollback。

`FS-TXN-001C` 已实现 component-level composite recovery owner：唯一 branded owner绑定 native receipt/journal authority、Artifact Registry reservation/activation state与 owner/task/generation/format，持久文件只含 opaque/path-free metadata；Exporter在 begin前准备同进程 handoff，terminal或 Registry未收敛时执行 single-claim adoption，目录 mutex/fence、owner release、shutdown all-settled retry和shutdown后 late adoption均已覆盖。prepared handoff只存于进程内 `WeakMap/Map`，既不在 begin前持久化，也不关闭“native已修改 namespace、正式 adopt尚未持久化”时的进程退出窗口，因此只能称 component-level prepared handoff。重新授权后，`decision_required` 与 `not_found` 都保留 pending record、recovery direction 和已选目录 selection fence，二者都让 shutdown保持 `recovery_pending`；只有 native 与 Registry authority 真正收敛后才释放 fence。GC finalizer仍只能 best-effort。

`FS-TXN-001D` 已实现Windows x64 native parity：verified directory HANDLE跨transaction retained，所有child operation都绑定`RootDirectory`并使用`OBJ_DONT_REPARSE`；existing victim用exact hard-link backup + POSIX replace，absent victim用no-replace rename，rollback以identity-bound HANDLE收敛。Windows身份只接受固定宽度lowercase volume serial/FileId；creation time因NTFS tunneling不属于identity，128-bit FileId也不得压成JS safe-number。fresh-process rollback crash/retry矩阵已通过，但finalize crash仍只保留显式unsupported boundary。`FS-TXN-001E` 已把该identity贯通输出目录authorization、Artifact Registry、Exporter和reauthorization recovery selection。production main尚未实例化 native runtime/repository/owner，也没有 reauthorization IPC/UI；durable preclaim、abandoned/open receipt discovery、finalize-crash durable decision与 packaged validation完成前，Job Manager/Executor继续只允许 index。
...
- overwrite recovery repository只保存 schema、opaque recovery ID、owner fingerprint、task/generation/format、terminal direction、Registry/native状态与时间戳；原子 rename后若 parent sync失败，只有 exact payload read-back一致才可接受，且不得把这项验证扩张为 power-loss声明。重启后必须由用户重新选择输出目录并重验 directory object，再按 exact ID lazy recover；`decision_required` 与 `not_found` 都保留 recovery record 和已选目录 fence，只有 native 与 Registry authority 真正收敛后才允许删除 record 并释放 fence。当前这套 owner/repository/reauthorization只完成组件合同，production main/IPC/UI尚未接线。
...
本节保留架构层面的阶段划分；可认领的工作包、依赖、状态、验证和实施记录以 `local-subtitle-transcriber_execution_plan.md` 为唯一执行台账。Execution Plan 已于 2026-07-16 建立，当前 39 个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`与`BE-002`进行中，21个未开始）。M1 的 schema、resource staging、IPC/capability、renderer session、official server contract、真实 Supervisor 生命周期、media normalization/PCM proof、canonical post-processing、标准字幕原子产物和 managed model 合同均已冻结。`BE-002` 的 index-only production 批次已形成；`FS-TXN-001A`～`FS-TXN-001E` 已完成 macOS arm64 native/cooperative-writer child-leaf/protocol v3/journal v2/composite owner/exact reauthorization recovery、Windows x64 HANDLE-relative/reparse/no-follow/lossless FileId/fresh-process recovery，以及authorization/Artifact Registry/Exporter/recovery selection cross-platform exact identity composition，component checkpoint不增加顶层工作包数量。总包仍等待 durable preclaim/abandoned/finalize-crash decision、production main/IPC composition、generation-bound staging/load、builder与两平台 packaged validation。在这些证据完成前 production overwrite不可用。`NATIVE-002`、`MODEL-002` 与 `LINK-001` 可按各自里程碑继续推进。Windows personal distribution 的 unsigned profile已明确；Developer ID、公证和 Gatekeeper accepted只由未来 `QA-004` 验收macOS分发产物。
...
- Exporter 对未配置 backend 的 overwrite 在目录解析/partial 创建前 fail closed；native transaction 与 legacy test adapter 互斥。transaction 路径在同一同步段执行 `begin → Artifact Registry activate → finalize`：activation失败rollback；finalize/rollback或 Registry authority未收敛时移交给 branded composite owner，同一 receipt只能被 claim一次。prepared handoff在 begin前保留 recovery ID/metadata并避免 begin后才因重复ID或 handoff分配失败，但它仅为同进程准备态，不是 durable/atomic handoff。
- component/real Registry 回归覆盖 existing/absent victim、连续 read、activation/finalize/rollback 故障、两类 pending/reentry fence、同方向 retry、victim 恢复、partial cleanup 和 late cancel；Production Executor 组合回归证明前一格式已 commit 时后一格式 cleanup/cancel failure 不删除可读 artifact。
- macOS arm64 protocol v3 backend跨 Registry activation持有同一个目录 handle，在该 handle下做 no-follow child验证、victim receipt、atomic replace、finalize/rollback；journal v2绑定 exact `transactionId`与 canonical partial leaf。fresh-process recover只接收ID和重新授权目录三字段；rollback exact recovery、journal-unlink幂等retry、valid open `decision_required`与absent journal `not_found`均已覆盖。Windows backend、Windows lossless volume/file identity、durable finalize decision、production main/IPC composition仍需在放行前冻结。
...
- 未完成：production overwrite 的 durable preclaim/open decision、finalize-crash durable decision、generation-bound staging/load、main/reauthorization IPC/UI injection与packaged validation，以及 CUDA/Metal、VAD、translate、translation handoff、FE与真实 native/packaged E2E。当前 source output、batch pin、多格式、overwrite receipt、composite owner与两平台raw native/identity composition证据仍不构成真实 FFmpeg + official server + PRE-006模型及target packaged runtime的完整product E2E；non-cooperative child writer safety明确不属于当前或未来本合同的保证范围。完整范围闭环前 `FS-TXN-001` 与 `BE-002` 保持进行中。
...
35. 不得把进程内 `prepareAdoption()` 称为 durable preclaim/atomic handoff；`decision_required` 与 `not_found` 都必须保留 pending record、durable direction 和已选目录 fence，只有 native 与 Registry authority 真正收敛后才允许释放 fence。
...
1. `BE-002` 的 Job Manager foundation、最多 100文件的custom/source CPU/no-VAD/index-only批次、SRT/LRC保序单/双格式、full/ordinary partial/cancellation partial、逐文件失败隔离、exact-identity batch runtime pin与可信多父目录source output已实现；`FS-TXN-001A`～`FS-TXN-001E` 已完成macOS arm64与Windows x64 developer components、strict loader、protocol v3 / journal v2、cooperative-writer child-leaf边界、test-only terminal/crash harness、durable `rollback_pending` recovery、composite owner/file repository、exact reauthorization recovery、lossless Windows volume/FileId与cross-platform Registry/authorization/Exporter/recovery identity composition。下一步继续durable prepared preclaim/open/finalize decision、generation-bound resource staging/load、builder、production main与reauthorization IPC/UI；两平台packaged validation后才评估gate放行。完整验收前不得把`FS-TXN-001`、`BE-002`或M2标记为已完成。
