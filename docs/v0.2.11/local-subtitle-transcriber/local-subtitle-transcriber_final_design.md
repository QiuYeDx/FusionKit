...
>
> 状态：39个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`、`NATIVE-002A1`、`NATIVE-002B`与`NATIVE-002C` component checkpoints已完成。001J已闭环真实Windows protocol-v4矩阵；002A/002A1已完成macOS arm64 canonical runtime及CPU/Metal production-model target smoke；002B已完成Windows x64 CPU/media/addon canonical assembly、独立CUDA 12.4 candidate与CPU/CUDA target smoke；002C已完成真实macOS arm64 `.app` 的exact packaged resource/addon消费、外层签名完整性冻结、CPU/Metal production-model smoke与fail-closed矩阵。剩余packaged范围严格收口为Windows x64真实app/installer与目标机矩阵；CUDA delivery与分发许可仍分别受`MODEL-002`和`QA-005`门禁。Job Manager / Production Executor双重`index-only` gate不变；顶层`NATIVE-002`、`FS-TXN-001`、`BE-002`与M2均不得提前完成
>
...
本节保留架构层面的阶段划分；可认领的工作包、依赖、状态、验证和实施记录以 `local-subtitle-transcriber_execution_plan.md` 为唯一执行台账。Execution Plan 已于 2026-07-16 建立，当前 39 个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`与`NATIVE-002`进行中，`FE-001`等20个未开始）。M1 的 schema、resource staging、IPC/capability、renderer session、official server contract、真实 Supervisor 生命周期、media normalization/PCM proof、canonical post-processing、标准字幕原子产物和 managed model 合同均已冻结。`BE-002` 的 index-only production 批次已形成；`FS-TXN-001A`～`FS-TXN-001J` 已完成两平台developer components与真实Windows protocol-v4矩阵；`NATIVE-002A`/`NATIVE-002A1`已闭环macOS canonical runtime与CPU/Metal target smoke，`NATIVE-002B`已闭环Windows CPU/media/addon canonical runtime、独立CUDA candidate与CPU/CUDA target smoke，`NATIVE-002C`已闭环macOS arm64真实`.app` packaged component。component checkpoints均不增加顶层工作包数量。总包仍等待Windows x64真实app/installer与目标机packaged矩阵。在该证据完成前production overwrite不可用，Job Manager/Executor双重`index-only` gate保持。Windows personal distribution 的unsigned profile已明确；CUDA delivery/notice closure仍归`MODEL-002`/`QA-005`，Developer ID、公证和Gatekeeper accepted只由未来`QA-004`验收macOS分发产物。
...
- macOS arm64与Windows x64 protocol v4 backend跨Registry activation持有同一个目录handle，在该handle下做no-follow child验证、victim receipt、atomic replace、finalize/rollback；journal v3绑定exact `transactionId`与canonical partial leaf并提供`.open/.finalize/.rollback` marker。fresh-process recover只接收ID、重新授权目录identity与durable decision；module/receipt acknowledgement只在main durable settled proof后删除terminal marker。native finalizer对`.open`保持方向中立，只续跑已armed terminal且不自行ack。001G已完成独立versioned staging、content-addressed final-byte publication、generation-bound verified load proof与正式builder consumption gate；001H已完成proof-only production bootstrap、versioned repository及同一runtime/owner向Exporter与SessionLifecycle的成对注入；001I已完成strict recovery IPC、owner-scoped picker admission、锁内TTL与app-scoped UI；001J已在真实Windows x64闭环compile/load/terminal/recovery/crash/retry/conflict/acknowledgement矩阵。`NATIVE-002B`已把production addon以exact final bytes纳入Windows canonical runtime并通过module probe；`NATIVE-002C`已冻结macOS `.app` 内addon的outer-sign前后完整性与fresh load。放行前仍须闭环Windows x64真实app/installer矩阵。
...
- 当前 production output conflict policy 仅放行 `index`；001H已在main组成verified backend/recovery owner，但Job Manager/Executor尚未放行，也不表示具备hostile filesystem replacement安全证据。
...
- 未完成：Windows x64真实app/installer与目标机packaged validation。`NATIVE-002C`已闭环macOS arm64真实`.app`的component消费、签名完整性、CPU/Metal和fault matrix，但不声明完整UI product E2E或QA-004发布门禁；`FS-TXN-001J`和`NATIVE-002B`的Windows developer/assembly/target smoke也不能替代真实installer。VAD、translate、translation handoff与`FE-001`同样未完成；Windows收尾记录的矩阵闭环前`FS-TXN-001`、`BE-002`与`NATIVE-002`保持进行中，双重`index-only` gate不得解除。
...
1. `BE-002` 的 Job Manager foundation、最多100文件的custom/source CPU/no-VAD/index-only批次、SRT/LRC保序单/双格式、逐文件失败隔离、exact-identity batch runtime pin与可信source output已实现；`FS-TXN-001A`～`FS-TXN-001J`已完成overwrite developer/main/recovery components及真实Windows protocol-v4矩阵，`NATIVE-002A`/`NATIVE-002A1`已闭环macOS canonical runtime与CPU/Metal target smoke，`NATIVE-002B`已闭环Windows CPU/media/addon canonical runtime、独立CUDA candidate与CPU/CUDA target smoke，`NATIVE-002C`已闭环macOS arm64真实`.app` packaged component。下一步严格按002C实施记录中的Windows handoff闭环Windows x64真实app/installer与目标机矩阵。完整验收前不得解除Job Manager/Executor双重`index-only` gate，也不得把`FS-TXN-001`、`BE-002`、`NATIVE-002`或M2标记为已完成；`FE-001`仍按原计划单独认领。
