...
- 未完成：production overwrite 的 durable preclaim/open decision、finalize-crash durable decision、Windows x64 backend与lossless FileId、generation-bound staging/load、main/reauthorization IPC/UI injection与packaged validation，以及 CUDA/Metal、VAD、translate、translation handoff、FE与真实 native/packaged E2E。当前 source output、batch pin、多格式、overwrite receipt、composite owner与macOS raw component证据仍不构成真实 FFmpeg + official server + PRE-006模型及target packaged runtime的完整product E2E；non-cooperative child writer safety明确不属于当前或未来本合同的保证范围。完整范围闭环前 `FS-TXN-001` 与 `BE-002` 保持进行中。
...
1. `BE-002` 的 Job Manager foundation、最多 100文件的custom/source CPU/no-VAD/index-only批次、SRT/LRC保序单/双格式、full/ordinary partial/cancellation partial、逐文件失败隔离、exact-identity batch runtime pin与可信多父目录source output已实现；`FS-TXN-001A`～`FS-TXN-001C` 已完成macOS arm64 developer component、strict loader、protocol v3 / journal v2、cooperative-writer child-leaf边界、test-only terminal/crash harness、durable `rollback_pending` recovery、composite owner/file repository与重新授权后的exact lazy recovery组件合同。下一步认领Windows-only `FS-TXN-001D`：完成win32/x64 protocol v3/journal v2 parity、directory HANDLE + RootDirectory-relative child operations、reparse/no-follow边界、lossless volume/FileId codec及fresh-process fault/crash/真实Node load。001D不包含durable prepared preclaim、finalize-crash decision、resource staging/builder、production main injection/reauthorization IPC/UI、gate放行或packaged validation。之后再分别闭环这些跨平台composition项，证据齐全后才接通production overwrite；完整验收前不得把`FS-TXN-001`、`BE-002`或M2标记为已完成。
