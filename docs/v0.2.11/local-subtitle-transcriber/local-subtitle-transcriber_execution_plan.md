...
>
> 当前状态：39个顶层包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`、`NATIVE-002A1`与`NATIVE-002B` component checkpoints已完成。001J已闭环真实Windows protocol-v4矩阵；002A/002A1已完成macOS arm64 canonical assembly及CPU/Metal production-model target smoke；002B已完成Windows x64 CPU/media/addon canonical assembly、独立20-file CUDA 12.4 candidate、CPU/CUDA production-model private health、exact-PID VRAM、media decode/no-PATH与cleanup。Job Manager / Production Executor双重`index-only` gate不变；`NATIVE-002`剩余范围收口为macOS arm64 / Windows x64 packaged validation，CUDA分发仍受`MODEL-002`与`QA-005`门禁
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23～2026-07-29完成`FS-TXN-001A`～`FS-TXN-001J` component checkpoints，2026-07-28完成`NATIVE-002A` macOS arm64 official runtime assembly及`NATIVE-002A1` unrestricted Metal production-model复验，2026-07-29完成`NATIVE-002B` Windows CPU/CUDA/media canonical assembly与target smoke，均不增加顶层工作包数量；三个顶层包仍等待两平台packaged剩余矩阵
>
...
| NATIVE-002 | 进行中 | 2026-07-29 | NATIVE-001/CORE-002 | 三类 official server artifact、runtime manifest 与 builder 接线 | staging/source-build/smoke scripts、resource manifests、`electron-builder.json`、可选 workflow | 001G完成canonical builder/addon消费合同；002A/002A1完成macOS arm64 canonical server/media/addon及CPU/Metal production-model target smoke。002B focused Node 56/56 + hash-close 1/1；Windows base manifest `f8058656...d8798a`、addon generation `04f2f4b5...d8a7`；独立CUDA 20-file candidate manifest `67215097...dbabe`，CPU/CUDA private health、bundled media/no-PATH与cleanup通过，CUDA exact-PID peak VRAM 1,815,437,312 bytes | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001G_verified-native-addon-staging-load-and-builder-contract.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-28_NATIVE-002A_macos-arm64-official-runtime-assembly.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-28_NATIVE-002A1_macos-arm64-metal-production-model-verification.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-29_NATIVE-002B_windows-x64-official-runtime-assembly.md` | 002A/002A1/002B components完成但顶层仍进行中；剩余两平台packaged矩阵。CUDA下载/install/update/rollback归`MODEL-002`，NVIDIA分发notice/EULA归`QA-005` |
...
截至2026-07-29，本工作包仍为进行中。001G已完成唯一canonical `extraResources` mapping、固定beforePack与official runtime + overwrite addon静态消费合同。`NATIVE-002A`/`NATIVE-002A1`已完成macOS arm64 exact-source canonical assembly以及CPU/Metal production-model target smoke；runtime manifest为`19271b1f...22530`。`NATIVE-002B`已完成Windows x64 CPU/media/addon canonical assembly：15个base artifact manifest为`f8058656...d8798a`，addon generation为`04f2f4b5...d8a7`；独立CUDA 12.4 candidate固定20个x64 PE / 1,199,083,008 bytes，manifest为`67215097...dbabe`。CPU与CUDA均用production模型`d75795ec...ad1`通过private health、bundled PCM16 media/no-PATH与close-confirmed cleanup；CUDA exact-PID peak VRAM为1,815,437,312 bytes且`backendVerified=true`。静态pack合同自身保持`targetSmokeStatus=pending`，不自证外部target结果；通过事实由独立ignored smoke report与本实施记录承载。Windows target artifact矩阵已关闭，剩余范围为macOS arm64 / Windows x64 packaged smoke；CUDA delivery仍由`MODEL-002`实现，`QA-005`完成NVIDIA分发复核前candidate不得分享。
...
未完成范围严格收口为：macOS arm64/Windows x64两平台packaged validation。`FS-TXN-001J`已关闭真实Windows protocol v4矩阵；`NATIVE-002A`/`NATIVE-002A1`已关闭macOS assembly与CPU/Metal target smoke；`NATIVE-002B`已关闭Windows CPU/CUDA/media assembly、production-model target launch/no-PATH与backend verification。`FS-TXN-001`、`BE-002`与`NATIVE-002`仍进行中，`FE-001`仍未开始，Job Manager/Production Executor双重`index-only` gate继续保留。
...
剩余overwrite/native范围：macOS arm64/Windows x64两平台packaged validation。真实Windows protocol v4矩阵已由`FS-TXN-001J`闭环；`NATIVE-002A`/`NATIVE-002A1`与`NATIVE-002B`分别闭环两平台canonical runtime/target smoke，但都不是完整packaged product E2E。VAD、translate与非export-only路径仍未完成。上述范围完成前`FS-TXN-001`、`BE-002`与`NATIVE-002`均不得标记为已完成，Job Manager/Executor双重`index-only` gate不得解除。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中包及20个未开始包；`FE-001`仍未开始。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`、`NATIVE-002A1`与`NATIVE-002B` component checkpoints已完成；真实Windows protocol v4及两平台canonical runtime/target smoke均已闭环。下一步完成macOS arm64/Windows x64 packaged validation；CUDA delivery与分发许可分别保留给`MODEL-002`和`QA-005`。全部证据完整前Job Manager/Executor双重`index-only` gate不变，`FS-TXN-001`、`BE-002`与`NATIVE-002`不得标记完成。
