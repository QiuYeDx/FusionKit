...
> 创建日期：2026-07-16；最近更新：2026-07-30
>
...
>
> 当前状态：39个顶层包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`、`NATIVE-002A1`、`NATIVE-002B`、`NATIVE-002C`与`NATIVE-002D` component checkpoints已完成。001J已闭环真实Windows protocol-v4矩阵；002A/002A1已完成macOS arm64 canonical assembly及CPU/Metal production-model target smoke；002B已完成Windows x64 CPU/media/addon canonical assembly、独立20-file CUDA 12.4 candidate与CPU/CUDA target smoke；002C已完成真实macOS arm64 `.app` 的exact runtime/addon消费、外层签名完整性冻结、CPU/Metal packaged smoke与fail-closed矩阵；002D已完成真实Windows x64 app/NSIS的exact runtime/addon消费、packaged fail-closed矩阵、CPU/CUDA production-model smoke与NTFS overwrite/recovery全矩阵回归。Job Manager / Production Executor双重`index-only` gate不变；`NATIVE-002`剩余范围收口为NSIS安装生命周期与非ASCII路径的手动QA，CUDA分发仍受`MODEL-002`与`QA-005`门禁
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23～2026-07-29完成`FS-TXN-001A`～`FS-TXN-001J` component checkpoints，2026-07-28完成`NATIVE-002A` macOS arm64 official runtime assembly及`NATIVE-002A1` unrestricted Metal production-model复验，2026-07-29完成`NATIVE-002B` Windows CPU/CUDA/media canonical assembly与target smoke，2026-07-30完成`NATIVE-002C` macOS arm64 packaged component，2026-08-01完成`NATIVE-002D` Windows x64 packaged consumption（app/NSIS/packaged validator/CPU+CUDA smoke/NTFS overwrite+recovery回归），均不增加顶层工作包数量；三个顶层包仍等待NSIS安装生命周期手动QA
>
...
| 项目 | 当前状态 | 对计划的影响 |
| --- | --- | --- |
| Electron 打包 | 正式 `electron-builder.json` 已接入唯一canonical `extraResources` mapping、固定 `beforePack`、macOS arm64 / Windows x64 target、`${arch}` artifact name与macOS runtime `signIgnore`；001G冻结official runtime + overwrite addon的`launch:false`双verifier消费合同，002C已用正式配置生成真实macOS `.app`并完成签名后双完整性复验 | macOS arm64 exact packaged component、CPU/Metal health、media no-PATH与fault matrix已闭环；剩余Windows x64真实app/installer与目标机矩阵。builder只能消费版本化staging，不能提交machine path或在electron-builder期间联网 |
...
| FS-TXN-001 | 进行中 | 2026-08-01 | SUB-002/CORE-002 | 目录句柄相对 overwrite transaction | branded Coordinator/Exporter、两平台protocol v4/journal v3、schema-v2 durable preclaim/decision、terminal marker + acknowledgement、composite owner/exact reauthorization、cross-platform identity、verified addon staging/load/builder、proof-only production main composition、strict recovery IPC/picker admission与app-scoped UI、真实Windows compile/load/crash/retry矩阵 | 001J Windows protocol-v4矩阵通过；002C已证明macOS `.app` 内content-addressed production addon在outer sign前后bytes不变且fresh-process module probe通过。002D已在真实Windows x64 packaged app中通过NTFS overwrite integration 15 cases与recovery全矩阵回归（修复delete-pending竞态后稳定通过）。该component仍不接通production overwrite，不替代product E2E | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/`中的001A～001J、`2026-07-30_NATIVE-002C_macos-arm64-packaged-consumption.md`及`2026-08-01_NATIVE-002D_windows-x64-packaged-consumption.md` | 顶层仍进行中；双重`index-only`保持 |
...
截至2026-07-30，本工作包仍为进行中。001G已完成唯一canonical `extraResources` mapping、固定beforePack与official runtime + overwrite addon静态消费合同。`NATIVE-002A`/`NATIVE-002A1`已完成macOS arm64 exact-source canonical assembly以及CPU/Metal production-model target smoke；`NATIVE-002B`已完成Windows x64 CPU/media/addon canonical assembly与独立CUDA 12.4 candidate。`NATIVE-002C`进一步使用正式builder生成真实arm64 `.app`，只从`Contents/Resources/local-subtitle`消费official runtime与protocol-v4 addon；outer ad-hoc sign前后双完整性冻结、fresh module load、system deep/strict codesign、production-model CPU/Metal、bundled PCM16 media/no-PATH及9类fail-closed fault matrix通过。该component不声明Developer ID、公证、Gatekeeper accepted、DMG或完整product E2E。剩余范围严格收口为Windows x64真实app/installer与目标机packaged矩阵；CUDA delivery仍由`MODEL-002`实现，`QA-005`完成NVIDIA分发复核前candidate不得分享。
...
未完成范围严格收口为：Windows x64真实app/installer与目标机packaged validation。`NATIVE-002C`已关闭macOS arm64真实`.app` component consumption；`FS-TXN-001J`已关闭真实Windows protocol v4矩阵；`NATIVE-002B`已关闭Windows CPU/CUDA/media assembly与target smoke，但后二者都不能替代真实installer。`FS-TXN-001`、`BE-002`与`NATIVE-002`仍进行中，`FE-001`仍未开始，Job Manager/Production Executor双重`index-only` gate继续保留。
...
当前进展（2026-07-30）：Job Manager/revision/capability foundation、最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次和逐文件失败隔离已完成；production executor已串联MEDIA normalization、Supervisor pinned task lease、逐窗identity-bound inference、SUB-001 raw gate/retry/canonical processing、cleanup boundary与SUB-002 SRT/LRC index exporter。`FS-TXN-001A`～`FS-TXN-001J`已提供两平台protocol v4/journal v3、durable recovery、cross-platform identity、generation-bound addon staging/load、production main与recovery IPC/UI，并闭环真实Windows protocol矩阵；`NATIVE-002A`～`NATIVE-002C`已闭环macOS canonical/target smoke和真实`.app` packaged component，`NATIVE-002B`已闭环Windows canonical CPU/CUDA target smoke。Job Manager/Executor仍不接通overwrite；当前证据不包含Windows installer、packaged native product E2E或完整BE-002范围验收。
...
剩余overwrite/native范围：Windows x64真实app/installer与目标机packaged validation。`NATIVE-002C`已证明macOS `.app` 内exact addon/runtime消费，但不解除production gate；`FS-TXN-001J`与`NATIVE-002B`的Windows developer/assembly/target smoke也不是installer E2E。VAD、translate与非export-only路径仍未完成。Windows handoff矩阵完成前`FS-TXN-001`、`BE-002`与`NATIVE-002`均不得标记为已完成，Job Manager/Executor双重`index-only` gate不得解除。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中包及20个未开始包；`FE-001`仍未开始。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`、`NATIVE-002A1`、`NATIVE-002B`与`NATIVE-002C` component checkpoints已完成；真实Windows protocol v4、两平台canonical runtime/target smoke及macOS真实`.app` packaged component均已闭环。下一步只做Windows x64真实app/installer与目标机矩阵；CUDA delivery与分发许可分别保留给`MODEL-002`和`QA-005`。全部证据完整前Job Manager/Executor双重`index-only` gate不变，`FS-TXN-001`、`BE-002`与`NATIVE-002`不得标记完成。
