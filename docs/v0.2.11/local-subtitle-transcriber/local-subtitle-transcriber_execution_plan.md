...
>
> 当前状态：`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002` 与 `MODEL-001` 已完成，M1 的 domain/runtime schema、versioned resource staging、preload/IPC/capability、renderer session、official server contract、真实 Supervisor 生命周期、media normalization/PCM proof、canonical post-processing、标准字幕原子产物与 managed model 合同已冻结；Windows 使用 `unsigned_personal_distribution`。`BE-002` 已支持最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次，可按请求顺序生成SRT-only、LRC-only或无重复双格式，并稳定结算full、普通partial与commit后取消partial；原子批次发布、app-global FIFO、逐文件失败隔离、owner分区续权、exact-identity batch runtime pin与可信多父目录source output已实现。`FS-TXN-001A`～`FS-TXN-001E` component checkpoints 已完成：macOS arm64冻结protocol v3/journal v2、exact transaction ID、composite recovery owner、path-free repository、Exporter component handoff与重新授权后的exact lazy recovery；Windows x64完成directory HANDLE + RootDirectory-relative child operations、reparse/no-follow、lossless volume/FileId、existing/absent terminal、same-receipt retry与fresh-process crash recovery；cross-platform composition已把POSIX/Windows exact filesystem identity贯通输出目录authorization、Artifact Registry、Exporter、recovery selection与Executor directory proof。prepared handoff仍只在同进程内single-claim，production main尚未实例化该runtime；durable preclaim/finalize decision、verified staging/main injection与packaged validation未完成。Job Manager / Production Executor继续保持双重`index-only` gate，`FS-TXN-001`与`BE-002`均保持进行中。39个顶层包中16个已完成、23个剩余（2个进行中、21个未开始）
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23以`FS-TXN-001A`～`FS-TXN-001D`作为其平台/恢复component checkpoints，不增加总包数量；2026-07-24四个component checkpoints全部完成；2026-07-26新增并完成`FS-TXN-001E` cross-platform filesystem identity composition，顶层`FS-TXN-001`仍等待durable/production composition
>
...
| 项目 | 当前状态 | 对计划的影响 |
| --- | --- | --- |
| Electron 打包 | 正式 `electron-builder.json` 仍只包含 `dist-electron`、`dist`，但 Windows/macOS artifact name 已含 `${arch}`；PRE-005 ignored spike 已在 macOS/Windows 跑通 `extraResources` 与 `beforePack`，PRE-006 已冻结离线 acquisition/staging 合同 | 正式 `extraResources` 接线由 CORE-002/NATIVE-002 完成；只能消费版本化 staging，不能提交含 machine path 的 spike config，也不能在 electron-builder 期间联网 |
...
- transaction路径在一个同步段执行`begin → Artifact Registry activate → finalize`。activation失败调用rollback；terminal或Registry authority未收敛时由Exporter移交给branded composite owner，同一receipt只能被claim一次。owner release、shutdown与shutdown后late adoption沿原方向重试，未收敛则保留record并返回稳定failure，禁止丢弃receipt或反向terminal。
- component/real Registry 测试覆盖 existing/absent victim、连续 read、activation/finalize/rollback 故障、两种 pending 状态与反向 terminal 拒绝、同方向重试、partial cleanup、late cancel，以及 Production Executor 已提交 SRT 后 LRC commit/cleanup 失败的保留语义。
...
- native begin在`napi_wrap`后commit失败时会同步detach/delete transaction；64次permission-denied begin的`/dev/fd` delta为0。strict loader只接受absolute `.node`、protocol v3 production exact exports `protocolVersion/platform/architecture/begin/recover`与当前target，并把begin-only transaction authority和single-claim recovery authority分离；多出`testFaultInjection: true`的test-only artifact会被拒绝。
- shell-free developer build 固定当前 Node exact headers、N-API v8、C++17、arm64 与 macOS 11.0，真实构建得到 thin/loadable Mach-O 且保留 `LC_UUID`。`begin` 在首次 namespace mutation 前写入 addon-write-once、checksummed `<partialLeaf>.fusionkit-overwrite.open` journal；rollback 在首次 namespace mutation 前将其原子改名为 `.rollback`，fresh process 只从 exact journal 重放该方向。production begin path在 rename 后不再执行可抛步骤；test-only `begin_after_namespace` fault checkpoint是用于 crash/error evidence的唯一有意例外。journal unlink 后的 sync/proof fault 可用 named-leaf absence + pinned-fd `nlink == 0` 幂等续跑。
...
- persistent discovery 采用 output-directory exact journal + 用户重新授权后的 lazy recovery：main 只持久化 opaque transaction id、owner/task/generation/format 与脱敏状态，不保存 user-output raw path、capability 或 token，也不在启动时扫描任意用户目录。只有 output capability 再次解析并证明相同 directory object 后，recovery manager 才可用 exact id 打开该目录内 journal；不得按 prefix 扫描并认领未知 journal。
- 跨进程durable decision仍只有`rollback_pending`：第一次rollback namespace mutation前先持久化intent，fresh-process recover只允许恢复victim/absence并清理exact new leaf。`finalize_pending`只有当前receipt的驻内存方向锁，不是durable commit decision；合法open journal返回`decision_required`，finalize crash在journal unlink后可能只返回`not_found`，不能靠布局猜测terminal选择。
- 001C owner/repository已实现component-level composite ownership和exact reauthorization recovery：`decision_required`与`not_found`都保留pending record、durable direction和已选目录fence，并让shutdown返回`recovery_pending`；只有native与Registry authority真正收敛后才删除record并释放fence。file repository使用0600 temp、fsync、atomic rename与exact payload read-back，但不构成power-loss声明。production main/IPC/UI composition前继续禁用overwrite。
...
未完成：prepared handoff的durable preclaim/open decision、finalize crash与Registry commit/revoke的durable cross-process decision、generation-bound verified load、resource staging/builder、production main/reauthorization IPC/UI接线和两平台packaged validation。macOS与Windows native component证据都不覆盖non-cooperative writer、断电或production composition，因此`FS-TXN-001`保持进行中。001A～001E component checkpoints均已完成；下一步继续durable/production composition。
...
当前进展（2026-07-26）：Job Manager/revision/capability foundation、最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次和逐文件失败隔离已完成；production executor已串联MEDIA normalization、Supervisor pinned task lease、逐窗identity-bound inference、SUB-001 raw gate/retry/canonical processing、cleanup boundary与SUB-002 SRT/LRC index exporter，并按请求顺序支持SRT-only、LRC-only与无重复双格式。至少一个格式commit即稳定结算completed：普通第二格式失败为无warning partial，commit后取消由artifact marker形成cancellation partial；late cancel与lease abort不覆盖已提交结果。batch runtime pin按queue admission execution wave持有并绑定完整exact load identity，阻止sibling间model smoke、不兼容切换和idle retirement。source output从task input authorization-time parent proof私下派生并按写入边界重验，不使用全局目录lease。`FS-TXN-001A`～`FS-TXN-001E`已提供branded receipt、macOS arm64 protocol v3/journal v2、composite owner/file repository、exact reauthorization recovery、Windows x64 HANDLE-relative/reparse/no-follow/lossless FileId/fresh-process recovery，以及authorization/Artifact Registry/Exporter/recovery selection的cross-platform exact identity composition；但`electron/main/index.ts`仍实例化未配置transaction/owner的Exporter，production main、reauthorization IPC/UI与verified staging未接线，Job Manager/Executor仍不接通overwrite。main已实例化并合并Session/Model/Job IPC。该进展只证明production code、unit/contract harness与raw native component integration，不包含renderer UI、packaged native E2E或完整BE-002范围验收。
...
剩余范围：production overwrite仍缺durable prepared preclaim/open decision、finalize-crash durable decision、generation-bound staging/load、production main/reauthorization IPC/UI injection与packaged validation；CUDA/Metal、VAD、translate与非export-only路径也未完成。translation handoff、FE和packaged/native E2E分别由LINK、FE、NATIVE/QA工作包闭环。当前source output、batch pin、多格式、overwrite receipt/composite owner与两平台raw component/identity composition证据仍不是一次真实FFmpeg + official server + PRE-006模型及target packaged runtime的产品E2E；non-cooperative child writer safety明确不属于当前或未来本合同的保证范围。上述范围完成前`FS-TXN-001`与`BE-002`均不得标记为已完成。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`两个进行中包及21个未开始包。`FS-TXN-001A`～`FS-TXN-001E`已完成macOS arm64 protocol v3/journal v2、cooperative-writer边界、composite owner/file repository、exact reauthorization recovery、Windows x64 HANDLE-relative/lossless FileId/fresh-process recovery，以及authorization/Artifact Registry/Exporter/recovery selection的cross-platform exact identity composition。下一步继续durable prepared preclaim/open/finalize decision、generation-bound staging/load、builder、production main与reauthorization IPC/UI；之后由NATIVE/QA补两平台packaged validation。只有全部证据完整后才能解除Job Manager/Executor双重index gate，完整验收前`FS-TXN-001`与`BE-002`保持进行中。
