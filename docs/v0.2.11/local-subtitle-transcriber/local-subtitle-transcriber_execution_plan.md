...
>
> 当前状态：39个顶层包中20个已完成、19个未开始；当前没有进行中的顶层包。`FE-001`已完成独立subtitle category/route、四语言、runtime/resource probe、单文件opaque capability授权、CPU/no-VAD/SRT/index/export-only enqueue、session进度、取消及committed artifact reveal纵向UI。页面只选择已ready managed model，未提前开放GPU、VAD、模型下载、批量、LRC或翻译交接。TypeScript、5 files / 28 tests、四语言i18n/usage、renderer/main/preload三段Vite test build与diff gate通过；本轮未运行真实模型Electron产品E2E，因此不把M2 packaged/目标机验收记为完成。下一步`MODEL-002`、`BE-003`或`FE-002`；NSIS生命周期仍归`QA-003`
>
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`～`NATIVE-002`、`BE-001`～`BE-002`、`MEDIA-001`、`SUB-001`～`SUB-002`、`FS-TXN-001`、`MODEL-001`与`FE-001`已完成。39个顶层工作包中20个已完成、19个未开始。下一步可认领`MODEL-002`推进模型/CUDA pack生命周期，或认领`BE-003`/`FE-002`接通环境诊断和资源管理UI。NSIS真实安装/更新/卸载、非ASCII路径与数据保留留给`QA-003`，CUDA分发许可留给`QA-005`。M2的renderer代码路径已具备，但仍等待真实模型Electron产品E2E后再声明packaged/目标机验收；M3仍等待`MODEL-002`、`BE-003`与`FE-002`～`FE-004`。
