...
> 创建日期：2026-07-16；最近更新：2026-07-27
>
...
>
> 当前状态：39个顶层包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001I` component checkpoints已完成：001A～001H冻结两平台protocol v4/journal v3、schema-v2 durable preclaim/decision、terminal marker + acknowledgement、composite owner/exact reauthorization、cross-platform identity、verified addon staging/load/builder与proof-only production main composition；001I完成strict path-free分页IPC、stable displayCode、owner-scoped picker admission/锁内TTL、shutdown tail fencing及app-scoped四语言recovery UI。001I focused 9 files / 175 passed；local-subtitle 42 passed + 2 skipped files / 951 passed + 2 skipped tests；全量145 passed + 2 skipped files / 1921 passed + 2 skipped tests；overwrite-native 29 passed + 1 skipped；manifest 0/0、validator 17/17、四语言各1522 keys、TypeScript、三段Vite与diff通过。Job Manager / Production Executor双重`index-only` gate不变。剩余overwrite范围仅为真实Windows protocol v4矩阵、完整official server/media artifact assembly + target launch/no-PATH、macOS arm64 / Windows x64 packaged validation
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23～2026-07-27完成`FS-TXN-001A`～`FS-TXN-001I` component checkpoints，不增加顶层工作包数量；顶层`FS-TXN-001`仍等待真实Windows protocol v4、完整official server/media runtime assembly/launch与两平台packaged validation
>
...
未完成范围严格收口为：真实Windows protocol v4 compile/load/terminal/recovery/crash/acknowledgement矩阵；`NATIVE-002`完整official server/media artifact assembly与target launch/no-PATH；macOS arm64/Windows x64两平台packaged validation。`FS-TXN-001`保持进行中，001A～001I component checkpoints均已完成；`BE-002`与`NATIVE-002`仍进行中，`FE-001`仍未开始，Job Manager/Production Executor双重`index-only` gate继续保留。
...
当前进展（2026-07-27）：Job Manager/revision/capability foundation、最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次和逐文件失败隔离已完成；production executor已串联MEDIA normalization、Supervisor pinned task lease、逐窗identity-bound inference、SUB-001 raw gate/retry/canonical processing、cleanup boundary与SUB-002 SRT/LRC index exporter，并按请求顺序支持SRT-only、LRC-only与无重复双格式。至少一个格式commit即稳定结算completed：普通第二格式失败为无warning partial，commit后取消由artifact marker形成cancellation partial；late cancel与lease abort不覆盖已提交结果。batch runtime pin按queue admission execution wave持有并绑定完整exact load identity，阻止sibling间model smoke、不兼容切换和idle retirement。source output从task input authorization-time parent proof私下派生并按写入边界重验，不使用全局目录lease。`FS-TXN-001A`～`FS-TXN-001I`已提供branded receipt、两平台protocol v4/journal v3、schema-v2 durable preclaim/finalize decision、terminal marker + acknowledgement、composite owner/file repository、exact reauthorization recovery、cross-platform exact identity composition、generation-bound addon staging/load与builder gate、canonical proof-only production main composition，以及strict recovery IPC/picker admission/app-scoped UI。`electron/main/index.ts`将同一runtime/recovery owner经admission coordinator注入Exporter、SessionLifecycle和recovery IPC；Job Manager/Executor仍不接通overwrite。该进展包含renderer recovery UI，但仍只是production code、unit/contract harness与native component/staging/main composition，不包含真实Windows protocol v4、packaged native E2E或完整BE-002范围验收。
...
剩余overwrite范围：真实Windows protocol v4矩阵；NATIVE-002完整official server/media真实artifact assembly与target launch/no-PATH；macOS arm64/Windows x64两平台packaged validation。CUDA/Metal、VAD、translate与非export-only路径也未完成；translation handoff、`FE-001`和packaged/native E2E分别由LINK、FE、NATIVE/QA工作包闭环。当前source output、batch pin、多格式、overwrite durable decision/composite owner、两平台raw component/identity composition、addon staging/builder、production main composition与reauthorization IPC/UI证据仍不是一次真实FFmpeg + official server + PRE-006模型及target packaged runtime的产品E2E；non-cooperative child writer safety明确不属于当前或未来本合同的保证范围。上述范围完成前`FS-TXN-001`、`BE-002`与`NATIVE-002`均不得标记为已完成，Job Manager/Executor双重`index-only` gate不得解除。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中包及20个未开始包；`FE-001`仍未开始。`FS-TXN-001A`～`FS-TXN-001I` component checkpoints已完成。下一步仅闭环真实Windows protocol v4矩阵、完整official server/media artifact assembly + target launch/no-PATH及macOS arm64/Windows x64 packaged validation。全部证据完整前Job Manager/Executor双重`index-only` gate不变，`FS-TXN-001`、`BE-002`与`NATIVE-002`不得标记完成。
