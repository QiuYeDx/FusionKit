...
| LINK-007 | 进行中 | 2026-08-04 | LINK-006/LINK-002 | 翻译配置快照、任务级 capability、导入协调器与回执闭环 | hydration-aware snapshot coordinator、公开/私有配置隔离、TTL/custom lease/release lifecycle、tests | coordinator 1 file / 7 tests；联合focused 5 files / 38 tests；TypeScript、三段Vite test build、diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_LINK-001_LINK-007A_translator-config-snapshot.md` | `importArtifact`、candidate factory、receipt/exact start未实现；custom等待LINK-003 capability；私有快照/capability不持久化 |
...
2026-08-04 `LINK-007A`已完成配置快照前半段：安全配置Store与model Store hydration readiness、`ready | needs_configuration`、公开/私有字段隔离、snapshot TTL和custom lease release lifecycle已经实现。`LINK-002`随后完成稳定任务身份与精确启动，但仍没有可安全承接生成字幕的无路径source/target合同，因此尚不实现`importArtifact`，也不把LINK-006 token换成旧`originFileURL`/`targetFileURL`；后续先完成`LINK-003`，再由`LINK-007B`接candidate、完整receipt和exact start。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`～`NATIVE-002`、`BE-001`～`BE-003`、`MEDIA-001`、`SUB-001`～`SUB-002`、`FS-TXN-001`、`MODEL-001`、`FE-001`、`FE-003`、`LINK-002`与`LINK-006`已完成。39个顶层工作包中24个已完成、10个未开始，`MODEL-002`、`FE-002`、`FE-004`、`LINK-001`与`LINK-007`进行中。`LINK-001`/`LINK-007A`已关闭安全配置白名单迁移、双import order、fail-closed readiness、公开/私有快照隔离及release retry；`LINK-002`已关闭稳定task identity、显式execution binding、幂等批量回执与精确启动。legacy path仍保留，下一步优先完成`LINK-003`目录capability，再由`LINK-007B`接main-private candidate、one-shot token消费、target handle与完整import receipt；随后按`LINK-004`/`LINK-005`完成新任务生产者和恢复消费者cutover，`LINK-008`最后接通三种后处理模式和FE-004手动入口。真实大文件下载、NSIS生命周期、目标GPU、CUDA分发许可、M2产品E2E、Electron窄窗口/键盘/cancel race与其余QA仍按原边界闭环；M3继续等待`MODEL-002`、`FE-002`与`FE-004`。
