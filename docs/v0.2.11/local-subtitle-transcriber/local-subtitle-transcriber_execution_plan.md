...
>
> 当前状态：实施计划已建立，所有实现工作包均为 `未开始`；下一步只认领 `PRE-001`
>
...
3. 阅读 `.agents/skills/fusionkit-pitfall-guard/references/index.md`，至少检查 `FK-PIT-0021`；涉及持久化、preload、capability、i18n、Electron 视觉验证时，再读取对应条目。
...
8. 若需要变更 JS 依赖或 lockfile，必须使用兼容 lockfile v6 的 pnpm 8.x。当前环境为 pnpm 11.7.0，不得用其执行会改写 `pnpm-lock.yaml` 的安装命令。
...
| 当前 Windows 工具链 | 当前 shell 找不到 CMake、MSVC `cl` 或 Clang | 本机不能直接声称完成 native PoC；PRE-001 必须先补工具链或换合格环境 |
| 当前 FFmpeg | 系统存在 2024-04-18 GPL/full build | 可用于行为 PoC，但不得未经许可证审计直接打进 FusionKit |
| 包管理器 | pnpm 11.7.0，lockfile 为 v6 | 依赖变更必须切换 pnpm 8.x；普通验证优先使用 `node_modules/.bin/*` |
...
| 字幕翻译配置/路径 | 配置分散于 Store、页面 state、localStorage 和 `useModelStore`；现有任务直接携带 `originFileURL`/`targetFileURL` | LINK-001～003 必须收敛配置所有权，并为生成任务引入无路径 source 标记与不透明 target 引用；自动交接不能跨页偷读状态，也不能把 token 换回 renderer raw path 迁就旧字段 |
...
6. 正式 runner 必须使用固定版本 JSONL 协议、request correlation、最大帧长、超时和结构化错误；不得解析 stock CLI 人类日志作为唯一合同。
...
flowchart TD
  ...
  CORE3 --> LINK3
  LINK3 --> LINK4["LINK-004 无路径 checkpoint"]
  LINK4 --> LINK5["LINK-005 Token/快照协调器"]
  SUB2 --> LINK5
  CORE3 --> LINK5
  LINK5 --> LINK6["LINK-006 三模式 UI/流水线"]
  FE4 --> LINK6
...
| M4 烤肉流水线闭环 | `LINK-001`～`LINK-006` 完成，三种模式和精确启动合同通过 | 禁止默认或范围不明地启动翻译队列 |
...
用户选择 1 个音频文件
  ...
  → 已导入且验证过的小模型/large-v3 由 persistent CPU runner 加载
...
该切片明确不包含模型联网下载、CUDA/Metal、LRC、逐词字幕、批量并发、翻译交接或完整视觉精修。先用它验证权限、协议、进程、时间轴和文件提交，再扩展功能。

### 6.3 首版发布闭环

首版发布必须额外具备：

- Windows x64 NVIDIA CUDA、Windows CPU fallback、macOS arm64 Metal、macOS x64 CPU fallback 的真实 packaged 验收。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PRE-001 | 未开始 | — | — | 基准语料、工具链与 clean-room 证据基线 | `docs/v0.2.11/local-subtitle-transcriber/poc/*`、`scripts/local-subtitle/benchmark/*` | fixture/manifest 校验、工具链报告、`git diff --check` | — | 当前 Windows shell 缺少 CMake/编译器；测试媒体许可需确认 |
...
| PRE-004 | 未开始 | — | PRE-002 | macOS arm64 Metal 与 x64 CPU PoC | benchmark records、mac build scripts | Metal/CPU backend、RTF、内存、签名后执行 | — | 需要 Apple Silicon 与 Intel/等价 x64 目标机 |
| PRE-005 | 未开始 | — | PRE-002 | FFmpeg、sidecar staging、签名/公证与许可证 PoC | `electron-builder.json` spike、license inventory | packaged smoke、非 ASCII/长路径、二进制来源审计 | — | 当前系统 GPL/full FFmpeg 不能直接作为发行结论 |
...
| CORE-001 | 未开始 | — | PRE-006 | domain、状态机、事件、错误与 runtime schema | `src/type/localSubtitle.ts`、`src/type/localSubtitleIpc.ts`、tests | type/schema/state transition/error tests、tsc | — | 需冻结 protocol/model manifest version |
...
| CORE-003 | 未开始 | — | CORE-001 | preload、IPC、文件/目录 capability 安全边界 | preload policy、`electron/main/local-subtitle/ipc.ts`、authorization tests | public/internal channel、owner、TTL、revoke、path containment | — | 不复用 audio registry |
| CORE-004 | 未开始 | — | CORE-001/003 | Renderer 偏好 Store、事件 reducer 与 cleanup retry | `src/store/tools/subtitle/useLocalSubtitleTranscriberStore.ts`、services/tests | persist partialize、非法事件、stale generation、revoke retry | — | token/任务不得持久化 |
| NATIVE-001 | 未开始 | — | PRE-006/CORE-001 | 正式 whisper.cpp runner 与版本化 JSONL 协议 | `native/local-subtitle-runner/*` | ctest、partial frame、未知消息、模型复用、abort、stdout purity | — | 上游引入方式必须可复现并保留 MIT notice |
| NATIVE-002 | 未开始 | — | NATIVE-001/CORE-002 | Windows/macOS 原生构建矩阵、artifact manifest 与 builder 接线 | native build scripts、resource manifests、`electron-builder.json`、可选 workflow | CPU/CUDA/Metal build + packaged smoke + SHA manifest | — | 签名凭据不得进入仓库；staging 缺失必须在打包前明确失败 |
...
| SUB-002 | 未开始 | — | SUB-001/CORE-003 | SRT/LRC 导出、原子写和 Artifact Registry | exporters、artifact registry、tests | parse-back、冲突、partial failure、artifactRef owner/expiry/revoke/retry | — | 增强 LRC 不得自动交接 |
| MODEL-001 | 未开始 | — | BE-001/CORE-002 | 模型 manifest、managed 本地导入与 load smoke | model manager、model manifest、tests | header/size/hash/load、CTranslate2 拒绝、复制原子性 | — | 不接受任意外部路径作为运行时 modelId |
| MODEL-002 | 未开始 | — | MODEL-001/NATIVE-002 | 下载续传、VAD、删除、磁盘和 accelerator pack | download manager、accelerator manager、tests/UI service | Range/no-Range、`.part`、SHA、并发锁、busy delete、签名 | — | 下载源与 redistributable 结论需固化 |
| BE-002 | 未开始 | — | BE-001/MEDIA-001/SUB-002/MODEL-001/CORE-003 | Job Manager、批量队列、进度、取消和失败隔离 | job manager、IPC registration、tests | 状态机、单 GPU 串行、批次级错误、partial outputs、owner cleanup | — | 不支持中途断点续跑 |
...
| FE-001 | 未开始 | — | CORE-004/BE-002 | 工具注册、route、i18n 与单文件 SRT 纵向 UI | `App.tsx`、router、toolMeta、locales、page/store | route/meta、单文件 completed/reveal、四语言、tsc、Vite build | — | 与 Audio category/route 保持独立；尚不宣称完整批量 UX |
| FE-002 | 未开始 | — | FE-001/MODEL-002/BE-001 | 环境探测、设备与模型管理 UI | LocalSubtitleTranscriber components、services | executable probe、真实 backend、模型 CRUD、长诊断 containment | — | 不展示不可更新的 verified 状态 |
| FE-003 | 未开始 | — | FE-001/BE-002 | 文件授权、批量队列、进度与取消 UI | drop zone、queue/task components、store/tests | identity 去重、stale event、cancel race、窄窗口、键盘操作 | — | GPU 并发固定 1 |
| FE-004 | 未开始 | — | FE-003/BE-003/SUB-002 | 预览、结果操作、错误详情与手动交接入口 | preview/result/error dialogs、tests | reveal/copy/retry、partial outputs、ScrollableDialog、overflow | — | 手动交接由 LINK-005 接通 |
| LINK-001 | 未开始 | — | CORE-001 | 字幕翻译当前配置 Store 与分阶段无损迁移 | `useSubtitleTranslatorConfigStore.ts`、translator page/legacy adapter、migration tests | 双 import order、幂等、失败重试、live rehydrate、raw path 仅留待 LINK-003 切换 | — | API Key 仍归 `useModelStore`；旧 `outputURL` 不是授权 |
| LINK-002 | 未开始 | — | LINK-001 | 稳定 taskId、批量回执与精确启动 | subtitle types、queue service/store/tests | taskId、handoffKey 幂等、同名隔离、start ids only、late event、日志脱敏 | — | 本包不改目录授权；禁止自动调用 `startAllTasks()` |
| LINK-003 | 未开始 | — | LINK-002/CORE-003 | 字幕翻译目录 capability 与无路径任务引用 | main adapter/picker/registry、subtitle types/tests | capability cutover、registry authority、target ref resolve/rotate、legacy output migration | — | 新任务不得生成 `targetFileURL`；历史 path 分支隔离 |
| LINK-004 | 未开始 | — | LINK-003 | 无路径 checkpoint 与恢复兼容 | checkpoint/recovery discovery/types/tests | v2 manifest_fragments、checkpointRef、v1 conversion、atomic recovery writes | — | 未知版本拒绝；恢复 target 必须重新授权 |
| LINK-005 | 未开始 | — | LINK-004/SUB-002/CORE-003 | Artifact ref、one-shot import token、任务级 capability 与导入协调器 | `generatedSubtitleImportCoordinator.ts`、main/preload contracts、tests | ref owner/revoke、token TTL/one-shot、ID/handle 绑定、未入队立即撤销、snapshot secrecy、no raw path | — | 私有快照/capability 不持久化 |
| LINK-006 | 未开始 | — | LINK-005/FE-004/BE-002 | 三种后处理模式和逐文件流水线 | local page/store/coordinator wiring、i18n/tests | 三模式、格式选择、费用提示、partial failure、exact start | — | 默认必须为 `export_only` |
| QA-001 | 未开始 | — | NATIVE-002/BE-003/FE-001～004/LINK-006 | 自动化、边界与现有 Audio/Subtitle 回归矩阵 | `test/local-subtitle/*`、现有 audio/subtitle tests | tsc、i18n、全量 vitest、Vite test build、native ctest | — | 依赖链隐含全部 CORE/MEDIA/SUB/MODEL/LINK 包；fake runner 不能替代 packaged 验收 |
...
| QA-004 | 未开始 | — | QA-001/NATIVE-002/MODEL-002 | macOS arm64 Metal 与 x64 CPU packaged 验收 | mac release artifacts、验收记录 | 签名、公证、Gatekeeper、Metal/CPU、可执行位、更新 | — | 需要 arm64/x64 目标机和签名身份 |
...
验收口径：同一 manifest 可在 Windows/macOS 运行；缺失工具/样本会明确失败；仓库不含受限媒体、模型、二进制或凭据。
...
### PRE-004：macOS arm64 Metal 与 x64 CPU PoC

目标：证明 Apple Silicon 使用 Metal，并给 Intel Mac 定义诚实的 CPU 边界。

实施范围：

- 在 arm64 packaged-like bundle 中验证 Metal backend、模型加载、转写、取消和模型复用。
- 在 x64 目标机或合格等价环境验证 CPU runner；不得把 Rosetta/arm64 CPU 结果当作 Intel 实机结论。
- 验证签名后 runner 的可执行位、动态库解析、Gatekeeper 行为和资源路径。
- 使用与 Windows 相同的样本与指标 schema。

验收口径：arm64 日志和 probe 明确显示 Metal；x64 只声明真实验证过的 CPU 能力；性能不足必须进入产品提示。

### PRE-005：FFmpeg、sidecar staging、签名与许可证 PoC
...
- 用 `extraResources` spike 验证 Windows/macOS 资源布局、`${arch}` 产物名、可执行位、签名/公证顺序。
- 覆盖 mp3/wav/flac/aac/m4a/mp4/mkv/mov/webm、多个音轨、非 ASCII/长路径、损坏/零时长输入。
- packaged smoke 临时移除/隔离系统 FFmpeg/CUDA PATH，证明资源解析没有隐式依赖。

验收口径：能从安装产物内启动 runner 与 FFmpeg；许可清单可审计；当前 GPL/full 系统 FFmpeg 不被直接当作发行资源。
...
- Windows CUDA/CPU、macOS Metal/CPU 的支持矩阵和 fallback 文案。
...
- 冻结 IPC frame、媒体/字幕 artifact 字节数、cue/segment 数、diagnostics 长度和 batch 文件数上限；错误返回稳定 code，UI 在选择/启动前展示可操作限制。
- 状态机以纯函数实现并测试全部允许/拒绝迁移；事件携带 task generation，旧事件必须丢弃。
- 错误分为用户稳定 code 与受控 diagnostics；定义单文件错误、批次级错误和可重试性。

验收口径：types、schema、state transition 和序列化 round-trip 测试通过；`audio:*` 类型不被导入或扩展。
...
- 建立 `resolveLocalSubtitleResourcePath()` 和只读 runtime artifact manifest。
...
- 验证 manifest version、protocol、platform/arch/backend、文件大小、SHA 与可执行位。

验收口径：dev/packaged resolver 单测通过；缺失/错架构/错 hash 立即失败；业务代码无散落 `process.cwd()` 或相对资源路径。
...
- 文件、输出目录、artifact ref/import token 分 registry，绑定 owner/TTL/allowed operations；路径 resolve 后检查 containment，并覆盖 symlink/junction/reparse 风险。
- renderer cleanup queue 同时处理 rejected Promise、`ok:false` 和幂等 `revoked:false`，SPA 卸载后继续有界重试。

验收口径：内部/prefix-confusable channel 均被拒绝；跨 webContents/session、reload/navigation 后重放、过期、重复消费、目录逃逸和失败 revoke 测试通过。
...
- Store 仅 partialize 模型 ID、device preference、语言/VAD/质量/整形/格式和安全显示名。
- task queue、File、真实路径、token、segments、stderr 和临时路径不持久化。
- 事件 reducer 处理 generation、重复事件、乱序进度、取消 race、partial completion 和 post-action 独立状态。
- capability 清理由 renderer service 持有，不依赖单一页面组件生命周期。

验收口径：rehydration 不恢复任务/token；旧 generation 无法覆盖新任务；cleanup 重试和 TTL 结束行为可测。
...
- JSONL parser 有最大 frame、UTF-8 校验、stdout purity、request id 和错误枚举；stderr 限长脱敏。
- 单进程单任务，跨任务模型驻留；abort 超时后由父进程强杀。

验收口径：native unit/integration tests 覆盖所有命令和异常；相同模型连续任务不重载；进程退出无 orphan thread。

### NATIVE-002：原生构建矩阵与 artifact manifest

目标：生成可重复的 win-x64 CPU/CUDA、mac-arm64 Metal、mac-x64 CPU artifact。

实施范围：

- build script 固定 toolchain、flags、engine commit 和输出目录；禁止从开发机随机复制 DLL。
- 每个 artifact 生成版本、平台、架构、backend、依赖、大小、SHA-256 和 license manifest。
...
- 在 staging 流程稳定后才更新 `electron-builder.json` 的 `extraResources` 和 `${arch}` artifact name；打包前置脚本必须对缺失 manifest/二进制给出明确错误，不能生成缺 runtime 的“成功安装包”。

验收口径：四种 artifact 均可复现并与 manifest 匹配；错误架构不会被打包或加载。
...
- probe 时长/音轨/codec，返回受控 streamId + default/language/title/codec/channels 摘要；`auto` 优先唯一 default，否则首轨并提示，用户可逐文件覆盖。main 在执行前把 streamId 与同一 authorized file identity/最新流表重新校验，再转 16 kHz mono PCM16 WAV。
...
验收口径：格式矩阵、无音轨、唯一/多个/无 default、多音轨逐文件覆盖、伪造/陈旧 streamId、文件替换、损坏输入、非 ASCII/长路径、取消和超期 temp 清理全部通过。
...
验收口径：0、59.999、1h+、空文本、长词、重叠、缺词时间戳和多语言 fixtures 通过。
...
- SRT `HH:MM:SS,mmm`、UTF-8 no BOM；标准行级 LRC 为首版交接格式。
...
验收口径：golden/parse-back/冲突/原子失败测试通过；artifactRef 跨 owner、过期、撤销、文件替换/symlink、hash/size 变化、重复 reveal/handoff 请求和清理受控，renderer 无路径。
...
- 批次启动时冻结成员列表、model manifest/hash、device/language/preset/VAD/整形/输出/冲突/handoff 设置；custom 模式持有 main-owned batch output lease，source 模式按任务从 authorized file identity 派生/校验父目录写入目标。等待任务只读 snapshot，运行中新增文件进入新 draft batch，配置中途修改只影响新批次。
...
- owner/window/app lifecycle 统一释放 input/output token、temp 和 child process。

验收口径：单文件和批量、配置中途修改、原 snapshot 重试/当前配置新 generation、模型 hash/custom lease 失效、source 多父目录与单目录不可写隔离、失败隔离、取消 race、模型切换、batch-level failure、partial outputs 和 cleanup 测试通过。
...
- 会话 manifest 只保存非敏感摘要和已提交 artifact metadata；未完成任务标记 interrupted，不声称断点续跑。
...
验收口径：模拟 crash、强杀、OOM、disk full、cleanup 再入和启动恢复；无进程/临时文件/capability 泄漏。
...
- 复用 `ToolDetailLayout`、`ToolConfigSection`、`ToolFileDropZone`、`ToolRadioButtonGroup`，不复制视觉 CSS。
...
- 预览分页/摘要、复制纯文本、在文件夹中显示、按格式展示 partial success。
- 手动“一键送入字幕翻译”先提供入口和状态，实际 coordinator 由 LINK-005 接通。
...
- 为避免本包单独落地时破坏现有手动翻译，旧 `outputURL` 暂由显式 legacy adapter 继续提供当前行为；只有 LINK-003 的 capability picker、task target ref 和回滚测试全部通过后才能删除该源值。旧路径在任何阶段都不能被自动交接当成授权。
...
验收口径：两种 module import order、hydrate 前读取、旧数据缺失/损坏、写失败、二次迁移、same-session retry/live rehydrate 全覆盖；新 Store 无 token/raw path，迁移失败保留旧行为且不误标完成，字幕翻译原回归通过。旧 `outputURL` 的最终清理由 LINK-003 验收，不能提前宣称完成安全切换。
...
验收口径：同名不同任务、同一 taskId/handoffKey 重试、入队前失败重试、commit 后回执丢失、删除后自动重试、receipt 集合不变量、partial add、精确 start、并发满、取消/进度 late event、旧恢复任务和原队列不被启动测试通过；日志扫描无完整 task、API Key 或字幕内容。本包不改目录授权，现有字幕翻译回归必须保持通过。

### LINK-003：字幕翻译目录 capability 与无路径任务引用

目标：在不依赖自动交接 coordinator 的前提下，先让字幕翻译运行时支持 main-only 目录解析和 generated source 标记，并安全完成旧输出目录迁移。

实施范围：

- 新目录选择器由 main 直接签发 sender-bound capability，renderer 只保存内存 token 与 label；新建手动/生成任务都使用 `authorized_directory` target ref。target handle 绑定 owner/taskId/写操作/会话租约，终态、删除、窗口销毁或超时撤销。
...
- capability picker、target ref 和回滚测试通过后，从旧 Store partialize/localStorage 删除 `outputURL`；写入或回读验证失败则保留 legacy 源值、回滚页面切换并保持自动 `custom` 禁用。

验收口径：目录切换成功/写失败/回读失败/重启重选、同任务原子 reauthorize、跨 owner/终态/换绑拒绝、旧 handle 撤销、旧 raw path 清理、generated registry 权威校验、恶意/设备/超长文件名、target containment/symlink、互斥 schema、历史 path 分支隔离、租约过期/撤销和日志/持久化扫描测试通过。

### LINK-004：无路径 checkpoint 与恢复兼容

目标：在目录 capability 切换稳定后，单独迁移字幕翻译恢复合同，避免把 checkpoint path/token 持久化或返回 renderer。
...
验收口径：v2 round-trip、无 path/token/key 扫描、checkpointRef 跨 owner/reload/过期/撤销、重启重签、target 重新授权、v1 可转换/不可转换/未知版本、manifest 原子写 crash 矩阵通过。

### LINK-005：Artifact ref、one-shot import token、任务级 capability、配置快照与导入协调器

目标：在不共享 Store、路径或密钥的前提下，把标准 SRT/LRC 交给已经具备安全任务引用的字幕翻译模块。

实施范围：

- `artifactRef` 可在同会话内用于 reveal/重试；每次 `handoffArtifact` 都先以 no-follow/containment 方式复核 identity/size/SHA、UTF-8、格式、cue 数和固定最大字节数，再把不可变内容快照放入新的 owner-bound、短 TTL、one-shot `translationImportToken` 内存记录。消费成功或失败后 token 都不可复用，且消费阶段不再重读文件路径；随后生成绑定在一起的候选 `taskId`、`handoffKey` 和 target handle，只返回字幕内容、展示名、无路径 source 标记和不透明 target ref，不返回真实路径。
- translator-owned coordinator 先等待 config/model Store readiness，再在批次开始原子冻结安全配置、私有 model fields 和 `handoffMode`，返回 discriminated prepare result、脱敏 summary + opaque snapshotId；超时返回 `configuration_not_ready`，不得使用默认值。`importArtifact` 不接收 `autoStart`，只能执行 snapshot 冻结模式。`custom` 目录从当前 capability 派生 snapshot-bound batch lease，只能在同 owner/窗口存活/批次 active 时有界续期，并设最大墙钟寿命。
...
验收口径：配置/目录中途修改不影响批次快照；enqueue snapshot 无法通过 payload 篡改为 auto-start，失败预检不产生 snapshot；长任务 batch lease 有界续期、最大寿命和过期提示正确；artifact changed/too large/invalid UTF-8/cue 上限、token 内容快照与消费后清零可测；候选 ID/handle 不可换绑；duplicate、partial add、入队前异常和协调器取消后无 orphan handle；release retry 可收敛；入队后估算/启动失败保留任务并准确归入 `startFailures`；renderer 永远看不到 source/target raw path；密钥、path 和 capability 不进入持久化/log；token 跨 owner/过期/重放/非法 LRC 均拒绝，恢复缺少自包含输入时明确要求重新导入。

### LINK-006：三种后处理模式与逐文件流水线
...
node_modules/.bin/vitest run test/local-subtitle src/store/tools/subtitle src/services/subtitle test/audio
...
验收范围：安装路径含空格/非 ASCII、无系统 FFmpeg、CUDA pack 缺失/损坏、CPU fallback、large-v3 长媒体、批量取消、OOM、更新后模型保留、卸载策略、签名和 artifact arch 命名。
...
### QA-004：macOS packaged Metal/CPU 验收

目标：在签名/公证后的 DMG/ZIP 验证 Apple Silicon Metal 和 Intel CPU fallback。

实施范围：

验收范围：Gatekeeper、可执行位、arm64/x64 artifact 不覆盖、无 Homebrew FFmpeg、Metal probe、长媒体、取消、模型保留、更新兼容和 crash cleanup。

验收口径：公证后的 arm64 packaged app 通过 Gatekeeper、显示真实 Metal backend 并达到准确度/RTF 门槛；`auto` 在 Metal 不可用时于 commit 前明确解析为 CPU，显式 Metal 或 commit 后故障不静默降级。x64 packaged app 完成 CPU 与更新矩阵；签名、可执行位、取消和 cleanup 全部通过后才可声明 Metal，x64 不声明未验证的 GPU。
...
- 扫描新本地字幕 Store/manifest、生成字幕翻译任务/checkpoint、相关日志和 crash artifacts，确认无媒体内容、完整路径、密钥/header/模型 token；同时确认 LINK-003 成功迁移后 legacy `outputURL` 已从持久化移除。历史手动翻译恢复清单的 path 兼容分支单独列入已有行为审计，不能被误报为本功能生成的新数据。
- 核对 runner/whisper.cpp/model/VAD/FFmpeg/CUDA/构建工具的来源、版本、SHA、许可证、notice 和源码获取说明。
...
实施范围：README、CHANGELOG、隐私说明、第三方 notices、模型/加速包下载说明、硬件矩阵、磁盘需求、错误恢复、卸载/模型保留和 Final Design/Execution Plan 台账。
...
| 层级 | 必测内容 | 不能替代它的验证 |
| --- | --- | --- |
| Pure TS | schema、状态机、时间轴、formatter、queue reducer、config migration | Electron 截图 |
...
| Native | JSONL frame、模型 load/reuse、progress/segment/abort、CPU/CUDA/Metal build | fake runner |
| Integration | file token → media → runner → canonical → SRT/LRC → artifact | 单个 formatter test |
| Translation link | snapshot、one-shot token、path-free generated source、opaque target ref、main-only target path resolve、receipt、exact start、failure isolation | 手动 `addTask()` 或把 token 换成 renderer raw path 后适配旧字段 |
...
- **G2 数据门禁**：时间轴不可回读、原子写会破坏旧文件、单文件失败会删除其他产物即 No-Go。
- **G3 费用门禁**：默认会调用外部翻译、auto-start 可启动旧任务、配置摘要与实际 snapshot 不一致即 No-Go。
- **G3.5 路径授权门禁**：生成任务的 source/target path 进入 renderer、持久化 raw path/capability、或从旧 `outputURL` 静默扩权即 No-Go。
...
下一次实现会话只认领 `PRE-001`：建立样本/指标 manifest、clean-room 与许可证候选清单、工具链预检脚本和 PoC 记录模板。

不要直接下载并提交模型，不要先画完整页面，也不要把当前系统 PATH 中的 GPL/full FFmpeg 当作发行资源。`PRE-001` 完成后再进入 `PRE-002` 的 CPU persistent runner PoC。
