...
截至2026-08-10，`FE-003`产品模型已收敛为扁平任务队列：drop zone一次接收并授权最多100个文件，文件立即以draft task进入列表；再次选择追加且Store保留既有顺序。Start All只提交probe-ready成员，成功后按fileToken精确消费已转移capability，failed draft继续留在列表。revisioned session snapshot/event仍是已提交任务权威源，但renderer只flatten内部`batch.tasks`，不显示batch标题、编号或进度；main的batchId/config snapshot/lease transaction/queue admission仍保留。任务标题栏提供Start All、Clear Completed与确认式Clear All，逐task继续支持cancel/retry/CPU retry/remove/reveal/handoff。多音轨改为行内Select。main以owner-session scoped随机opaque sourceKey表达canonical path相等关系，draft/task summary贯通，页面和Store双层拒绝未清除重复文件并回收新token；raw path/path hash均不进入renderer。SPA route unmount保留draft queue，已提交任务继续由页面外service/main运行。Electron窄窗口/键盘/cancel race属于`QA-002`，不阻塞`FE-004`。
