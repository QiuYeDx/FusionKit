...
> 创建日期：2026-07-16；最近更新：2026-07-23
>
...
| FS-TXN-001 | 进行中 | 2026-07-23 | SUB-002/CORE-002 | 目录句柄相对 overwrite transaction | branded Coordinator/Exporter、macOS arm64 protocol v3/journal v2、composite recovery owner/file repository、exact reauthorization recovery、test-only fault/crash harness、tests、设计/台账/实施记录 | native 11/11；focused 7 files / 257 pass；local-subtitle 37 pass + 2 skip files / 883 pass + 2 skip tests；全量138 pass + 2 skip files / 1836 pass + 2 skip tests；TypeScript、三段Vite test build、manifest 0/0、validator17/17与diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-22_FS-TXN-001_overwrite-transaction-contract-and-exporter-integration.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-23_FS-TXN-001A_macos-arm64-native-overwrite-component.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-23_FS-TXN-001B_darwin-child-leaf-threat-and-recovery-ownership.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-23_FS-TXN-001C_composite-recovery-owner-and-native-protocol-v3.md` | 001A～001C已完成；prepared handoff非durable。下一checkpoint `FS-TXN-001D`仅实现Windows x64 component；durable finalize decision、production main/reauthorization IPC/UI、verified staging/builder、gate放行与packaged validation仍未完成，双重`index-only` gate保持 |
...
> 2026-07-23 `FS-TXN-001C` 已完成composite recovery owner component checkpoint：protocol v3/journal v2与exact transaction ID消除caller-supplied recovery metadata；branded owner统一持有native receipt/journal、Registry reservation/activation与owner/task/generation/format，file repository只保存path-free records。Exporter的prepared handoff只保证同进程预留ID/metadata、single-claim和明确`prepared/claimed/discarded`状态，不是durable/atomic handoff；begin已修改namespace后、正式adopt持久化前退出的窗口仍未关闭。production main仍未实例化native runtime/repository/owner，reauthorization IPC/UI也未接线。下一checkpoint `FS-TXN-001D`严格为Windows x64 component，不顺带扩张上述cross-process或production composition范围。
...
未完成：prepared handoff的durable preclaim/open decision、finalize crash与Registry commit/revoke的durable cross-process decision、Windows x64 backend与lossless FileId、generation-bound verified load、resource staging/builder、production main/reauthorization IPC/UI接线和两平台packaged validation。当前macOS证据只覆盖cooperative-writer协议和宿主APFS developer component，不覆盖non-cooperative writer、断电或production composition，因此`FS-TXN-001`保持进行中。下一component checkpoint `FS-TXN-001D`仅闭环Windows x64 protocol v3/journal v2 parity、directory HANDLE/RootDirectory-relative operations、reparse/no-follow边界、lossless identity与fresh-process fault/crash/真实Node load。
...
当前进展（2026-07-23）：Job Manager/revision/capability foundation、最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次和逐文件失败隔离已完成；production executor已串联MEDIA normalization、Supervisor pinned task lease、逐窗identity-bound inference、SUB-001 raw gate/retry/canonical processing、cleanup boundary与SUB-002 SRT/LRC index exporter，并按请求顺序支持SRT-only、LRC-only与无重复双格式。至少一个格式commit即稳定结算completed：普通第二格式失败为无warning partial，commit后取消由artifact marker形成cancellation partial；late cancel与lease abort不覆盖已提交结果。batch runtime pin按queue admission execution wave持有并绑定完整exact load identity，阻止sibling间model smoke、不兼容切换和idle retirement。source output从task input authorization-time parent proof私下派生并按写入边界重验，不使用全局目录lease。`FS-TXN-001A`～`FS-TXN-001C`已提供branded receipt、macOS arm64 protocol v3/journal v2、composite owner/file repository和exact reauthorization recovery组件；但`electron/main/index.ts`仍实例化未配置transaction/owner的Exporter，production main、reauthorization IPC/UI与verified staging未接线，Job Manager/Executor仍不接通overwrite。main已实例化并合并Session/Model/Job IPC。该进展只证明production code、unit/contract harness与raw native component integration，不包含renderer UI、packaged native E2E或完整BE-002范围验收。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`两个进行中包及21个未开始包。`FS-TXN-001A`～`FS-TXN-001C`已完成macOS arm64 protocol v3/journal v2、cooperative-writer边界、composite owner/file repository与exact reauthorization recovery组件合同。下一步是Windows-only `FS-TXN-001D`：实现win32/x64 parity、directory HANDLE + RootDirectory-relative child operations、reparse/no-follow、lossless volume/FileId codec、existing/absent victim、terminal retry、exact recovery、fresh-process fault/crash与真实Node load。001D不包含prepared handoff durable preclaim、finalize-crash decision、resource staging/builder、production main injection/reauthorization IPC/UI、gate放行或packaged validation；这些仍由后续cross-platform composition与NATIVE/QA闭环。只有全部证据完整后才能解除Job Manager/Executor双重index gate，完整验收前`FS-TXN-001`与`BE-002`保持进行中。
