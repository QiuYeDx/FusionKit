...
>
> 当前状态：39个顶层包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001I`与`NATIVE-002A` component checkpoints已完成；002A新增exact source-build receipt，将macOS arm64 official server/media与protocol-v4 addon组装到canonical ignored root，并通过beforePack static双门禁、CPU production-model private health、media decode与no-PATH。focused Node 24/24通过；server final SHA-256为`b5227f8b...23b65e`，runtime manifest为`19271b1f...22530`，addon generation为`cce55b2e...0530d5`。Metal production-model health因buffer allocation failure未通过，不能以embedded capability或PRE-004替代。Job Manager / Production Executor双重`index-only` gate不变；剩余范围为Metal production-model资源失败收敛、真实Windows protocol v4、Windows official CPU/CUDA + media assembly/launch/no-PATH，以及macOS arm64 / Windows x64 packaged validation
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23～2026-07-27完成`FS-TXN-001A`～`FS-TXN-001I` component checkpoints，2026-07-28完成`NATIVE-002A` macOS arm64 official runtime assembly component checkpoint，均不增加顶层工作包数量；三个顶层包仍等待Metal production-model资源失败收敛与Windows/runtime/packaged剩余矩阵
>
...
| NATIVE-002 | 进行中 | 2026-07-28 | NATIVE-001/CORE-002 | 三类 official server artifact、runtime manifest 与 builder 接线 | staging/source-build/smoke scripts、resource manifests、`electron-builder.json`、可选 workflow | 001G完成canonical builder/addon消费合同；002A focused Node 24/24，macOS exact source build、canonical server/media/addon assembly、beforePack static双门禁、CPU production-model private health、media decode/no-PATH通过。final server `b5227f8b...23b65e`、manifest `19271b1f...22530`、addon generation `cce55b2e...0530d5`；Metal production-model health因buffer allocation失败 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001G_verified-native-addon-staging-load-and-builder-contract.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-28_NATIVE-002A_macos-arm64-official-runtime-assembly.md` | `NATIVE-002A` component完成但顶层仍进行中；Windows CPU/CUDA + media assembly/launch/no-PATH、两平台packaged矩阵与Metal production-model resource failure仍未收敛，签名凭据不得进入仓库 |
...
未完成范围严格收口为：真实Windows protocol v4 compile/load/terminal/recovery/crash/acknowledgement矩阵；Windows CPU/CUDA + media canonical assembly与target launch/no-PATH；Metal production-model buffer allocation failure收敛；macOS arm64/Windows x64两平台packaged validation。`NATIVE-002A`已关闭macOS assembly/CPU health/media no-PATH component，但`FS-TXN-001`、`BE-002`与`NATIVE-002`仍进行中，`FE-001`仍未开始，Job Manager/Production Executor双重`index-only` gate继续保留。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中包及20个未开始包；`FE-001`仍未开始。`FS-TXN-001A`～`FS-TXN-001I`与`NATIVE-002A` component checkpoints已完成。下一步闭环真实Windows protocol v4矩阵、Windows CPU/CUDA + media artifact assembly/target launch/no-PATH、Metal production-model资源失败，以及macOS arm64/Windows x64 packaged validation。全部证据完整前Job Manager/Executor双重`index-only` gate不变，`FS-TXN-001`、`BE-002`与`NATIVE-002`不得标记完成。
