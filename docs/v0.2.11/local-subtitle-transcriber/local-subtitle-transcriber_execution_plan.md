...
> 创建日期：2026-07-16；最近更新：2026-07-28
>
...
>
> 当前状态：39个顶层包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001I`、`NATIVE-002A`与`NATIVE-002A1` component checkpoints已完成；002A新增exact source-build receipt，将macOS arm64 official server/media与protocol-v4 addon组装到canonical ignored root，并通过beforePack static双门禁、CPU production-model private health、media decode与no-PATH。focused Node 24/24通过；server final SHA-256为`b5227f8b...23b65e`，runtime manifest为`19271b1f...22530`，addon generation为`cce55b2e...0530d5`。002A在restricted sandbox中因7.33 MiB Metal allocation失败并在private health前触发SIGSEGV；002A1随后对同一canonical manifest与production模型`d75795ec...ad1`执行unrestricted hardened复验，约8秒通过Metal private health/media decode/no-PATH，backend probe记录model load 531～547 ms、peak RSS 1,516,158,976～1,535,049,728 bytes、initialization/device为true、failure为false、`backendVerified=true`；CPU hardened smoke仍通过。Job Manager / Production Executor双重`index-only` gate不变；剩余范围为真实Windows protocol v4、Windows official CPU/CUDA + media assembly/launch/no-PATH，以及macOS arm64 / Windows x64 packaged validation
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23～2026-07-27完成`FS-TXN-001A`～`FS-TXN-001I` component checkpoints，2026-07-28完成`NATIVE-002A` macOS arm64 official runtime assembly及`NATIVE-002A1` unrestricted Metal production-model复验component checkpoints，均不增加顶层工作包数量；三个顶层包仍等待Windows/runtime/packaged剩余矩阵
>
...
| 项目 | 当前状态 | 对计划的影响 |
| --- | --- | --- |
| Electron 打包 | 正式 `electron-builder.json` 已接入唯一canonical `extraResources` mapping、固定 `beforePack`、macOS arm64 / Windows x64 target、`${arch}` artifact name与macOS runtime `signIgnore`；001G冻结official runtime + overwrite addon的`launch:false`双verifier消费合同，002A已用真实macOS canonical assembly通过该static gate | macOS arm64 target assembly/CPU health/media no-PATH已闭环，但这不是packaged证据；Windows assembly/launch与两平台packaged矩阵仍属进行中的`NATIVE-002`。builder只能消费版本化staging，不能提交machine path或在electron-builder期间联网 |
...
| NATIVE-002 | 进行中 | 2026-07-28 | NATIVE-001/CORE-002 | 三类 official server artifact、runtime manifest 与 builder 接线 | staging/source-build/smoke scripts、resource manifests、`electron-builder.json`、可选 workflow | 001G完成canonical builder/addon消费合同；002A focused Node 24/24，macOS exact source build、canonical server/media/addon assembly、beforePack static双门禁、CPU production-model private health、media decode/no-PATH通过。final server `b5227f8b...23b65e`、manifest `19271b1f...22530`、addon generation `cce55b2e...0530d5`；002A1以同一manifest/production模型`d75795ec...ad1`完成unrestricted hardened Metal private health/media decode/no-PATH，约8秒ready，独立backend probe记录model load 531～547 ms、peak RSS 1,516,158,976～1,535,049,728 bytes、initialization/device=true、failure=false、backend verified；CPU hardened smoke通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001G_verified-native-addon-staging-load-and-builder-contract.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-28_NATIVE-002A_macos-arm64-official-runtime-assembly.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-28_NATIVE-002A1_macos-arm64-metal-production-model-verification.md` | `NATIVE-002A`与`NATIVE-002A1` components完成但顶层仍进行中；仍缺Windows CPU/CUDA + media assembly/launch/no-PATH与两平台packaged矩阵，签名凭据不得进入仓库 |
...
截至2026-07-28，本工作包仍为进行中。001G已完成唯一canonical `extraResources` mapping、固定beforePack、macOS arm64 / Windows x64 target、macOS sign-ignore，以及official runtime + overwrite addon的`launch:false`双verifier消费子合同。`NATIVE-002A`进一步完成macOS arm64 exact-source official server构建receipt与canonical assembly：unsigned raw server为3,641,216 bytes / `aba72ffc...b1be5d`，nested ad-hoc signed final server为3,638,160 bytes / `b5227f8b...23b65e`，FFmpeg/ffprobe分别为`55f36865...bacc53`与`8dfe0a7a...60d09`，runtime manifest为`19271b1f...22530`，protocol-v4 addon为`f54671ef...73ecc2`、generation为`cce55b2e...0530d5`。artifact为thin arm64、minimum macOS 11.0、8/8 system-only dependencies并内嵌Metal；beforePack static双门禁、CPU + production large model private `/health`、media decode与no-PATH通过。002A的Metal尝试在restricted sandbox中只能分配7.33 MiB并在private health前触发SIGSEGV；`NATIVE-002A1`随后对同一canonical manifest和production模型`d75795ec...ad1`执行unrestricted hardened target复验，约8秒通过Metal private health/media decode/no-PATH，独立backend probe记录model load 531～547 ms、peak RSS 1,516,158,976～1,535,049,728 bytes、initialization/device=true、failure=false、`backendVerified=true`，CPU hardened smoke也通过，因此Metal resource blocker已撤销。完整Windows CPU/CUDA + media assembly/launch/no-PATH和两平台packaged smoke仍未完成。
...
未完成范围严格收口为：真实Windows protocol v4 compile/load/terminal/recovery/crash/acknowledgement矩阵；Windows CPU/CUDA + media canonical assembly与target launch/no-PATH；macOS arm64/Windows x64两平台packaged validation。`NATIVE-002A`已关闭macOS assembly/CPU health/media no-PATH component，`NATIVE-002A1`已关闭Metal production-model resource blocker；`FS-TXN-001`、`BE-002`与`NATIVE-002`仍进行中，`FE-001`仍未开始，Job Manager/Production Executor双重`index-only` gate继续保留。
...
当前进展（2026-07-28）：Job Manager/revision/capability foundation、最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次和逐文件失败隔离已完成；production executor已串联MEDIA normalization、Supervisor pinned task lease、逐窗identity-bound inference、SUB-001 raw gate/retry/canonical processing、cleanup boundary与SUB-002 SRT/LRC index exporter。`FS-TXN-001A`～`FS-TXN-001I`已提供两平台protocol v4/journal v3、durable recovery、cross-platform identity、generation-bound addon staging/load、production main与recovery IPC/UI；`NATIVE-002A`已补充macOS arm64 canonical official runtime及CPU private health/media no-PATH，`NATIVE-002A1`已用同一manifest/production模型补齐unrestricted Metal private health/no-PATH与backend verification。Job Manager/Executor仍不接通overwrite；当前证据不包含真实Windows protocol v4、packaged native E2E或完整BE-002范围验收。
...
剩余overwrite范围：真实Windows protocol v4矩阵；Windows CPU/CUDA + media canonical assembly与target launch/no-PATH；macOS arm64/Windows x64两平台packaged validation。`NATIVE-002A`/`NATIVE-002A1`的macOS CPU/Metal health与media no-PATH不是完整product E2E；VAD、translate与非export-only路径也未完成。上述范围完成前`FS-TXN-001`、`BE-002`与`NATIVE-002`均不得标记为已完成，Job Manager/Executor双重`index-only` gate不得解除。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中包及20个未开始包；`FE-001`仍未开始。`FS-TXN-001A`～`FS-TXN-001I`、`NATIVE-002A`与`NATIVE-002A1` component checkpoints已完成。下一步闭环真实Windows protocol v4矩阵、Windows CPU/CUDA + media artifact assembly/target launch/no-PATH，以及macOS arm64/Windows x64 packaged validation。全部证据完整前Job Manager/Executor双重`index-only` gate不变，`FS-TXN-001`、`BE-002`与`NATIVE-002`不得标记完成。
