...
> 创建日期：2026-07-16；最近更新：2026-08-06
>
...
>
> 2026-08-04 `FE-002` Windows CUDA production admission checkpoint：Accelerator Manager签发main-only branded proof并在admission/执行前复验pack generation、exact manifest、server及全部DLL的path/size/SHA/file identity；Windows对象身份使用fixed-width volume serial + 128-bit FileId，不经过JS `number` inode或birthtime。Resolver仅在Windows x64 production attestor与fixed managed pack同时可用时解析CUDA，显式CUDAfail closed，`auto`只在commit前允许CPU解析。Descriptor从pack server目录以最小环境启动，load/pin绑定base runtime与完整pack组合身份；Supervisor在ready前要求exact PID WDDM dedicated memory正值，只有计数器不可用时才尝试同PID `nvidia-smi`，attestation额外绑定accelerator resource与pack generation。删除检查queued/pin/lease/resident使用。聚焦7 files / 159 tests、完整local-subtitle 51 files / 1048 tests（2个real-native skip）、TypeScript、三段Vite test build、manifest与diff通过；未执行真实Windows/NVIDIA或packaged产品验收，`FE-002`保持进行中。
>
> 2026-08-05 `MODEL-002` / `FE-002` managed VAD与完整配置checkpoint：Job Manager在enqueue、确认式CPU retry和task执行前解析、冻结并复验固定managed VAD identity，把VAD纳入queue-admission runtime slice、pin identity和pending/queued/running删除占用；Executor把真实`vadEnabled`送入segment-only request，继续强制`token_timestamps=false`。renderer配置面板接通device preference、源语言、原文/转英文task mode、质量预设、VAD、session-only initial prompt、6项数值高级设置、SRT/LRC、index/overwrite和模型GGML/量化摘要；backend preview按device preference做generation/identity guard，VAD启用但资源未ready时禁止开始，请求冻结全部当前偏好。既有server contract已原生支持`translate_to_english`，本轮只解除Job Manager/Executor的production gate并保留同一identity/VAD/质量合同。9 files / 256 focused tests、TypeScript、四语言各1777 keys/source usage、renderer/main/preload三段Vite test build与diff check通过；未启动前端服务，真实VAD下载/native smoke、目标GPU和packaged产品证据仍未执行，因此两个顶层包继续保持进行中。
>
> 2026-08-06 `MODEL-002` / `FE-002`真实资源与Windows production admission结项：固定1,081,140,203-byte模型和677,887,125-byte CUDA archive的SHA-256与manifest一致；VAD Manager完成真实公网885,098-byte下载、固定hash、CPU native load smoke、原子提交、ready解析与删除。真实archive暴露官方ZIP条目位于`Release/`而旧guard假定扁平叶名的问题，现已把exact prefix纳入contract并继续拒绝absolute、traversal、reserved name、symlink/reparse、unknown/duplicate与zip bomb；Manager随后完成20个PE安全解包/逐文件hash、最小环境probe、原子提交与删除。Windows CPU/CUDA production-model smoke均通过，CUDA exact PID观测到1,815,437,312-byte峰值VRAM；production resolver冻结managed pack proof，Supervisor通过同一exact-PID attestor后进入ready。两项opt-in real tests各1/1、archive/manifest与manager/resolver/attestor/VAD隔离回归6 files / 51 tests、TypeScript和manifest gate通过。packaged Electron/installer、发布许可与最终文档声明仍归`QA-003`～`QA-005`/`DOC-001`，不再阻塞两个代码工作包结项。
>
> 2026-08-06 `MODEL-002`手工验收取消修复：下载器此前只在取得新chunk后检查`AbortSignal`，第一轮修复又只依赖`discard()`唤醒iterator，真实UI仍出现取消后字节持续增加。active response现绑定abort并同时销毁request/response，每个`iterator.next()`额外直接与signal竞争，因此不合作的pending body也无法继续写文件；fixed public IPC回归证明cancel返回`true`，ResourceJob进入`cancelled`且`.part`、metadata和staging全部清理。聚焦3 files / 54 tests、本地字幕53 files / 1071 tests、TypeScript、三段Vite test build与preload bundle gate通过；按用户要求未启动应用或真实下载，1.08 GB UI取消留用户复验。
>
...
>
> 2026-08-04 `FE-003` media probe/track selection结项：draft文件严格串行调用fixed `probeMedia(fileToken)`，展示大小、时长、音轨数、脱敏音轨元数据与可重试错误；pending/failed probe阻止开始，多音轨使用完整交互面的纵向`ToolRadioButtonGroup`，默认保持`auto`，只有显式覆盖提交main签发的opaque `audioStreamId`。原子enqueue把显式选择从bounded draft LRU提升为task-owned main authority，绑定owner/file/task/exact identity/runtime generation与duration/table/selected-track signatures；回滚恢复draft capability，retryable failed保留选择，其他终态/remove/owner release/shutdown释放。7 files / 142 focused tests、TypeScript、四语言各1670 keys与source usage、renderer/main/preload三段Vite test build、diff check通过；未启动前端/Electron服务，窄窗口/键盘/cancel race与批量部分授权失败产品态转`QA-002`，`FE-003`按代码职责完成。
>
> 2026-08-04 `FE-004` artifact result/preview checkpoint：completed task按请求格式展示full/partial与committed/failed/skipped，成功artifact分别提供preview/reveal；preview只消费fixed `readArtifactText`返回的validated `rawText/plainText/cueCount`，以12,000字符固定页限制DOM并复制完整plain text，不在renderer解析字幕。失败task打开结构化error dialog，展示stage/retryable/cause/field与main已脱敏的summary/lines/metadata；两个长内容面都使用`ScrollableDialog`并约束Radix viewport内部宽度。8 files / 145 focused tests、TypeScript、四语言各1701 keys/source usage、三段Vite test build与diff check通过。该checkpoint时`handoffArtifact`按合同返回`configuration_not_ready`，因此未暴露必然失败的按钮；手动与自动交接入口随后已由`LINK-008`完成，`FE-004`现已结项。
>
> 2026-08-04 `BE-003`结项：新增固定`<userData>/local-subtitle/session-summary.v1.json`的strict private repository，以`0700` root、`0600` temporary、fsync和atomic rename只保存匿名task label、状态/阶段、格式、backend/build、时间、稳定error code与数值资源水位；path、token、capability、model hash、artifact ref、字幕正文、segment、命令和诊断文本均不落盘。重启时非终态task转为`interrupted + runtime_crashed`，terminal摘要保持终态；公开recovered schema和只读上一会话UI不含reveal/handoff/retry authority。受控启动清理扩展到exact `temp/server-*`、`temp/media/media-*`与summary temporary，未知叶名、symlink、replacement及成功artifact保持fail closed；当前进程server/media recursive removal补Windows有限重试。11 files / 199 tests、TypeScript、四语言各1706 keys/source usage、三段Vite test build与diff check通过；未做crash/强杀/OOM/disk-full真实Electron矩阵，归QA。
>
> 2026-08-04 `LINK-006`结项：Artifact Registry新增有效ref复用、过期ref完整identity/size/SHA/UTF-8/格式/cue复核后的安全轮换，以及旧ref稳定过期语义；completed session snapshot只为可复核的过期ref发布revisioned替换，并同步更新`artifactResults`与`completion.artifacts`。`handoffArtifact`公开响应只返回owner-bound短TTL one-shot token及expiry，main-private consumer取得不可变字幕内容、格式、展示名、cue数与path-free artifact identity，消费成功/失败、TTL、owner end或result clear/task remove后Buffer均清零且token不可重放。production JobManager、IPC和session bridge共享同一组artifact/token/handoff实例；本包不读取翻译Store，不创建taskId、handoffKey或target handle。6 files / 76 focused tests、完整local-subtitle 53 passed + 2 skipped files / 1062 passed + 2 skipped tests、TypeScript、renderer/main/preload三段Vite test build与diff check通过；下一步进入`LINK-007`。

> 2026-08-04 `LINK-001` / `LINK-007A` checkpoint：新增字幕翻译模块自有安全配置Store，分阶段复制语言、输出/切片/冲突/并发偏好和path-free目录展示名；API Key、endpoint、capability/token与raw `outputURL`不进入新Store，legacy key继续保留。cross-key bootstrap在新旧Store任一hydrate前幂等执行并做exact readback，失败时协调器fail closed；同会话失败后即使重写成功仍要求clean reload。translator-owned coordinator等待config/model hydration后冻结公开脱敏摘要、私有model fields和可选custom batch lease；enqueue-only无profile生成`needs_configuration`，auto-start无profile拒绝，snapshot TTL/release fence/并发合并/失败恢复已覆盖。该checkpoint不创建`taskId`、`handoffKey`或target handle，不消费LINK-006 token，不接入旧filename-keyed queue，也不调用`startAllTasks()`；`LINK-003`随后补齐目录capability，`LINK-007B`现已完成真实token消费与target handle所有权转移，`LINK-007`按职责结项，`LINK-001`继续等待页面/Agent/恢复消费者cutover。
>
> 2026-08-04 `LINK-002`结项：`SubtitleTranslatorTask`新增受控生成且不可由编辑接口换绑的稳定`taskId`，queue/Store/UI/Agent/recovery/renderer event/Electron main active controller/cancel/checkpoint统一按ID定位，同名任务可并存。根级模型字段收敛为`ready | needs_configuration` execution binding，单个、全部和精确批量启动均在effect前拒绝待配置或非法绑定；RecoveryDialog只启动本次新增ID。新增内存import ledger，原子提交handoff reservation与队列插入，支持partial add、同owner/snapshot跨receipt精确重放、冲突预检、删除后幂等及snapshot release；回执不含正文、密钥或路径。相关21个测试文件 / 127项测试、TypeScript、三段Vite test build与diff check通过；legacy路径与恢复三字段去重按LINK-003～005顺序保留。
>
...
| 字幕翻译任务身份 | LINK-002已把queue/Store/UI/Agent/renderer event/main active controller/cancel/checkpoint切换为稳定`taskId`，并建立幂等import ledger与精确start回执 | LINK-003～005继续在保持task identity的前提下迁移source/target/checkpoint路径消费者；自动交接只能启动receipt新增ID |
...
| FE-002 | 已完成 | 2026-08-06 | FE-001/MODEL-002/BE-001 | 环境探测、设备、VAD与模型管理 UI | 环境/resource UI、完整配置、device-aware preview、managed VAD门禁、原文/转英文、macOS Metal、Windows managed CUDA与确认式CPU新generation均完成；resolver/load/pin/attestor冻结并复核exact runtime/artifact/model/VAD/backend/pack identity | 既有auto/preview/Metal/CPU retry/CUDA矩阵；真实VAD native smoke；Windows x64 CPU/CUDA production-model smoke；RTX 4070 Ti SUPER production resolver + Supervisor exact-PID CUDA attestation 1/1通过 | 既有FE-002记录、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-06_MODEL-002_FE-002_windows-real-resource-admission-closure.md` | 代码/target component职责已完成；packaged Electron、installer和产品UX矩阵归QA-002/003/005，不展示虚假GPU执行或静默fallback |
| FE-003 | 已完成 | 2026-08-04 | FE-001/BE-002 | 文件授权、批量队列、进度与取消 UI | `LocalSubtitleTranscriber/{index,LocalSubtitleDraftMediaList,LocalSubtitleTaskQueue,localSubtitleTranscriberModel}.tsx/ts`、`media-normalizer.ts`、`job-manager.ts`、page/model/main tests、`ToolRadioButtonGroup`、`src/locales/*/subtitle.json`；完成1～100文件draft/request、snapshot batch/task列表、逐task操作、串行media probe、多音轨Radio与task-owned selection proof | 7 files / 142 focused tests；TypeScript、四语言各1670 keys与source usage、renderer/main/preload Vite test build、diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_FE-003_batch-draft-task-queue.md`；`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_FE-003_media-probe-track-selection.md` | 代码职责已完成；批量部分授权失败产品态、Electron窄窗口/键盘/cancel race归`QA-002`；GPU并发固定1 |
...
| LINK-004 | 已完成 | 2026-08-05 | LINK-003 | 普通页面与 Agent 新建任务生产者 cutover | translator page、Agent tools/prompt、authorized input/selection main registry、preload、store/execution/tests | 普通/Agent新任务path-free source/target ref、main identity-bound read/register/reveal、fixed picker selection receipt；28 files / 166 tests、TypeScript、三段Vite通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_LINK-004_translator-page-producer-cutover.md` | 恢复扫描继续保留旧适配到LINK-005，不得删除`outputURL`或把raw recovery path升级为authority |
| LINK-005 | 已完成 | 2026-08-05 | LINK-004 | 无路径 checkpoint、恢复消费者兼容与最终 cutover | checkpoint/recovery discovery、Agent recovery tools、RecoveryDialog、renderer events、main translation/types/tests | path-free v2 manifest、owner-bound scan/ref、fixed picker target重授权、v1白名单转换、task-scoped atomic recovery/final output、删除清理、legacy path删除；15 files / 112 tests、TypeScript、三段Vite通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-05_LINK-005_path-free-recovery-final-cutover.md` | Electron人工picker/窗口/cancel race仍归QA-002；历史v1仅main读取 |
| LINK-006 | 已完成 | 2026-08-04 | LINK-005/SUB-002/CORE-003 | Artifact ref 与 one-shot import token | `subtitle-artifact-registry.ts`、`artifact-handoff.ts`、`session-ipc.ts`、`ipc.ts`、main composition、tests | 6 files / 76 focused tests；完整local-subtitle 53 pass + 2 skip files / 1062 pass + 2 skip tests；TypeScript、三段Vite test build、diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_LINK-006_artifact-one-shot-handoff.md` | 已完成safe ref rotation、main-only内容快照、TTL/one-shot/clear与task/owner清理；不创建taskId/target handle，不导入翻译Store |
| LINK-007 | 已完成 | 2026-08-04 | LINK-006/LINK-002/LINK-003 | 翻译配置快照、任务级 capability、导入协调器与回执闭环 | hydration-aware snapshot coordinator、dual-owner fixed IPC、main-private candidate、source/custom lease、atomic queue ownership transfer、immutable receipt/exact start、tests | 扩大回归30 files / 215 tests；TypeScript、三段Vite test build、diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_LINK-001_LINK-007A_translator-config-snapshot.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_LINK-007B_translation-import-coordinator.md` | 私有快照/capability不持久化；下游页面/Agent/恢复cutover与三模式接线已由LINK-004/005/008完成 |
| LINK-008 | 已完成 | 2026-08-05 | LINK-007/FE-004/BE-002 | 三种后处理模式和逐文件流水线 | local page/confirm dialog/task results、renderer-session post-action service、fixed completion IPC、Job Manager/Registry/Executor、translator coordinator existence query、四语言/tests | 12 files / 251 focused tests；TypeScript、四语言各1740 keys/source usage、renderer/main/preload Vite test build、diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-05_LINK-008_three-mode-post-action-pipeline.md` | 默认保持`export_only`；代码职责已完成，真实外部API、Electron cancel/import竞态和目标机验收归QA |
...
| DOC-001 | 进行中 | 2026-08-06 | QA-005 | README、CHANGELOG、隐私、第三方清单与发布说明 | `README.md`、`CHANGELOG.md`、`local-subtitle-transcriber_release_notes.md`、版本/主题台账、既有notices/manifests | 本地Markdown链接检查通过；`node scripts/local-subtitle/benchmark/validate-manifests.mjs`；`git diff --check` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-06_DOC-001_prerelease-documentation-baseline.md` | 已完成只陈述代码/既有component证据的预发布基线；最终平台、性能、下载链接和许可声明必须等待QA-005后回填，不把候选backend写成stable支持 |
...
- `readArtifactText` 与 `revealArtifact` 已接入 app-scoped `LocalSubtitleIpcService`，响应无路径；公开 `handoffArtifact` 已由 `LINK-006` 接入one-shot token，仍不返回path/content/hash。`rawText + plainText` 必须共同满足 frozen 16 MiB result schema，超出时返回 `content_too_large`。
...
2026-08-06结项：复用既有固定生产模型与完整官方CUDA archive，新增opt-in真实资源测试。VAD走production downloader的真实公网下载、静态校验、CPU native load smoke、commit/resolve/delete；CUDA走Accelerator Manager真实archive guard、20个PE安全解包与逐项hash、最小环境probe、atomic commit/resolve/delete。真实archive纠正了扁平fixture未发现的`Release/`entry root合同错误，修复后仍保持exact allowlist与traversal/reparse/unknown/duplicate/zip-bomb fail-closed。资源生命周期和目标组件证据已闭环，顶层`MODEL-002`改为`已完成`；packaged installer与发布许可仍由QA包负责。

2026-08-06手工验收修复：真实UI暴露点击取消后模型继续下载；第一轮仅绑定abort→discard后，用户再次确认下载字节仍持续增加，说明transport discard本身也不能作为iterator结算证据。第二轮同时销毁ClientRequest/response并把每次`iterator.next()`与signal竞争，但继续实测仍显示cancel spinner和递增字节。最终补齐上层原因：每个HTTPS小chunk同步产生ResourceJob revision与Electron event，1.01 GB下载形成消息积压，使cancel invoke和`cancelling`事件排在旧progress后；现在progress在IPC前按100 ms合并、await write后重查abort。首次瞬时连接/HTTP/短响应也不再直接终结job，而是在同一job内最多两次可取消重试并安全续传；无validator从零重启时保持已发布进度单调。聚焦3 files / 61 tests覆盖首连失败、503、body resume、无validator重启、retry-delay cancel、256 chunk首尾progress、1024 chunk public IPC单job低事件量和不合作stalled body；修复不改变公开IPC/schema/commit合同，按要求不启动应用、不触发公网下载，真实大模型UI复验继续由用户完成。
...
截至2026-08-04，确认式CPU新generation与Windows CUDA production admission均已完成代码checkpoint：失败task只有在main根据真实`resolvedBackend`与固定GPU/runtime错误白名单签发`cpuRetryAvailable`时才显示入口；用户确认后fixed IPC只提交`taskId + generation`。Windows CUDA只消费managed pack branded proof，admission与执行前复验完整server/DLL组合身份，exact child在ready前必须取得绑定pack generation的exact-PID正显存attestation。普通retry保留当前execution binding；确认CPU retry创建相邻新generation和全新queue admission/runtime slice。renderer不持有backend proof、pack path/hash或运行时身份。真实Windows NVIDIA、unrestricted Metal与packaged产品证据仍未完成，故`FE-002`保持`进行中`。

截至2026-08-05，可在当前开发环境完成的配置与VAD代码链路也已闭环：完整配置面板冻结device/language/task mode/quality/VAD/session-only prompt/advanced shaping/output/conflict，转英文与prompt位于高级区；model摘要显示main提供的GGML/quantization信息。device-aware preview防止旧preference响应覆盖，VAD启用但fixed managed资源未ready时阻止开始；Job Manager与Executor把managed VAD identity纳入enqueue/retry/execute、runtime pin和删除占用，并启用既有`translate_to_english` server合同。剩余项只有真实资源下载、native/target GPU与packaged证据，仍保持`进行中`而不死磕当前环境无法提供的QA。

2026-08-06结项：Windows x64真实CPU/CUDA smoke使用固定production模型均通过，CUDA exact PID取得1,815,437,312-byte峰值VRAM；随后完整managed pack由production resolver签发branded CUDA resolution，Supervisor以相同pack generation、server artifact和exact PID通过production attestor并进入ready。真实公网VAD load smoke同样通过。macOS unrestricted Metal证据已由既有NATIVE-002A1 checkpoint提供，FE-002职责内的配置、预览、资源门禁、CPU/Metal/CUDA解析和确认式CPU新generation均已闭环，故改为`已完成`；packaged Electron/installer/UX发布矩阵继续归QA。
...
截至2026-08-04，`FE-003`代码职责已完成：drop zone一次接收并授权最多100个文件，request为每个opaque file token生成独立条目；Store继续负责替换/逐项移除/离页draft capability cleanup，成功enqueue后只清renderer draft，不撤销main已提交lease。已有任务不再阻止下一批draft与enqueue。任务区直接以revisioned session snapshot/event batches为权威源，展示batch与逐task状态、stage/stageProgress/overallProgress、backend、格式、媒体时长、artifact和错误，并提供cancel、普通retry、确认CPU retry、terminal remove及SRT reveal。enqueue回执fallback在同batch首次被runtime观察后即退休，避免task removal后由renderer fallback复活。每个draft文件严格串行probe，pending/failed阻止开始，多音轨使用纵向完整交互面的`ToolRadioButtonGroup`，默认`auto`、显式覆盖仅提交main签发的opaque stream id。原子enqueue把显式选择从bounded draft LRU提升为task-owned main authority，绑定owner/file/task/exact identity/runtime generation与duration/table/selected-track signatures；回滚、retry与终态清理均按任务生命周期处理。批量部分授权失败产品态、Electron窄窗口/键盘/cancel race属于`QA-002`，不阻塞`FE-004`。
...
截至2026-08-04，artifact result/preview checkpoint已完成：completed task按请求格式展示full/partial与committed/failed/skipped，成功artifact分别提供preview/reveal；preview只消费fixed `readArtifactText`返回的validated `rawText/plainText/cueCount`，以12,000字符固定页限制DOM并复制完整plain text，不在renderer用正则或临时parser处理字幕。失败task可打开结构化error dialog，展示stage/retryable/cause/field及main已脱敏的summary/lines/metadata；两个长内容面均使用固定Header/Footer的`ScrollableDialog`，并约束Radix viewport内部wrapper避免超长诊断扩大横向宽度。该checkpoint时`handoffArtifact`尚未接通，因此没有暴露必然失败的交接按钮；手动/自动交接、回执和已删除任务恢复入口随后由`LINK-008`完成，`FE-004`现已结项。
...
2026-08-04本工作包已完成。普通、Agent与恢复任务统一通过受控factory创建taskId；所有队列操作、token estimate、renderer事件、main active controller/cancel和checkpoint均按ID定位，main拒绝非法ID、无效ready binding及重复active identity。内存import ledger以candidate binding绑定handoff内容/配置，支持同owner/snapshot跨receipt精确重放、partial add、冲突预检、删除后幂等和snapshot release；receipt只含不透明ID与展示名。RecoveryDialog的“添加并开始”只启动本批`addedTaskIds`。legacy `originFileURL`/`targetFileURL`/`checkpointPath`和恢复三字段去重按LINK-003～005迁移顺序继续保留。
...
2026-08-04本工作包已完成。新增独立`subtitle-translation:*`固定preload namespace与document owner session，main目录registry冻结canonical identity并管理draft token、task-scoped target handle、TTL、终态与owner cleanup；generated/legacy严格互斥，main已登记generated taskId不能回退legacy。generated target只在main解析，启动与最终实际leaf写入前复核目录、containment和symlink；同owner active task支持失败不破坏旧handle的原子重授权，过期handle可重新选择目录。现有普通页面、Agent、RecoveryDialog和恢复路径继续走显式legacy adapter，`outputURL`及旧path字段未删除。聚焦与扩大回归共25 files / 141 tests，TypeScript、三段Vite test build和diff check通过。
...
2026-08-04普通页面producer checkpoint完成：新增`authorized_task_v1`，preload只在固定方法内从真实`File`提取路径，main冻结input file及其parent identity，并在identity前后复核之间读取UTF-8正文；source/custom target分别派生父目录proof或复用当前translator draft，注册后输入、输出handle与taskId不可换绑。页面新任务的兼容path字段为空，执行IPC剥离这些字段，只携带opaque reference；目录展示、source reveal和删除清理均走path-free label或owner-bound fixed API。`outputURL`继续只供未迁移恢复兼容。Agent仍拒绝以任意renderer raw path换取authority，下一checkpoint需新增main-owned`subtitleSelectionRef`和用户确认扫描后才能关闭LINK-004。

2026-08-05本工作包已完成。Agent `queue_subtitle_translate`改为调用固定原生多文件picker，由main在真实选择后签发owner-bound、短TTL `subtitleSelectionRef`与逐项opaque `itemRef`；读取正文和任务注册均要求同一selection/item binding，跨owner、过期、重放、重复文件和非字幕扩展名fail closed。Agent工具schema与executor显式拒绝`filePaths`、`scanId`和raw `outputDir`；custom输出只通过固定目录picker签发draft authority。注册任务的兼容path字段为空，未消费selection和custom draft在finally中进入保留opaque handle的有界撤销重试，已注册任务取得独立source/target authority。convert/extract和LINK-005 recovery路径未扩权或提前迁移。扩大回归28 files / 166 tests、TypeScript、renderer/main/preload三段Vite test build和diff check通过。
...
2026-08-05本工作包已完成。checkpoint升级为白名单重建的path-free v2，JSON、completed/remaining artifact与最终译文均使用同目录unique temporary、`fsync`、close、rename；新恢复artifact按稳定taskId hash隔离，同名任务不会互相覆盖。成功后删除含fragments的resume manifest，只保留脱敏`*.fusionkit.completed.json`；用户删除任务时，main先复核task-bound目录并逐项有界清理resume/completed/remaining/error/summary，全部成功后才释放authority，清理失败保留authority供renderer有界重试，且从不删除最终译文。main新增owner/document-session-bound短TTL `recoveryScanId`/`checkpointRef`，目录或单manifest均只能经fixed picker授权，prepare前复核manifest file identity/size/mtime并要求重新选择target；失败时刷新原子rename后的checkpoint identity，保证同owner retry可续跑。RecoveryDialog、Agent schema/executor/prompt、renderer events与Store已移除raw恢复path和`outputURL`，prepare失败会有界撤销未消费target draft；历史v1只在main读取并按字段白名单转v2，未知版本拒绝。15个聚焦测试文件/112项测试、TypeScript、renderer/main/preload三段Vite test build与diff check通过；Electron人工picker、窄窗口、键盘和cancel race留给QA-002。
...
2026-08-04 `LINK-007A`已完成配置快照前半段：安全配置Store与model Store hydration readiness、`ready | needs_configuration`、公开/私有字段隔离、snapshot TTL和custom lease release lifecycle已经实现。`LINK-002`随后完成稳定任务身份与精确启动，`LINK-003`进一步完成可安全承接生成字幕的无路径source/target、目录capability与原子重授权合同。

2026-08-04 `LINK-007B`完成导入闭环：fixed preload内部同时验证local/translation两个document owner session，main一次性消费token并按artifact稳定identity+snapshot生成taskId/handoffKey/candidate binding；source复用artifact冻结目录proof，custom从translator draft派生snapshot lease。candidate target先保持不可执行，renderer ledger同步入队成功后才幂等commit；duplicate、未入队与异常显式release，added task在releaseBatch后继续持有handle。协调器构造自包含字幕任务、尝试费用估算、返回满足三分区集合不变量的不可变receipt，auto-start只调用`startTasks(addedTaskIds)`；执行IPC剥离兼容path字段，删除/清空生成任务通过owner-bound固定API有界重试释放authority。旧`outputURL`与legacy path仍未删除，普通页面/Agent/Recovery cutover继续归`LINK-004`/`LINK-005`。
...
2026-08-06预发布文档checkpoint：新增独立预发布说明，覆盖功能/非目标、候选平台矩阵、固定资源体积与瞬时磁盘、bundled FFmpeg/no-PATH边界、默认离线与显式外部翻译费用、持久化例外、恢复/取消、更新/卸载保留和第三方许可入口；README与CHANGELOG只声明代码职责和既有component证据，明确Windows CUDA包仍为`artifactSharingAllowed=false`，未把CPU/CUDA/Metal候选写成stable支持。版本级台账已随MODEL-002/FE-002结项同步到当前33完成/5未开始/1进行中。最终installer、性能、下载链接、签名/公证和许可结论仍依赖QA-005，因此`DOC-001`保持进行中。
