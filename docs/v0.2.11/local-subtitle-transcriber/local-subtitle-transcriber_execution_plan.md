...
> 创建日期：2026-07-16；最近更新：2026-07-26
>
...
>
> 当前状态：`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002` 与 `MODEL-001` 已完成，M1 的 domain/runtime schema、versioned resource staging、preload/IPC/capability、renderer session、official server contract、真实 Supervisor 生命周期、media normalization/PCM proof、canonical post-processing、标准字幕原子产物与 managed model 合同已冻结；Windows 使用 `unsigned_personal_distribution`。`BE-002` 已支持最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次，可按请求顺序生成SRT-only、LRC-only或无重复双格式，并稳定结算full、普通partial与commit后取消partial；原子批次发布、app-global FIFO、逐文件失败隔离、owner分区续权、exact-identity batch runtime pin与可信多父目录source output已实现。`FS-TXN-001A`～`FS-TXN-001H` component checkpoints 已完成：macOS arm64与Windows x64当前冻结protocol v4/journal v3，schema-v2 path-free preclaim在native begin前持久化`rollback_unpublished + not_started`，Registry activation后持久化`finalize_committed`；native以`.finalize`/`.rollback` terminal marker收敛namespace，main持久化`settled`后才ack并删除marker/record。POSIX/Windows exact filesystem identity已贯通输出目录authorization、Artifact Registry、Exporter、recovery selection与Executor directory proof；001G新增独立versioned addon staging、content-addressed final-byte publication、generation-bound load proof与builder gate，001H再把canonical proof-only native runtime和versioned repository/composite owner组成production bootstrap，并将同一runtime/owner成对注入Exporter与SessionLifecycle。001H focused 11 files / 313 passed、local-subtitle 40 passed + 2 skipped files / 931 passed + 2 skipped tests、全量141 passed + 2 skipped files / 1884 passed + 2 skipped tests，overwrite-native 29 passed + 1 skipped，manifest 0/0、validator 17/17、TypeScript、三段Vite与diff通过。reauthorization IPC/UI、真实Windows protocol v4矩阵、完整official server/media artifact assembly与两平台packaged validation仍未完成；Job Manager / Production Executor继续保持双重`index-only` gate，`FS-TXN-001`、`BE-002`与`NATIVE-002`均保持进行中。39个顶层包中16个已完成、23个剩余（3个进行中、20个未开始）
>
> 发布门禁：M0 已解除；正式实现必须遵守 `poc/pre006-production-decision.json`，不得静默更换引擎、平台、模型或 runtime acquisition policy
>
> 2026-07-16 审查修订：补齐部分导出、会话 revision 重同步、resource job、runner 取消、无模型入队与 Agent/恢复消费者迁移；修正 pnpm/工具链事实，并把过大的 LINK 包拆为8个；2026-07-22新增`FS-TXN-001`后总计39个工作包；2026-07-23以`FS-TXN-001A`～`FS-TXN-001D`作为其平台/恢复component checkpoints，不增加总包数量；2026-07-24四个component checkpoints全部完成；2026-07-26新增并完成`FS-TXN-001E` cross-platform filesystem identity composition；2026-07-27新增并完成`FS-TXN-001F` durable recovery decisions、`FS-TXN-001G` verified native-addon staging/load + builder contract与`FS-TXN-001H` production overwrite main composition，顶层`FS-TXN-001`仍等待001I reauthorization IPC/UI、真实Windows与packaged composition
>
...
3. 阅读 `.agents/skills/fusionkit-pitfall-guard/references/index.md`，至少检查 `FK-PIT-0021`、`FK-PIT-0022` 与 `FK-PIT-0023`；实现本地推理/字幕合并时还必须检查 `FK-PIT-0027`、`FK-PIT-0028`、`FK-PIT-0031`、`FK-PIT-0032`；涉及 native build/staging/signing 时检查 `FK-PIT-0029`、`FK-PIT-0033`～`FK-PIT-0035`；涉及 child close、process epoch、startup retry、warm epoch 或 session finalization 时必须检查 `FK-PIT-0039`、`FK-PIT-0053`；涉及多 owner admission/capability renewal 时检查 `FK-PIT-0052`；涉及持久化、preload、capability、i18n、Electron 视觉验证时，再读取对应条目。
...
| 项目 | 当前状态 | 对计划的影响 |
| --- | --- | --- |
| Electron 打包 | 正式 `electron-builder.json` 已接入唯一canonical `extraResources` mapping、固定 `beforePack`、macOS arm64 / Windows x64 target、`${arch}` artifact name与macOS runtime `signIgnore`；001G已冻结official runtime + overwrite addon的`launch:false`双verifier消费合同 | 消费门禁已接通，但完整official server/media真实artifact assembly、target launch/no-PATH与packaged矩阵仍属进行中的`NATIVE-002`；只能消费版本化staging，不能提交machine path或在electron-builder期间联网 |
...
截至 2026-07-22，以上切片的 Electron main production code、IPC wiring 和 unit/contract harness 已完成；尚未接入 `FE-001`，也未使用真实 FFmpeg、official server 与 PRE-006 模型形成 native E2E 证据。因此 M2 仍未达成，不能把后端 harness 中可读 SRT artifact 的成功冒充用户可运行或 packaged 验收。
...
| NATIVE-002 | 进行中 | 2026-07-27 | NATIVE-001/CORE-002 | 三类 official server artifact、runtime manifest 与 builder 接线 | staging/source-build scripts、resource manifests、`electron-builder.json`、可选 workflow | 001G正式canonical `extraResources` / beforePack / mac signIgnore与addon双verifier消费子合同21/21；真实macOS addon build/sign/stage/fresh-load通过。完整win-x64 CPU/CUDA、mac-arm64 Metal/CPU与media artifact + launch/no-PATH + packaged smoke + SHA manifest仍待验证 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001G_verified-native-addon-staging-load-and-builder-contract.md` | builder消费子合同已部分完成；official server/media真实artifact assembly、目标平台launch/no-PATH和packaged矩阵未完成，签名凭据不得进入仓库 |
...
| FS-TXN-001 | 进行中 | 2026-07-27 | SUB-002/CORE-002 | 目录句柄相对 overwrite transaction | branded Coordinator/Exporter、macOS arm64 + Windows x64 protocol v4/journal v3 native components、schema-v2 path-free durable preclaim/decision、terminal marker + acknowledgement、composite recovery owner/file repository、exact reauthorization recovery、lossless Windows volume/FileId、cross-platform identity composition、versioned addon staging/content-addressed publication/generation-bound load proof与builder gate、canonical proof-only production bootstrap及Exporter/SessionLifecycle成对注入、test-only fault/crash harness、tests、设计/台账/实施记录 | 001H focused 11 files / 313 pass；local-subtitle 40 pass + 2 skip files / 931 pass + 2 skip tests；全量141 pass + 2 skip files / 1884 pass + 2 skip tests；overwrite-native 29 pass + 1 skip；manifest 0/0、validator 17/17、TypeScript、三段Vite与diff通过。不能替代真实Windows/packaged矩阵 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-22_FS-TXN-001_overwrite-transaction-contract-and-exporter-integration.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-23_FS-TXN-001A_macos-arm64-native-overwrite-component.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-23_FS-TXN-001B_darwin-child-leaf-threat-and-recovery-ownership.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-23_FS-TXN-001C_composite-recovery-owner-and-native-protocol-v3.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-24_FS-TXN-001D_windows-x64-native-overwrite-component.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-26_FS-TXN-001E_cross-platform-filesystem-identity-composition.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001F_durable-recovery-decisions.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001G_verified-native-addon-staging-load-and-builder-contract.md`、`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-07-27_FS-TXN-001H_production-overwrite-main-composition.md` | 001A～001H component checkpoints已完成；顶层仍进行中。下一步001I完成reauthorization IPC/UI，并先关闭新admission再等待已有recovery tails；随后闭环真实Windows protocol v4与两平台packaged validation；双重`index-only`保持 |
...
截至2026-07-27，本工作包进入进行中。001G已完成唯一canonical `extraResources` mapping、固定beforePack、macOS arm64 / Windows x64 target、macOS sign-ignore，以及official runtime + overwrite addon的`launch:false`双verifier消费子合同；完整win-x64 CPU/CUDA、mac-arm64 Metal/CPU与FFmpeg/ffprobe真实artifact assembly、target launch/no-PATH、最终SHA/signature evidence和两平台packaged smoke仍未完成。
...
- durable preclaim必须在native begin前以`rollback_unpublished + not_started`落盘；Registry activation后必须先持久化`finalize_committed`，fresh-process recovery只按该decision发布/续跑`.finalize`或`.rollback` terminal marker。pending状态看到`not_found`仍保留record/fence；只有not-started rollback preclaim能证明begin未建立journal，或已有durable settled proof后的acknowledgement返回`not_found`时，才可幂等完成。
- recovery owner只有在native terminal收敛后才能把schema-v2 record写为`settled`；settled持久化成功前禁止receipt/module acknowledgement。持久化结果不确定时保留record/fence并重试exact state；ack成功或幂等`not_found`后才删除record并释放所有directory fence。file repository使用0600 temp、fsync、atomic rename与exact payload read-back，但不构成power-loss声明。001H已完成production main composition，但IPC/UI、reauthorization与Job Manager/Executor双重`index-only` gate仍继续禁用overwrite。
...
- main只有在native backend及其resource manifest/load protocol通过时才把同一generation的Coordinator/recovery owner成对注入Exporter与SessionLifecycle；验证失败时不注入transaction。无论runtime是否ready，Job Manager与Production Executor的index-only双重gate均不变。
...
未完成：001I reauthorization IPC/UI（shutdown/owner-release先关闭新admission，再等待已有recovery tails收敛）、真实Windows protocol v4 compile/load/terminal/recovery/crash/acknowledgement矩阵、NATIVE-002完整official server/media artifact assembly与launch/no-PATH，以及两平台packaged validation。001H已关闭production main/repository/recovery-owner composition子合同，但不构成overwrite产品放行或完整product E2E，也不覆盖non-cooperative writer或断电。`FS-TXN-001`保持进行中，001A～001H component checkpoints均已完成；Job Manager/Production Executor双重`index-only` gate继续保留。
...
当前进展（2026-07-27）：Job Manager/revision/capability foundation、最多100文件的CPU/transcribe/no-VAD、custom或source、index-only、export-only批次和逐文件失败隔离已完成；production executor已串联MEDIA normalization、Supervisor pinned task lease、逐窗identity-bound inference、SUB-001 raw gate/retry/canonical processing、cleanup boundary与SUB-002 SRT/LRC index exporter，并按请求顺序支持SRT-only、LRC-only与无重复双格式。至少一个格式commit即稳定结算completed：普通第二格式失败为无warning partial，commit后取消由artifact marker形成cancellation partial；late cancel与lease abort不覆盖已提交结果。batch runtime pin按queue admission execution wave持有并绑定完整exact load identity，阻止sibling间model smoke、不兼容切换和idle retirement。source output从task input authorization-time parent proof私下派生并按写入边界重验，不使用全局目录lease。`FS-TXN-001A`～`FS-TXN-001H`已提供branded receipt、macOS arm64/Windows x64 protocol v4/journal v3、schema-v2 durable preclaim/finalize decision、terminal marker + acknowledgement、composite owner/file repository、exact reauthorization recovery、cross-platform exact identity composition、generation-bound addon staging/load与builder gate，以及canonical proof-only production main composition。`electron/main/index.ts`现在将同一runtime/recovery owner成对注入Exporter与SessionLifecycle；reauthorization IPC/UI仍未接线，Job Manager/Executor仍不接通overwrite。main已实例化并合并Session/Model/Job IPC。该进展只证明production code、unit/contract harness与native component/staging/main composition，不包含renderer UI、真实Windows protocol v4、packaged native E2E或完整BE-002范围验收。
...
剩余范围：production overwrite仍缺001I reauthorization IPC/UI、真实Windows protocol v4矩阵与两平台packaged validation；NATIVE-002完整official server/media真实artifact assembly、launch/no-PATH，CUDA/Metal、VAD、translate与非export-only路径也未完成。translation handoff、FE和packaged/native E2E分别由LINK、FE、NATIVE/QA工作包闭环。当前source output、batch pin、多格式、overwrite durable decision/composite owner、两平台raw component/identity composition、addon staging/builder与production main composition证据仍不是一次真实FFmpeg + official server + PRE-006模型及target packaged runtime的产品E2E；non-cooperative child writer safety明确不属于当前或未来本合同的保证范围。上述范围完成前`FS-TXN-001`与`BE-002`均不得标记为已完成，Job Manager/Executor双重`index-only` gate不得解除。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`与`MODEL-001`已完成。39个顶层工作包中16个已完成，剩余23个为`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中包及20个未开始包。`FS-TXN-001A`～`FS-TXN-001H`已完成macOS arm64/Windows x64 protocol v4/journal v3、schema-v2 durable preclaim/decision、terminal marker + acknowledgement、composite owner/file repository、exact reauthorization recovery、lossless Windows volume/FileId、cross-platform exact identity composition、addon generation-bound staging/load与builder consumption，以及canonical proof-only production main composition。下一步执行001I reauthorization IPC/UI，再由NATIVE/QA补完整official runtime、真实Windows和两平台packaged validation。只有全部证据完整后才能解除Job Manager/Executor双重index gate，完整验收前`FS-TXN-001`、`BE-002`与`NATIVE-002`保持进行中。
