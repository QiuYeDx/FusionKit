...
>
> 当前状态：39个顶层包中20个已完成、17个未开始、2个进行中。`MODEL-002A`～`MODEL-002D`已接通model/VAD/CUDA managed lifecycle与受控root启动孤儿清理；`FE-002`已完成环境/resource UI，并新增main-only identity-bound backend resolution proof：单文件请求以`auto`进入Job Manager，在capability commit前冻结`auto -> cpu`和exact runtime/server/model identity，Executor加载时重新核对同一proof。显式CUDA/Metal在production attestor缺失时稳定返回`backend_unverified`且不回退。顶层`MODEL-002`仍等待真实大文件下载与目标环境证据；`FE-002`仍等待开始前的main preview、GPU production admission和用户确认CPU新generation，均不提前标完成。真实模型/VAD/CUDA archive Electron产品E2E、NSIS生命周期、目标GPU与M2 packaged验收仍按QA边界处理
>
...
| FE-002 | 进行中 | 2026-08-04 | FE-001/MODEL-002/BE-001 | 环境探测、设备与模型管理 UI | 已接环境/resource UI；main-only branded backend proof在capability commit前冻结`auto -> cpu`与exact runtime/artifact/model identity，Executor load/pin二次核对；显式GPU无attestor时fail closed | backend/Job Manager/IPC/Executor/renderer model 5 files / 139 tests；TypeScript、三段Vite test build、四语言i18n与diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-03_FE-002_environment-resource-management-ui.md`；`docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_FE-002_main-auto-backend-resolution.md` | 开始前main preview、Metal/CUDA production admission/attestation、GPU失败后的用户确认CPU新generation未接线；不展示虚假GPU执行或静默fallback |
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`～`NATIVE-002`、`BE-001`～`BE-002`、`MEDIA-001`、`SUB-001`～`SUB-002`、`FS-TXN-001`、`MODEL-001`与`FE-001`已完成。39个顶层工作包中20个已完成、17个未开始，`MODEL-002`与`FE-002`进行中。`MODEL-002D`已关闭受控下载/staging root启动孤儿清理；`FE-002`已关闭环境/resource UI及commit-time main `auto -> cpu` exact proof，下一步应复用同一resolver增加开始前preview summary，再接通真实Metal/CUDA artifact与attestation；只有GPU generation真实进入production后才实现用户确认的CPU新generation。真实大文件下载、NSIS生命周期、目标GPU、CUDA分发许可、M2产品E2E与其余QA仍按原边界闭环；M3继续等待`MODEL-002`、`BE-003`与`FE-002`～`FE-004`。
