...
>
> 当前状态：39个顶层包中20个已完成、16个未开始、3个进行中。`MODEL-002A`～`MODEL-002D`已接通model/VAD/CUDA managed lifecycle与受控root启动孤儿清理；`FE-002`已完成环境/resource UI、main-only identity-bound backend resolution proof、开始前path/hash-free preview、macOS arm64 Metal、Windows x64 managed CUDA production admission及用户确认CPU新generation代码闭环。两平台enqueue均在capability commit前重新解析并冻结exact runtime/server/model identity；CUDA额外冻结pack generation与全部server/DLL identity，Supervisor在exact child进入ready前完成main-only positive attestation；GPU失败后只有main签发资格的exact generation可由用户确认创建显式CPU新generation，普通retry不改变backend。`FE-003`已进入进行中并完成1～100文件draft/batch request、revisioned session批次队列及逐task取消/重试/CPU确认重试/移除/reveal checkpoint，运行中允许准备并提交下一批。顶层`MODEL-002`与`FE-002`仍等待真实大文件下载、目标环境和packaged证据；`FE-003`仍等待多音轨probe/Radio与Electron窄窗口/键盘验收，均不提前标完成。真实模型/VAD/CUDA archive Electron产品E2E、NSIS生命周期、目标GPU与M2 packaged验收仍按QA边界处理
>
...
| FE-003 | 进行中 | 2026-08-04 | FE-001/BE-002 | 文件授权、批量队列、进度与取消 UI | `LocalSubtitleTranscriber/{index,LocalSubtitleTaskQueue,localSubtitleTranscriberModel}.tsx/ts`、page/model tests、`src/locales/*/subtitle.json`；完成1～100文件draft/request、snapshot batch/task列表与逐task cancel/retry/CPU retry/remove/reveal | renderer/store/runtime 9 files / 65 tests；TypeScript、四语言i18n/source usage、renderer/main/preload Vite test build、diff check通过 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_implementation_records/2026-08-04_FE-003_batch-draft-task-queue.md` | 多音轨probe/Radio、批量部分授权失败产品态、Electron窄窗口/键盘/cancel race验收未完成；GPU并发固定1 |
...
截至2026-08-04，基础批量draft与任务队列checkpoint已完成：drop zone一次接收并授权最多100个文件，request为每个opaque file token生成独立条目；Store继续负责替换/逐项移除/离页draft capability cleanup，成功enqueue后只清renderer draft，不撤销main已提交lease。已有任务不再阻止下一批draft与enqueue。任务区直接以revisioned session snapshot/event batches为权威源，展示batch与逐task状态、stage/stageProgress/overallProgress、backend、格式、媒体时长、artifact和错误，并提供cancel、普通retry、确认CPU retry、terminal remove及SRT reveal。enqueue回执fallback在同batch首次被runtime观察后即退休，避免task removal后由renderer fallback复活。多音轨probe/Radio与Electron窄窗口/键盘/cancel race验收仍未完成，因此`FE-003`保持`进行中`。
...
`PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`～`NATIVE-002`、`BE-001`～`BE-002`、`MEDIA-001`、`SUB-001`～`SUB-002`、`FS-TXN-001`、`MODEL-001`与`FE-001`已完成。39个顶层工作包中20个已完成、16个未开始，`MODEL-002`、`FE-002`与`FE-003`进行中。`MODEL-002D`已关闭受控下载/staging root启动孤儿清理；`FE-002`已关闭环境/resource UI、commit-time main exact proof、开始前path/hash-free preview summary、macOS arm64 Metal、Windows x64 managed CUDA production admission及用户确认CPU新generation代码闭环；`FE-003`已关闭基础多文件draft/batch request、revisioned batch/task队列与逐task操作。下一步继续`FE-003`的main media probe摘要和逐文件多音轨Radio选择，再做窄窗口/键盘/cancel race的必要Electron验收。真实大文件下载、NSIS生命周期、目标GPU、CUDA分发许可、M2产品E2E与其余QA仍按原边界闭环；M3继续等待`MODEL-002`、`BE-003`与`FE-002`～`FE-004`。
