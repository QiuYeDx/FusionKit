...
> 当前状态：Execution Plan 已按实时音频、全局设置、MiMo 流式要求修订；全部实现工作包均 `未开始`；下一步建议认领 `PRE-001`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| PRE-001 | 未开始 | — | 音频契约、全局设置模型、能力矩阵与 fake server | `src/type/audio.ts`、`src/type/audioIpc.ts`、`test/audio/fakeAudioApiServer.ts`、`test/audio/audioCapability.test.ts` | `node_modules/.bin/vitest run test/audio src/type/audioIpc.test.ts`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | 需要固定 OpenAI/MiMo response、SSE、Realtime fixture 的最小稳定字段 |
| CORE-001 | 未开始 | — | 全局音频 profile、assignment 与 store migration | `src/type/model.ts`、`src/store/useModelStore.ts`、`src/store/useModelStore.test.ts`、`src/lib/audio-profile.ts` | `node_modules/.bin/vitest run src/store/useModelStore.test.ts src/lib/audio-profile.test.ts`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | 需要决定 audio profile 是否阻止删除被引用的 connection profile |
| CORE-002 | 未开始 | — | Endpoint、文件、输出与流式音频工具 | `src/lib/audio-endpoint.ts`、`src/lib/audio-endpoint.test.ts`、`electron/main/audio/audio-file.ts`、`electron/main/audio/audio-stream.ts`、`electron/main/audio/audio-errors.ts` | `node_modules/.bin/vitest run src/lib/audio-endpoint.test.ts test/audio`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | PCM16 到 WAV 包装需覆盖采样率和声道元数据 |
| BE-001 | 未开始 | — | AudioRuntimeClient 骨架与 OpenAI 文件 ASR/TTS adapter | `electron/main/audio/audio-runtime-client.ts`、`electron/main/audio/adapters/openai-audio-adapter.ts`、`test/audio/audioRuntimeClient.test.ts` | `node_modules/.bin/vitest run test/audio/audioRuntimeClient.test.ts -t "OpenAI"`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | 先覆盖文件 ASR/TTS，不做真实网络 |
| BE-002 | 未开始 | — | MiMo ASR 与 MiMo TTS 非流式 adapter | `electron/main/audio/adapters/mimo-chat-audio-adapter.ts`、`test/audio/audioRuntimeClient.test.ts`、`test/audio/fakeAudioApiServer.ts` | `node_modules/.bin/vitest run test/audio/audioRuntimeClient.test.ts -t "MiMo non-stream"`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | MiMo ASR 仅启用 `auto/zh/en` 与 `json/text` |
| BE-003 | 未开始 | — | MiMo TTS 三模型流式低延迟输出 | `electron/main/audio/adapters/mimo-chat-audio-adapter.ts`、`electron/main/audio/audio-stream.ts`、`test/audio/mimoStreamingTts.test.ts` | `node_modules/.bin/vitest run test/audio/mimoStreamingTts.test.ts`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | 真实 `voicedesign/voiceclone` 低延迟状态需 QA-002 实测；若降级则该能力阻塞 |
| BE-004 | 未开始 | — | Audio IPC、stream event 与 renderer service facade | `electron/main/audio/ipc.ts`、`electron/main/index.ts`、`src/services/audio/audioTranscriptionService.ts`、`src/services/audio/speechSynthesisService.ts`、`electron/electron-env.d.ts` | `node_modules/.bin/vitest run src/type/audioIpc.test.ts test/audio/audioRuntimeClient.test.ts`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | IPC 不得接收完整音频内容，只接收文件路径和任务参数 |
| BE-005 | 未开始 | — | Realtime session runtime 与 OpenAI WebRTC bridge | `electron/main/audio/realtime-ipc.ts`、`electron/main/audio/realtime/openai-realtime-adapter.ts`、`src/services/audio/audioRealtimeService.ts`、`test/audio/audioRealtimeSession.test.ts` | `node_modules/.bin/vitest run test/audio/audioRealtimeSession.test.ts`；`node_modules/.bin/tsc --noEmit`；`git diff --check` | — | Electron WebRTC 实测在 QA-002，单测只覆盖 session contract 和 cleanup |
| FE-001 | 未开始 | — | 设置页全局音频模型配置 | `src/pages/Setting/components/AudioModelConfig.tsx`、`src/pages/Setting/components/ModelConfig.tsx`、`src/locales/*/setting.json`、`src/locales/*/audio.json` | `node_modules/.bin/tsc --noEmit`；`node scripts/check-i18n.mjs`；`git diff --check` | — | 不复用文本模型过滤列表作为音频模型列表 |
| FE-002 | 未开始 | — | 工具入口、路由、共享状态与 i18n 基线 | `src/pages/Tools/_shared/toolMeta.ts`、`src/pages/Tools/index.tsx`、`src/App.tsx`、`src/constants/router.ts`、`src/store/tools/audio/*`、`src/i18n/*` | `node_modules/.bin/tsc --noEmit`；`node scripts/check-i18n.mjs`；`git diff --check` | — | 保留现有 `music` soon 卡片，新增独立 audio 分类 |
| FE-003 | 未开始 | — | 音频转文本页面 | `src/pages/Tools/Audio/AudioTranscriber/index.tsx`、`src/pages/Tools/Audio/shared/*`、`src/store/tools/audio/useAudioTranscriberStore.ts`、`src/locales/*/audio.json` | `node_modules/.bin/tsc --noEmit`；`node scripts/check-i18n.mjs`；必要时 Electron 截图验收；`git diff --check` | — | 页面只读显示全局模型，不提供独立 API 选择 |
| FE-004 | 未开始 | — | 文本转音频页面与流式播放 | `src/pages/Tools/Audio/SpeechSynthesizer/index.tsx`、`src/pages/Tools/Audio/shared/*`、`src/store/tools/audio/useSpeechSynthesizerStore.ts`、`src/locales/*/audio.json` | `node_modules/.bin/tsc --noEmit`；`node scripts/check-i18n.mjs`；必要时 Electron 截图验收；`git diff --check` | — | MiMo 三模式流式开关必须按 capability 禁用 |
| FE-005 | 未开始 | — | 实时字幕页面 | `src/pages/Tools/Audio/RealtimeCaptions/index.tsx`、`src/store/tools/audio/useRealtimeCaptionsStore.ts`、`src/services/audio/audioRealtimeService.ts`、`src/locales/*/audio.json` | `node_modules/.bin/tsc --noEmit`；`node scripts/check-i18n.mjs`；Electron 麦克风验收；`git diff --check` | — | MiMo 分块近实时不得标记为 WebRTC |
| FE-006 | 未开始 | — | Realtime/WebRTC 双向语音页面 | `src/pages/Tools/Audio/RealtimeVoice/index.tsx`、`src/store/tools/audio/useRealtimeVoiceStore.ts`、`src/services/audio/audioRealtimeService.ts`、`src/locales/*/audio.json` | `node_modules/.bin/tsc --noEmit`；`node scripts/check-i18n.mjs`；Electron WebRTC 验收；`git diff --check` | — | 页面离开必须释放 mic tracks、peer connection 和 AudioContext |
...
| DOC-001 | 未开始 | — | README、CHANGELOG、隐私说明与发布文档同步 | `README.md`、`CHANGELOG.md`、`docs/v0.2.11/audio-toolkit/audio-toolkit_execution_plan.md`、必要时隐私/说明文档 | `git diff --check`；如改 i18n 则 `node scripts/check-i18n.mjs` | — | 需说明本地音频/麦克风内容会发送到用户选择的第三方 API |
...
- `voicedesign/voiceclone` 真实低延迟能力必须在 QA-002 验证；如降级为最终一次性返回，BE-003 和 QA-002 不得标为 `已完成`。
...
- 新增 `electron/main/audio/ipc.ts`，注册 `audio:transcribe`、`audio:synthesize-speech`、`audio:synthesize-speech-stream`、`audio:reveal-output`。
...
- 如类型需要，补齐 `electron/electron-env.d.ts`。
...
下一次实现会话建议认领 `PRE-001`。优先产出 `src/type/audio.ts`、`src/type/audioIpc.ts`、全局音频 profile/assignment 类型、capability guard 和 fake server，使 OpenAI/MiMo 文件音频、MiMo 流式 TTS、OpenAI Realtime 的协议差异先被测试固定住，再进入设置页、adapter 和 UI 实现。
