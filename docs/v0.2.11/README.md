...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`SUB-001` 已完成；private server lifecycle 与 segment-only canonical post-processing contract 已冻结。SUB-001 覆盖 structural root/retry graph、attempt/epoch/generation binding、verified no-speech、raw gate、owned-boundary merge、Unicode/grapheme-safe shaping 与 `estimatedTiming`；聚焦 4 files / 174 tests、全量 117 passed + 2 skipped files / 1223 passed + 2 skipped tests、TypeScript、三段 Vite test build 与 manifest/validator gate 通过。下一步可推进 `NATIVE-002` / `MEDIA-001` / `SUB-002` / `LINK-001`；`BE-002` 依赖仍未齐 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
