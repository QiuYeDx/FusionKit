...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`、`MODEL-001` 已完成；`BE-002` 的 Job Manager foundation 与单文件 CPU/transcribe/no-VAD/custom/SRT/export-only production 后端切片已接通 runtime admission、MEDIA → Supervisor → SUB → exporter identity binding、稳定 cleanup error、main/IPC wiring 与三阶段 session lifecycle，但工作包仍为进行中。local-subtitle 33 passed + 2 skipped files / 602 passed + 2 skipped tests、全量 134 passed + 2 skipped files / 1554 passed + 2 skipped tests、TypeScript、三段 Vite test build 与 manifest/validator gate 通过；2 个 skip 均为未启用的真实 native server 测试。下一步继续 `BE-002` 的多文件、source output、多格式和更多 backend/VAD 路径 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
