...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001`、`SUB-002`、`MODEL-001` 已完成；`BE-002` 的 backend unit/contract 已支持最多 100 文件的 CPU/transcribe/no-VAD、custom/source、index-only、export-only 批次，SRT/LRC 保序单/双格式、exact-identity batch runtime pin、可信 source parent isolation，以及 full、普通 partial、commit 后取消 partial 的稳定结算。`FS-TXN-001` 已完成同步 receipt 与 Exporter 接入，但 native backend/main injection 未完成；两个工作包均保持进行中，production 继续 index-only。local-subtitle 34 passed + 2 skipped files / 768 passed + 2 skipped tests、全量 135 passed + 2 skipped files / 1721 passed + 2 skipped tests、TypeScript、renderer/main/preload 三段 Vite test build、manifest 0/0、validator 17/17 与 diff gate 通过；canonical runtime staging 缺失并按合同 fail closed。下一步实现 macOS arm64 / Windows x64 directory-handle-relative backend、hostile parent replacement、rollback/victim recovery、resource staging 与 packaged validation，再按依赖扩展 CUDA/Metal/VAD/translate；translation handoff、FE 与 native/packaged E2E 仍分别闭环 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
