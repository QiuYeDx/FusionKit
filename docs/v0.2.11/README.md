...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001I` component checkpoints已完成；001I新增strict path-free分页IPC、stable displayCode、owner-scoped picker admission/锁内TTL、shutdown tail fencing与app-scoped四语言recovery UI。focused 175/175、local-subtitle 951 pass + 2 skip、全量1921 pass + 2 skip、overwrite-native 29 pass + 1 skip、manifest 0/0、validator 17/17、四语言各1522 keys、TypeScript、三段Vite与diff通过。Job Manager / Production Executor双重`index-only` gate不变；剩余overwrite范围仅为真实Windows protocol v4矩阵、完整official server/media artifact assembly + launch/no-PATH及macOS arm64 / Windows x64 packaged validation | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
