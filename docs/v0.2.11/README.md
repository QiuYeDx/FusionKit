...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`～`NATIVE-002C` component checkpoints已完成；002C已闭环真实macOS arm64 `.app` exact runtime/addon消费、outer-sign完整性、CPU/Metal production-model no-PATH与fault matrix。Job Manager / Production Executor双重`index-only` gate不变；下一步严格按002C实施记录闭环Windows x64真实app/installer目标机矩阵，CUDA delivery与分发许可仍归`MODEL-002`/`QA-005` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
