...
| `local-subtitle-transcriber` | 39个顶层工作包中20个已完成、19个未开始；当前无进行中的顶层包。`FE-001`已完成独立subtitle route/tool注册、四语言、runtime/resource probe、ready model选择、opaque单文件/目录授权、CPU/no-VAD/SRT/index/export-only入队、session进度/取消和artifact reveal。TypeScript、5 files / 28 tests、四语言各1577 keys与usage、三段Vite test build通过；真实模型Electron产品E2E未执行，不提前声明M2 packaged验收。下一步`MODEL-002`、`BE-003`或`FE-002`，NSIS生命周期留`QA-003` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
