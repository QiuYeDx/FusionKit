...
| `local-subtitle-transcriber` | 39个顶层工作包中20个已完成、18个未开始、1个进行中。`MODEL-002A`已接通首发模型下载/删除；`MODEL-002B1`已冻结CUDA archive redirect allowlist并实现同句柄全量SHA、两遍central-directory校验、安全流式解包、逐文件hash、no-clobber和取消清理。local-subtitle 45 files / 982 tests、TypeScript、manifest 0/0与validator 17/17通过。`MODEL-002`仍等待VAD、accelerator ResourceJob/install/probe/原子提交回滚、启动孤儿清理与真实大文件下载，不提前声明完成或M2 packaged验收。下一步继续`MODEL-002B2`，随后接`FE-002` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
