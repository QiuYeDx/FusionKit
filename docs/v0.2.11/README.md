...
| `local-subtitle-transcriber` | 39个顶层工作包中33个已完成、5个未开始、1个进行中。CORE/NATIVE/BE/MEDIA/SUB/FS-TXN、MODEL、FE与LINK均已按非QA代码职责闭环；`MODEL-002`已通过固定模型、真实公网VAD和完整CUDA archive生命周期，`FE-002`已通过Windows production CUDA target component admission。`DOC-001`已新增预发布说明并同步README/CHANGELOG/版本台账，最终平台、性能、下载链接和许可声明仍等待QA-005，因此保持进行中且不提前声明M2/M3、stable backend或packaged产品验收完成 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
