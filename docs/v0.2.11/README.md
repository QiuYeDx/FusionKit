...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-004` 已完成，`PRE-005` 进行中；macOS arm64 已通过 FFmpeg 8.1.2 LGPL build、signed manifest、builder 正反向门禁、outer ad-hoc deep/strict 和 packaged no-PATH media/fault matrix。下一步补 Windows x64 packaged/AuthentiCode 与 FFmpeg 固定指纹验签；Developer ID/Gatekeeper 归未来 `QA-004` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
