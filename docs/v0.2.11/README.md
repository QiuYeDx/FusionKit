...
| `local-subtitle-transcriber` | `PRE-001`、`PRE-002` 已完成：Node 管理官方 `whisper-server` 已在同一 CPU 模型进程跑通现有 3 段中/日媒体与取消恢复，证明 Windows 当前无需 CMake/MSVC/C++ runner；下一步 `PRE-003` 官方 CPU/CUDA + 目标模型验证，发布版 FFmpeg 固定内置 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
