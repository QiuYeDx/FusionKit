...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-005` 已完成；macOS arm64 与 Windows x64 bundled runtime、builder 正反向及 packaged no-PATH media/fault matrix 通过，runtime tests 38/38。Windows 按本人/朋友分发范围使用 explicit unsigned profile，不创建证书或修改信任库；下一步 `PRE-006` production 技术冻结 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
