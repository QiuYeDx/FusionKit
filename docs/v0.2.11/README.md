...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006` 已完成，M0 技术冻结通过；exact engine/platform/model/VAD/FFmpeg/体积决策已有机器校验。Windows personal/friend profile 保持 unsigned，不创建证书或修改信任库；下一步 `CORE-001` / `CORE-002` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
