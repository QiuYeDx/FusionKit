...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（2个进行中、21个未开始）。`BE-002`已支持最多100文件的CPU/transcribe/no-VAD、custom/source、index-only、export-only批次与SRT/LRC保序多格式。`FS-TXN-001A`～`FS-TXN-001E` component checkpoints已完成：macOS arm64闭环protocol v3/journal v2、exact transaction ID、composite recovery owner、path-free repository与exact reauthorization recovery；Windows x64闭环HANDLE/RootDirectory-relative operations、reparse/no-follow、lossless volume/FileId、existing/absent terminal与fresh-process recovery；cross-platform exact identity已贯通输出目录authorization、Artifact Registry、Exporter、recovery selection与Executor proof。001E定向10 files / 330 passed，TypeScript、三段Vite、manifest与validator通过。prepared handoff非durable，production main未注入，双重`index-only` gate不变。下一步继续durable decision、staging/builder/main/IPC/UI及packaged validation | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
