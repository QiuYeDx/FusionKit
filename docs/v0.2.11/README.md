...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（2个进行中、21个未开始）。`BE-002`已支持最多100文件的CPU/transcribe/no-VAD、custom/source、index-only、export-only批次与SRT/LRC保序多格式。`FS-TXN-001A`～`FS-TXN-001F` component checkpoints已完成：两平台当前冻结protocol v4/journal v3、schema-v2 path-free durable preclaim/decision、terminal marker + acknowledgement、composite owner/exact reauthorization recovery、Windows lossless volume/FileId与cross-platform exact identity；finalize decision同payload有界重试，native finalizer对`.open`方向中立，abandoned-open四象限矩阵已覆盖。001F验证为macOS native 11/11、Windows contract 6 pass + 真实Windows 1 skip、local-subtitle 914 pass + 2 skip、全量1867 pass + 2 skip、TypeScript、三段Vite、manifest 0/0与validator 17/17通过。production main/IPC/UI、verified staging/builder、真实Windows与两平台packaged validation未完成，双重`index-only` gate不变；`FS-TXN-001`仍进行中 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
