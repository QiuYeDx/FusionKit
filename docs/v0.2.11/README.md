...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（2个进行中、21个未开始）。`BE-002`已支持最多100文件的CPU/transcribe/no-VAD、custom/source、index-only、export-only批次与SRT/LRC保序多格式。`FS-TXN-001A`～`FS-TXN-001C`已完成macOS arm64 developer components；001C闭环protocol v3/journal v2、exact transaction ID、composite recovery owner、path-free repository、component-level prepared handoff与exact reauthorization recovery。prepared handoff非durable，production main未注入，双重`index-only` gate不变。native 11/11、focused 7 files / 257 passed、local-subtitle 37 passed + 2 skipped files / 883 passed + 2 skipped tests、全量138 passed + 2 skipped files / 1836 passed + 2 skipped tests、TypeScript、三段Vite test build、manifest 0/0、validator17/17与diff check通过。下一步`FS-TXN-001D`只做Windows x64 protocol parity、HANDLE-relative operations、reparse/no-follow、lossless FileId与fresh-process evidence；durable finalize decision、staging/builder/main/IPC/UI及packaged validation后续闭环 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
