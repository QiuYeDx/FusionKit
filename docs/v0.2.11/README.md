...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001` 已完成；official server HTTP/process contract、verified bundle load identity 与诊断边界已冻结，全量 113 passed + 1 skipped files / 1109 passed + 1 skipped tests 通过。Windows personal/friend profile 保持 unsigned；下一步 `BE-001` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
