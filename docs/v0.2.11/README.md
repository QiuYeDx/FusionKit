...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（3个进行中、20个未开始）。`BE-002`已支持最多100文件的CPU/transcribe/no-VAD、custom/source、index-only、export-only批次与SRT/LRC保序多格式。`FS-TXN-001A`～`FS-TXN-001G` component checkpoints已完成：两平台当前冻结protocol v4/journal v3、schema-v2 path-free durable preclaim/decision、terminal marker + acknowledgement、composite owner/exact reauthorization recovery、Windows lossless volume/FileId与cross-platform exact identity；001G新增独立versioned addon staging、content-addressed final-byte publication、generation-bound load proof与正式builder consumption gate。001G staging/builder 21/21、local-subtitle 922 pass + 2 skip、全量1875 pass + 2 skip及真实macOS build/sign/stage/fresh-load通过；runtime Node仍有1个既有fabricated Windows `where.exe` fixture失败，未写成全绿。production main/repository/recovery-owner injection、reauthorization IPC/UI、真实Windows protocol v4、完整official server/media artifact assembly、launch/no-PATH与两平台packaged validation仍未完成，双重`index-only` gate不变；`FS-TXN-001`、`BE-002`与`NATIVE-002`均进行中 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
