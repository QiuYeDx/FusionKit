...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（2个进行中、21个未开始）。`BE-002`已支持最多100文件的CPU/transcribe/no-VAD、custom/source、index-only、export-only批次与SRT/LRC保序多格式。`FS-TXN-001A`～`FS-TXN-001D` component checkpoints已全部完成：macOS arm64闭环protocol v3/journal v2、exact transaction ID、composite recovery owner、path-free repository与exact reauthorization recovery；Windows x64闭环HANDLE/RootDirectory-relative operations、reparse/no-follow、lossless volume/FileId、existing/absent terminal与fresh-process recovery。Windows native 6/6 real compile/load，production 4 terminal + 5 recovery + 6 rejection，fresh-process 3 begin + 14 rollback crash + 14 rollback retry + 5 finalize retry + 2 finalize-crash boundary；identity/loader focused 3 files / 147 passed，TypeScript通过。prepared handoff非durable，production main未注入，双重`index-only` gate不变。下一步可在Mac继续durable decision、Registry/authorization Windows identity composition、staging/builder/main/IPC/UI及packaged validation | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
