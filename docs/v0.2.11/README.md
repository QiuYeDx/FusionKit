...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-004` 已完成；macOS arm64 最终 30 s 窗口/5 s overlap、VAD mapped-segment timeline、raw gate/受控重试在 3 样本 Metal/CPU 全通过，最长连续重复最多 2 cue、时间轴错误 0；下一步 `PRE-005` bundled FFmpeg/runtime staging。Developer ID/Gatekeeper 归未来 `QA-004` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
