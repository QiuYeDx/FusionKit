...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`、`NATIVE-002A1`与`NATIVE-002B` component checkpoints已完成：真实Windows protocol-v4矩阵、macOS CPU/Metal canonical runtime target smoke、Windows CPU/media/addon canonical runtime与独立CUDA 12.4 candidate CPU/CUDA target smoke均已闭环。Job Manager / Production Executor双重`index-only` gate不变；下一步为macOS arm64 / Windows x64 packaged validation，CUDA delivery与分发许可仍归`MODEL-002`/`QA-005` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
