...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001I`、`NATIVE-002A`与`NATIVE-002A1` component checkpoints已完成；002A完成macOS arm64 exact-source official server、bundled media与protocol-v4 addon的canonical assembly，beforePack static双门禁、CPU production-model private health及media decode/no-PATH通过。server final `b5227f8b...23b65e`、runtime manifest `19271b1f...22530`、addon generation `cce55b2e...0530d5`；002A1以同一manifest/production模型完成unrestricted hardened Metal private health/media decode/no-PATH与backend verification，CPU hardened smoke也通过。Windows与packaged矩阵未做。Job Manager / Production Executor双重`index-only` gate不变；下一步闭环真实Windows protocol v4、Windows CPU/CUDA + media assembly/launch/no-PATH及两平台packaged validation | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
