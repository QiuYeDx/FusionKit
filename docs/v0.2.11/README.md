...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006`、`CORE-001`～`CORE-004`、`NATIVE-001`、`BE-001`、`MEDIA-001`、`SUB-001` 已完成；private server lifecycle、media normalization/PCM proof 与 segment-only canonical post-processing contract 已冻结。MEDIA-001 覆盖 bundled FFmpeg/ffprobe attestation、source snapshot/stream binding、decode guard、exact frame window/SHA proof 与 owner cleanup；聚焦 4 files / 84 tests、全量 121 passed + 2 skipped files / 1312 passed + 2 skipped tests、TypeScript、三段 Vite test build 与 manifest/validator gate 通过。下一步可推进 `NATIVE-002` / `SUB-002` / `MODEL-001` / `LINK-001`；`BE-002` 仍等待 SUB-002/MODEL-001 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
