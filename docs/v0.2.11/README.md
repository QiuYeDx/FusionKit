...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（3个进行中、20个未开始）。`BE-002`已支持最多100文件的CPU/transcribe/no-VAD、custom/source、index-only、export-only批次与SRT/LRC保序多格式。`FS-TXN-001A`～`FS-TXN-001H` component checkpoints已完成：两平台当前冻结protocol v4/journal v3、schema-v2 path-free durable preclaim/decision、terminal marker + acknowledgement、composite owner/exact reauthorization recovery、Windows lossless volume/FileId与cross-platform exact identity；001G冻结versioned addon staging/load与builder gate，001H完成canonical proof-only production bootstrap、`overwrite-recovery.v2.json` repository及同一runtime/owner向Exporter与SessionLifecycle的成对注入。001H focused 313/313、local-subtitle 931 pass + 2 skip、全量1884 pass + 2 skip、overwrite-native 29 pass + 1 skip、manifest 0/0、validator 17/17、TypeScript、三段Vite与diff通过。001I reauthorization IPC/UI、真实Windows protocol v4、完整official server/media artifact assembly、launch/no-PATH与两平台packaged validation仍未完成；双重`index-only` gate不变，001I必须先关闭新reauthorization admission再等待已有recovery tails；`FS-TXN-001`、`BE-002`与`NATIVE-002`均进行中 | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
