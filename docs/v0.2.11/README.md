...
| `local-subtitle-transcriber` | `PRE-001`～`PRE-006`、`CORE-001`～`CORE-003` 已完成；domain/runtime schema、resource staging 与独立 preload/IPC/capability contract 已冻结，全量 103 files / 975 tests 通过。Windows personal/friend profile 保持 unsigned；下一步优先 `CORE-004` 或 `NATIVE-001` | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
