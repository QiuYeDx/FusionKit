...
| `local-subtitle-transcriber` | 39个顶层工作包中16个已完成、23个剩余（`FS-TXN-001`、`BE-002`、`NATIVE-002`三个进行中，`FE-001`等20个未开始）。`FS-TXN-001A`～`FS-TXN-001J`、`NATIVE-002A`与`NATIVE-002A1` component checkpoints已完成；001J真实Windows x64 protocol-v4 build/load 7/7且0 skip，production terminal/recovery/rejection与fresh-process crash/retry/conflict/acknowledgement矩阵全部通过，双artifact仍为ignored developer evidence。002A/002A1完成macOS arm64 official server/media/addon canonical assembly、CPU/Metal production-model private health、media decode/no-PATH与backend verification。Job Manager / Production Executor双重`index-only` gate不变；下一步闭环Windows CPU/CUDA + media assembly/launch/no-PATH及两平台packaged validation | `docs/v0.2.11/local-subtitle-transcriber/local-subtitle-transcriber_execution_plan.md` |
