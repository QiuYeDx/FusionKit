...
导出选项为格式、轨道、source/target/bilingual、双语顺序、折行、编码/换行、文件名。选项不进入翻译身份；导出固定文档 revision 快照，不能混入导出中途的新结果。
...
- 原格式 source 导出可使用未修改原始内容，保留编码、BOM、换行与不连续编号；跨格式默认 UTF-8，精度量化列入诊断。
