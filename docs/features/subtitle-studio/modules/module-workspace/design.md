...
- 配置折叠与操作确认：单份/批量翻译“高级设置”、导出“编码与换行”复用字幕 AI 翻译“定时开始”的 `ToolConfigDisclosure`，共用整行触发、图标、展开动画和收起时的 inert，字段保持挂载。父正文12px内边距与组件负边距配对，仅最后一项消除重复底部留白。批量删除/恢复确认用 `StudioSelectedDocuments` 的静态清单直接展示范围，560px弹窗、12px正文与右对齐固定操作栏，保留取消初始焦点及关闭回焦。
- 计划概览与滚动反馈：翻译计划复用 `ToolStatBar`，就绪状态和三项用量集中呈现，长标签可换行、等宽数字底部对齐；单份估算沿用同一组件。翻译/导出计划分隔线与配置折叠区通栏对齐，紧邻时消除重复分组间距。两种工具的计划/结果清单共用 `StudioBatchItems`，在固定外壳中显示20px上下渐变，顶部/底部按实际滚动位置显隐，不溢出时均隐藏；同时观察viewport和列表内容的尺寸变化，保留滚动条、文件名焦点和Tooltip，不改业务行及逐项状态。
