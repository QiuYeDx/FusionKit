...
2026-09-18 后续交互调整：首次使用说明改为项目共用的 Tour，不再放置于右侧内容区；当前交互与验证见本文末尾“Tour 与工具目录间距调整”。
...
| 问题 | 现有证据与影响 | 本轮决定 |
| --- | --- | --- |
| 管理与执行关系不明 | 资料页主要介绍备份/分享，工作台管理链接未解释使用闭环 | 用 Tour 说明保存不等于启用，并引导创建、采纳、到工作台选用；保留常规操作入口 |
...
- 资料页：首次使用通过锚定真实控件的 Tour 介绍流程，标题帮助按钮可重新打开；关闭引导后右侧直接显示工作区。新建条目和前往工作台保留常规入口。新建术语优先显示原文、译文、语言与采纳；译法/匹配和标题/来源为折叠的可选设置。保留资料/方案/审核视图和既有数据。
...
2026-09-18 后续反馈指出：实验性工具上方留白明显大于经典工具，资料页右侧常驻说明占用工作空间且与其他工具的引导方式不一致。本次按以下方式调整。

- 工具目录：实验性分组原先叠加 `mt-10` 与 `pt-6`，总留白 64px；改为与经典分组一致的 `mt-8`，统一为 32px。
- 资料页：删除 `UsageGuide` 常驻说明，复用项目 `Tour` 组件与现有按钮样式。五步依次指向新建资料集、资料、待审核、方案与风格、前往字幕工作台，结合术语示例说明操作与使用边界。方案明确为可选；保存、采纳与本次选用分别解释。
- 首次引导：等待资料加载成功、全局 loading 退出且没有业务弹窗后自动展示。跳过或完成后记住状态；标题问号可从第一步重新打开。引导不切换当前视图、不写入资料、不在完成时自动跳转。
- 常规操作：右侧直接展示视图和条目列表；「新建条目」保留无资料集时先创建再续接录入的流程；前往字幕工作台放在左侧独立入口。经典字幕 AI 翻译器继续完全独立于翻译资料。
- 四语言：同步简中、英文、日文、繁中说明与引导操作文案。Popover 宽度上限 360px，使用共用组件处理滚动定位、视口边界及关闭行为。

### 本次验证

类型检查、四语言完整性与源码 key 检查、根 Vite 三段构建和 preload 检查均通过。源码检查 2,758 处调用、2,931 个 key；保留 21 条既有同源文案警告。构建仅有既有 chunk 相关警告。未安装或升级依赖，`package.json` 与 `pnpm-lock.yaml` 未变更。
...
**2 个文件、2 条原生场景通过，耗时 73.31 秒**，日志 `/tmp/fusionkit-tour-native.log`。覆盖首次自动展示、跳过记忆、刷新不重开、手动重开及五步完成，并保留资料创建/采纳、工作台明确选用和经典工具隔离的真实流程验证。资料管理场景覆盖原生导入、审核、编辑、导出与恢复。其他执行用例仅设置已完成引导的测试前置状态，本次未重新运行。

真实 Electron 中检查 1280×860 中文浅色与 820×700 英文深色。逐步测量 spotlight 与实际控件的位置，检查弹层和操作区在视口内，并亲自审阅截图中的定位、换行和遮挡；等待 loading 完全消失后才取证。Tab 锚点定位交互控件，避开 `ClipPathTabs` 的装饰副本。

| 渲染证据（位于 `test-results/translation-knowledge-consumer-ux/`） | 结论 |
| --- | --- |
| `catalog-spacing-zh-light-wide.png`、`catalog-spacing-en-dark-narrow.png` 及同名 geometry JSON | 两个窗口的经典/实验性分组上间距均为 32px，差值为 0 |
| `tour-zh-light-wide-step-1.png` 至 `step-5.png` | 五步高亮与实际入口匹配；术语示例、步骤计数和操作按钮可读 |
| `tour-en-dark-narrow-step-1.png` 至 `step-5.png` | 长英文自然换行；最后一步自动滚动到工作台入口，弹层操作可达 |
| `workspace-empty-zh-light-wide.png`、`workspace-empty-en-dark-narrow.png` | 右侧没有常驻说明卡；列表、新建入口和标题帮助按钮清晰可见 |

本次未启动 Vite 服务；原生测试启动的 Electron、本机 HTTP fixture 与 Vitest 已退出，进程表确认无本次遗留服务。验证范围为 macOS Electron 与本机模型 fixture，不涉及真实模型质量或 Windows 安装包。
