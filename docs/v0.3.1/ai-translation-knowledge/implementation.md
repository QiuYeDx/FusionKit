...
当前用户体验改造见 [2026-09-18 评审与实施](consumer-ux-review.md)：翻译资料仅用于字幕工作台，提供任务级显式选用和资料页首次使用闭环；经典字幕 AI 翻译器完全不涉及资料入口、配置、文件转交或执行。底层能力范围见 P1.2e：工作台单文件/批量正式资料翻译、转写后自动使用资料、失败报告与重检、冻结追溯与资料清除保护。校对积累仍未完成。2026-09-18 最终工作台限定版本已完成集成验收；以下保留当时的分期记录与验证事实，其中适配经典工具的旧计划已取消，不作为当前需求或本轮最终证据。
...
| 依赖与工作区 | 未调用 pnpm、未改依赖或 lockfile；上一轮已提交 `cd7539d`，本增量已提交为 `911aac3` 并推送至 origin 同名分支 |
...
## P1.2d：转写后自动使用资料翻译（2026-09-15）

授权：用户要求提交并推送后继续开发。P1.2c 已提交并推送 911aac3，起步工作区干净。本增量沿用 V3 AC-17、spec-driven-ai-coding、项目 pitfall guard 与 UI design。

用户流程：转写页开启自动翻译 → 选择模型/目标语言 → 可选开启资料并在独立弹窗选择方案、资料集、明确源语言、共同全文主题和要求 → 提交时固定资料 → 每份字幕生成后分别检查 → 通过后自动翻译；问题保留原字幕，打开工作台翻译页修正。默认关闭，普通旧偏好和旧自动意图兼容。英文转录模式按实际生成的英文字幕固定源语言 en；共同选择不允许行绑定、人物身份或条件确认。

协议决策：请求只携带选择与已读 generation，主进程在共享资料门内核对并冻结所有候选、依赖、采纳状态及选择。独立私有自动准备快照不伪造尚不存在的 cue/batch，最多 4 MiB；内存捕获总计 32 MiB/32 份。转写队列持有资料引用，文档与 pending intent 原子发布后才释放；失败和取消回收。盘点同时包含内存捕获、磁盘 current/previous 的意图和执行记录、物理模型请求。保留意图快照支持失败后的解释；维护界面明确区分准备资料与执行记录。FK-TK/1 导入导出协议不变。

字幕生成后与启动恢复共用知识准备路径，仅读冻结库，严格检查冲突及完整预算；任何失败禁止降级普通翻译。准入原子保存轨道、执行记录、checkpoint 和意图关联；重启 pending 只生成 needs_configuration，不请求模型；已准入任务按原执行记录恢复。共享门不等待供应商网络，避免初始化/恢复嵌套持门。

UI设计：参考 StudioTranscription 的 320px ToolConfigPanel 及 StudioKnowledgeBatch；配置列只放开关、简短选择摘要和按钮，完整选择用 ScrollableDialog，12px 固定头尾、中部独立滚动，目标语言只保留一个来源。取消弹窗保留旧选择；刷新失效资料可修复；保存偏好只含用户选择，不含资料正文、generation、密钥或临时能力。真实 Electron 验收 1280×860 浅色与 820×700 深色/英文、选择弹窗及知识问题状态，亲自审图并修复后交付。

| 任务 | 负责人 / 写集 | 状态 |
| --- | --- | --- |
| T26 自动准备快照和翻译初始化/交接/恢复 | p1_studio_scout：automatic snapshot、automatic contract、translation-service、相应测试 | 已完成 |
| T27 自动资料选择与兼容偏好 | knowledge_ui：StudioTranscription、新选择弹窗、controller/preferences、相应测试；locale 提案交 root | 已完成 |
| T28 转写捕获生命周期与原生场景 | p1_legacy_scout：task-service/runtime、真实服务测试、受控 runtime 和新原生测试 | 已完成 |
| T29 引用与维护、组合、四语言、审计、验证和文档 | root：capture service、reference contract、repository inventory、maintenance、index、locales、集成测试和本文档 | 已完成 |

必需验收：转写中编辑/归档不改变已固定资料；冲突不发模型请求且原字幕可用；pending 重启及重复交接无普通降级/重复任务；引用从内存到磁盘无空窗，取消/失败释放且清除保守阻止；旧自动偏好兼容；真实 controller→IPC→固定资料→自动交接→HTTP/记录链路。原生场景使用既有受控转写 runtime，保留生产 IPC/持久化/翻译和本地 HTTP；真实 task-service 测试另验证原生租约生命周期，不能将受控 runtime 当作真实语音推理验证。


### P1.2d 最终实现与边界

转写页新增“使用翻译资料”入口。选择弹窗支持方案、资料集、明确源语言、共同全文主题、排除条目、本次要求和内容说明；目标只继承自动翻译配置。取消保留先前选择，资料刷新和版本冲突可重新确认；既有偏好不因新增字段被重置。资料正文、generation 和凭据不写入偏好。自动翻译关闭时仍按原转写流程工作。

固定资料最多 4 MiB；同一媒体批次共享一个深度冻结对象，捕获服务累计最多 32 份/32 MiB，全部取消、失败或文档发布后释放。每份文档与 pending 自动意图同时持久化，再交还对应引用；终态内存任务清除资料与凭据，延迟交接只保留必要的身份和内存密钥。文档一旦发布，转写 owner 离开不撤销其已授权自动翻译。原语音执行器、共享模型资源和原生能力没有改动。

私有自动准备协议保存完整候选闭包、采用状态、共同选择及摘要，不含虚构 cue/batch；策略固定为 automatic-knowledge-preparation/1;fktk-execution/1;fk-tk-match/1-unicode-16.0.0;context-term-rule。未知准备/执行/匹配策略拒绝继续，禁止隐式升级后重新解释旧资料。请求在主进程核对资料目标语言与翻译目标一致（zh 归一为 zh-Hans），英文转录输出必须使用 en 源语言。普通模式仍由用户明确指定生成字幕的语言，不推断说话人或条件事实。

生成字幕后用冻结资料调用原有全文准备器，沿用 2000 非空 cue、完整预算、正式执行记录及恢复校验。初始化和交接共用同一路径；pending 重启只物化需要配置的任务，不调用模型。检查冲突、损坏或容量不足创建可见失败任务并保留字幕，不能通过普通恢复绕过资料检查。已准入任务保留精确请求字节和模型配置。自动队列在交接即发现阻塞时显示“自动翻译需处理”，提供查看字幕入口；失败文档保留具体错误，不再显示“旧任务”提示或虚假的“已有 AI 译文”。

资料维护的引用类型增加 automatic_preparation，队列捕获使用独立 preparationId，磁盘意图使用 documentId 与 intentId；不伪造尚不存在的执行记录身份。当前/上一代意图、所有执行记录及活动物理请求都进入保守盘点，未知或损坏意图阻止清除。启动准备仅从已盘点保护的意图转换为记录，无新增库引用，不额外持门；普通交接准备在门外，最终写入/启动在共享门内。门不等待供应商网络。维护界面显示“关联任务资料”和“自动翻译准备资料”，说明归档不改变固定内容。

失败自动意图仍保留私有资料快照；当前处理路径是打开字幕后重新选择资料并检查，尚未提供独立的自动准备快照详情/清理界面。历史意图或记录继续保留资料时，永久清除可能被阻止，归档仍可用。独立翻译器适配、表达/参考译文检索和积累、真实供应商质量对照、长片性能及 Windows 原生验收继续单独跟踪；本轮仅用本地 fixture 验证应用行为。FK-TK/1、便携校验器和外部 Agent Skill 完全兼容，没有云端依赖。

### 2026-09-15 P1.2d 验证结果

| 检查 | 结果 |
| --- | --- |
| 相关模块集成回归 | 38 文件：33 通过、5 个显式原生测试文件默认跳过；656 项通过、7 项平台/原生场景默认跳过，23.50 秒。命令与结果见本轮日志 /tmp/fusionkit-p12d-regression.log |
| 最终构建原生回归 | FUSIONKIT_KNOWLEDGE_E2E=1 FUSIONKIT_STUDIO_I5_TRANSCRIPTION_UI=1，automatic-electron、formal-electron、batch-electron、experience-transcription-ui 共 4 场景通过，66.61 秒；结果见 /tmp/fusionkit-p12d-native-delivery.log |
| 转写真实任务服务 | 35 项通过，包含 9 项新增知识场景；双文件资料移交、拒绝/取消/关闭清理、owner 退出与延迟 handoff、英文源语言校验、知识失败公开状态 |
| 冻结与清除 | 新自动准备快照 19 项、跨领域自动服务 12 项、捕获/磁盘移交 5 项；未知策略/损坏意图在初始化前后保持 unknown，真实清除被阻止；维护测试区分多个尚无文档的捕获 |
| 控制器与偏好 | 37 项通过，新增 7 项；旧偏好兼容、无库和失效引用不降级、提交快照与 generation 固定、凭据不进偏好、英文输出、过期后修复 |
| TypeScript / 构建 / preload | 最终 TypeScript、Vite renderer/main/preload 三段构建、sandboxed preload 外部模块检查通过；保留既有大 bundle/混合导入提示 |
| 四语言与协议 | 新文案四语言齐全，源码动态键改为有限映射；全部 source keys 可解析，3 个 FK-TK/1 便携产物一致 |
| 边界与来源 | 工作台独立边界 488 文件、0 错误；120 冻结副本及执行器 1 文件/26 依赖通过。只核对更新 Studio index/runtime 两项当前组合摘要；保留原始冻结来源与 SHA。新增跨领域维护测试放在 knowledge 测试目录，未将整个维护服务加入 Studio 依赖许可 |

原生复验使用 macOS、Node 20.19.5、Electron 41.10.6、Vitest 2.1.9，隔离临时 profile 与本地 HTTP。测试边界明确：整个语音 runtime 被受控夹具替换，但生产 main/IPC/preload/原生 File 捕获、资料捕获、文档保存、自动协调及 HTTP 翻译保持真实；真实 task-service 的租约语义由独立测试覆盖。没有真实语音推理、付费模型调用或模型质量结论。

首轮原生夹具试图批量采纳强制术语，被正式“逐条审核”规则拒绝；修为逐条采纳并使用每次最新 generation，未放宽产品规则。后续检查原有自动翻译场景发现原测试 setSize(787) 却只接受 innerWidth=786；调整为 macOS 原生奇数尺寸允许的 786/787 两值，保留高度、无水平溢出与完整可达断言。

主任务实际查看了浅色选择、820×700 深色英文选择、冲突队列、失败文档及转写期间的维护引用。独立 UI 审阅发现“旧任务”及“AI 译文，未复核”两处误导提示，已经修复并加入真实渲染断言。选择弹窗按按钮焦点自动滚动后截图裁到标签，最终截图显式回到顶部；功能滚动保持原样。维护条目展示准备资料身份、引用修订及说明，不展示虚构执行 ID。

本增量沿项目避坑流程记录 FK-PIT-0159。开始开发前的 P1.2c 已按要求提交并推送 911aac3；P1.2d 后续按用户要求提交并推送为 `84305d8`。未运行 pnpm，未安装依赖或改动锁文件。

最终图像和构建边界证据：test-results/studio-t06-ui/controlled-gqODQC/ 保存自动资料的浅色选择、英文窄窗、待处理队列/文档、转写期间维护引用与 actual HTTP/快照对照 JSON；controlled-68PuTp/ 保存原有自动流程的偏好恢复、真实拖入、取消和窄窗证据。单文件/批量回归截图分别在 test-results/translation-knowledge-formal/ 与 translation-knowledge-batch/。主任务已审阅最终选择标签、仅原文/明确错误及维护布局，未遗留本轮可处理的交互或视觉缺陷。

最终清理：所有本轮 Electron/Vitest/本地 HTTP fixture 均已退出；进程表仅剩用户另一项目原有 Vite，予以保留。最终类型、四语言、preload 与 git diff --check 通过，依赖与锁文件无变化。


## P1.2e：自动资料检查失败后的解释与重检（2026-09-16）

授权：用户要求“提交代码并推送，然后继续做必要的适当的完善工作”。已提交并推送 P1.2d 为 `84305d8`，起步工作区干净。沿用 V3 §4.3、§6.5、AC-16/17，保持 FK-TK/1、无云端和历史执行不可重写的边界。

本轮流程：自动翻译资料检查失败 → 查看当时发现的问题、资料条目和字幕定位 → 调整资料并重检 → 带入原有共同选择与翻译配置，读取当前资料库 → 用户检查并明确开始新翻译。原任务保留，不复制说话人绑定或条件确认；旧报告缺失、资料损坏或原文已变化均明确显示。

数据设计：只在有资料的自动准入失败时与任务原子保存有界检查报告，私有字段外层容忍不可读报告，独立严格验证新增写入与读取内容。报告保存固定问题代码、引用及当时的有界名称/原文摘录，不存供应商自由文本；最多100条问题、每条20个条目/字幕引用并声明截断。只读接口按文档能力授权，在初始化之前处理，不触发翻译准备、网络或历史改写。展示名称来自已验证冻结资料；原文变化标过期，不将旧序号冒充当前定位。常规文档分页仅增加报告轨道 ID 提示，不返回私有资料或报告正文；清理任务后，保留的轨道仍可查看报告。

UI设计：参考 StudioTranslationTask、StudioExecutionRecord 和现有全文资料表单；任务区保留紧凑操作，详情使用 ScrollableDialog，12px头尾留白、中部滚动，错误优先排列，长名称/原文自然换行。一次性恢复请求初始化原配置和共同选择，保留用户后续修改；失效资源提供可见移除入口，不静默改用普通翻译。复用现有全文表单，不叠加第二个资料计划owner。验收真实Electron浅色1280×860与深色英文820×700，亲自审阅截图并修复可处理问题。

| 任务 | 负责人 / 写集 | 状态 |
| --- | --- | --- |
| T30 有界历史报告与只读投影 | p1_studio_scout：新report contract/helper，automatic contract、translation-service、repository、必要persistence及相关单测 | 已完成 |
| T31 失败详情与资料重检表单 | knowledge_ui：StudioAutomaticKnowledgeFailure、Studio index、Translation/Status、KnowledgeTrial及UI helper | 已完成 |
| T32 原生用户场景 | p1_legacy_scout：automatic-electron测试与专属helper | 已完成 |
| T33 集成、四语言、只读IPC、安全边界与验证 | root：IPC contract、preload、main index、locales、IPC回归、当前组合审计与本文档 | 已完成 |

必需验收：冲突失败报告可解释且不请求模型；查看不修改revision/重新准备；报告损坏不回退当前文档，新增非法报告写入被拒绝；无报告旧任务兼容；身份越权和过期引用可控；重检恢复共同选择及配置但不恢复逐句确认，读取当前资料后新计划方可开始；关闭、换文档与迟到响应不覆盖用户选择；新任务成功且旧报告/冻结资料保留。固定模型fixture只证明流程，不作为真实翻译质量证据。


### P1.2e 最终实现与边界

失败轨道新增“查看资料问题”。报告按错误优先显示当时发现的问题、相关资料名称、原文片段及原序号，并标明检查时间、已发现数量和截断范围；不声称已扫描所有字幕。常规分页只返回关联轨道 ID，点击后通过专用只读接口验证完整性与身份，不触发初始化、当前资料读取或模型请求。已清理任务但仍保留轨道的报告可继续查看；删除轨道后拒绝读取。

报告最多100条问题，每条20个资料/字幕引用，名称160字符、原文片段240字符，总UTF-8大小不超过256KiB。仅在自动准入失败时与任务同步保存，后续不可改写。原文、时间、翻译配置变化按独立摘要标记过期；其他译轨的进度不会误判原文已变化。原文修改后仍显示旧摘录，不能当成当前定位。损坏报告在当前文档中保留为不可读，不触发上一代回退。诊断生成失败先尝试通用报告，仍失败则保留无报告的失败任务；资料或配置不符合严格JSON规范时不给可复用选择。

“调整资料并重检”一次性带入原翻译配置、共同资料选择、排除项与全文主题，读取当前资料库并打开全文表单。恢复固定为全文模式；不继承逐句说话人、提及对象或条件确认。用户可调整并检查，检查通过后明确开始新任务。恢复期间不显示普通翻译准入按钮。原模型档案丢失或配置变化时仍可查看/调整资料，开始前须明确选用当前模型；完整选择超过256KiB时不提供部分复用，避免静默改变范围。缺失、归档和不在范围内的选择保留可见移除项，当前自定义语言标签保留。

表单读库失败、关闭、文档切换和迟到响应使用身份/版本栅栏；同文档的修订或父配置变化使计划失效，但保留用户已改共同选择。未编辑的翻译要求继续继承父配置，显式空字符串仍表示覆盖。新任务响应始终交全局概览跟踪，只有对应当前文档时更新局部界面。历史意图与旧失败报告不因重检成功而改写。外部知识文件、FK-TK/1产物与Skill无变化，无云端依赖。

本轮独立审阅复现了未配对UTF-16代理项在普通字符串schema可读、JCS拒绝后让两级报告生成同时抛错的问题，已补最终不阻断失败任务落盘的保护与两个回归样例，记录为 FK-PIT-0160。界面初轮审图发现固定全文重检页仍提示切换片段试译，已改为准确范围说明，并补历史检查时间。


### 2026-09-16 P1.2e 验证结果

集成版本：`84305d8` 之后的 P1.2e 最终实现；以下验证对应提交前的最终代码。P1.2d 已提交推送；P1.2e 按用户后续“提交代码并推送”的要求纳入本次提交。未运行 pnpm，未安装或升级依赖，锁文件无变化。

| 检查 | 实际结果 / 证据 |
| --- | --- |
| 相关模块回归 | 25文件：20通过、5个显式原生文件默认跳过；447项通过、7项平台/原生跳过，15.07秒。包括知识协议/仓库/服务、自动翻译、执行记录、IPC、字幕仓库及翻译恢复。日志 `/tmp/fusionkit-p12e-regression.log` |
| 报告与持久化针对性验证 | automatic-service 30项，包含8种报告损坏、新增非法写入与不可变、源/时间/配置过期、清理任务仍可读、发布故障、超限裁剪、超大选择不部分复用、异常Unicode不阻断失败落盘。针对性组合91项通过，已包含在上述回归中 |
| 只读IPC与能力边界 | 32通过、2项平台跳过；固定preload方法、禁止通用channel、文档owner/子frame/迟到撤销、严格请求；spy确认报告读取不调用翻译初始化或当前资料库，不发模型请求。日志 `/tmp/fusionkit-p12e-ipc.log` |
| 最终原生自动流程 | 1完整场景通过，20.50秒。报告连续读取两次，全部文档文件字节与snapshot不变；原选择恢复后先重检仍冲突，排除其中一条强制术语后重检通过，显式start才新增第2次HTTP请求及新任务；旧失败任务/意图/报告不变。日志 `/tmp/fusionkit-p12e-native-delivery.log` |
| 正式全文翻译原生回归 | 同一最终构建的formal-electron场景通过。自动场景此前仅因已隐藏radio的过期定位及嵌套弹窗退出前发送Escape而失败；分别改为全文标题/检查按钮断言、等待子弹窗退出并点击父弹窗取消，未降低产品条件。日志 `/tmp/fusionkit-p12e-native-final.log`与上述最终自动日志 |
| 类型、构建与四语言 | 最终TypeScript、Vite renderer/main/preload构建、sandboxed preload检查通过；四语言齐全，2672处调用、2866个源码键可解析。构建仅有既有bundle体积/混合导入提示。日志 `/tmp/fusionkit-p12e-build.log`、`/tmp/fusionkit-p12e-i18n.log` |
| 隔离边界、来源与文件协议 | 工作台依赖492文件0错误，冻结副本120文件与执行器1文件/26依赖校验通过，3个FK-TK/1产物一致；只更新已审阅的Studio main组合摘要，原始来源、冻结副本与外部Agent Skill未更改 |

原生验证在隔离profile、受控语音runtime与本地HTTP模型fixture上运行，保留生产main/IPC/preload、文档仓库、资料捕获/报告与翻译请求。未调用付费模型或真实语音推理，不宣称实际模型翻译质量提升。完整证据在 `test-results/studio-t06-ui/controlled-Ob3urr/`：原始HTTP/报告对照JSON、构建边界清单、中文1280×860的报告/恢复/检查通过截图、英文深色820×700的历史报告/恢复表单截图。主任务亲自审阅最终报告时间与错误优先内容、排除项/检查摘要、窄窗长文案和固定底栏，正文滚动正常、按钮可达，无本轮遗留可处理视觉问题。
...
## 翻译资料的工作台体验改造（2026-09-18）

当前设计与验收条件集中在 [consumer-ux-review.md](consumer-ux-review.md)。资料使用范围限定在字幕工作台；经典字幕 AI 翻译器的界面、配置、队列、文件选择与执行均不关联翻译资料。

已实施的工作台与资料页体验：

- 资料页说明“保存、采纳、任务选用”的区别；空库提供三步与对应操作，创建资料集后可直接继续录入术语。术语核心字段与采纳先呈现，低频匹配和来源设置折叠；表达/参考译文当前仅保存，尚未执行的偏好与积累明确说明。
- 工作台单文件与批量翻译增加默认关闭的资料开关、所选集/方案摘要和明确的资料主操作。开启后普通预估/开始在界面和处理函数中同时被阻止；关闭选择弹窗保留本次资料状态，检查失败不回退普通翻译。
- 单文件默认全文，片段试译可选。原文语言明确选择，目标沿用父任务，方案不能静默改目标；源/目标使用执行契约校验。无可用方案或对象时隐藏空的高级区域。
- 模型已配置但语言/预算暂时无效时保留资料草稿并禁用执行；明确填写的要求（包括空字符串）不随父目标修改而丢失。关闭父表单清除旧打开指令，批量读库失败可显式刷新恢复。
- 工作台转写后自动翻译显示资料关闭说明及实际选集名称，沿用原有固定资料版本、冻结执行和失败重检链路。

本次工作台限定版本已通过类型、四语言、根 Vite 三段构建/preload、170 项领域回归及 4 条真实隔离 Electron 场景；完成经典工具与工作台截图审阅、边界/来源/协议检查及进程清理。原生用例确认经典上传留在自身队列、不会新增工作台文档，工作台仅发送显式选用且匹配的资料。完整命令、最新逐文件结果与日志见评审文档，不计入已删除的交接服务测试。未把真实模型翻译质量、Windows 安装包或实际语音推理纳入本轮验收。

项目避坑记录 FK-PIT-0161 已同步为工作台专属范围，保存显式选用、普通路径阻止、目标语言一致、草稿生命周期与经典工具边界的验证要求。
