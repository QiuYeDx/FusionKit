import { SpeechResourceSmokeError, type SpeechResourceModelSmoke } from "../../../src/speech-resources/contracts";
...
import type { LocalSubtitleResourceEventEnvelope } from "@/speech-resources/contracts";
import {
  LOCAL_SUBTITLE_MODEL_MANIFEST,
  LocalSubtitleModelError,
  type LocalSubtitleModelManifestEntry,
} from "../../../electron/main/speech-resources/engine/model-manifest";
import {
  verifyLocalSubtitleGgmlModelFile,
} from "../../../electron/main/speech-resources/engine/ggml-model";
import {
  LocalSubtitleModelManager,
  type LocalSubtitleModelManagerOptions,
} from "../../../electron/main/speech-resources/engine/model-manager";
import {
  downloadLocalSubtitleResource,
  type LocalSubtitleDownloadResponse,
} from "../../../electron/main/speech-resources/engine/resource-download";
...
describe("speech resource engine model manager", () => {
  ...
  it("copies, verifies, load-smokes and atomically commits a managed model", async () => {
    ...
    expect(fixture.smoke).toHaveBeenCalledOnce();
    expect(fixture.smoke.mock.calls[0]?.[0]).toMatchObject({
      model: {
        storage: "managed_staging",
        id: fixture.model.id,
      },
    });
    expect(fixture.smoke.mock.calls[0]?.[0]).not.toHaveProperty("vadModel");
  ...
  it("requires and honors an explicit target for multi-model imports", async () => {
    ...
    const manager = new LocalSubtitleModelManager({
      ...
      smokeModel: fixture.smoke, smokeVad: async () => undefined, vadManager: false, acceleratorManager: false,
  ...
  it("preserves a missing CPU server as a runtime resource failure", async () => {
    const fixture = await createFixture({
      smokeModel: async () => { throw new SpeechResourceSmokeError("runtime_missing", "CPU runtime unavailable"); },
    });
  ...
  it("deep-validates an injected catalog before any filesystem access", async () => {
    ...
    const createManager = (model: LocalSubtitleModelManifestEntry) =>
      new LocalSubtitleModelManager({
        ...
        platform: process.platform, arch: process.arch,
        smokeModel: smoke, smokeVad: async () => undefined, vadManager: false, acceleratorManager: false,
        modelCatalog: [model],
      });
...
interface FixtureOptions {
  ...
  readonly smokeModel?: SpeechResourceModelSmoke;
...
async function createFixture(options: FixtureOptions = {}) {
  ...
  const manager = new LocalSubtitleModelManager({
    managedResourceRoot: managedRoot,
    platform: process.platform, arch: process.arch,
    smokeModel: options.smokeModel ?? smoke, smokeVad: async () => undefined, vadManager: false, acceleratorManager: false,
