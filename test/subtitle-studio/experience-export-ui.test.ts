...
describe.runIf(process.env.FUSIONKIT_STUDIO_I5_EXPORT_UI === '1')('I5 export defaults and shared document lists', () => {
  it('uses real tracks, source folders and fallback with one collapsed scope and scroll-aware lists', async () => {
    ...
    try {
      ...
      await uiExpect(dialog.getByRole('combobox', { name: '文件编码', exact: true })).toHaveCount(0);
      await uiExpect(dialog.getByRole('combobox', { name: '换行方式', exact: true })).toHaveCount(0);
      ...
      await uiExpect(translationReview.getByRole('button', { name: '查看文件详情', exact: true })).toHaveCount(0);
      await uiExpect(translationReview.getByTestId('studio-batch-plan').locator('.studio-document-row')).toHaveCount(0);
      await capture('11-translation-plan-summary-dark', translationReview);
