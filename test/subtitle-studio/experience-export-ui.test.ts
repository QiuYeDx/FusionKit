...
describe.runIf(process.env.FUSIONKIT_STUDIO_I5_EXPORT_UI === '1')('I5 export defaults and shared document lists', () => {
  it('uses real tracks, source folders and fallback with one collapsed scope and scroll-aware lists', async () => {
    ...
    try {
      ...
      await page.getByTestId('studio-library-result').locator('[data-result-group="success"] > summary').click();
      await uiExpect(page.getByTestId('studio-library-result').locator('.studio-document-row')).toHaveCount(8);
      ...
      await uiExpect(page.getByTestId('studio-export-result')).toContainText('成功 1 份');
      await dialog.getByRole('button', { name: '关闭', exact: true }).click(); await uiExpect(page.getByRole('dialog')).toHaveCount(0);
      ...
      await uiExpect(page.getByTestId('studio-batch-result')).toContainText('成功 8 份');
