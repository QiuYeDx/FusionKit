...
describe('document snapshot/event reconciliation', () => {
  it('retains off-page observations and tombstones across snapshots; rejects old pages and events', () => {
    ...
    expect(state.acceptSnapshot({ documents: [document(b, 1)], total: 2, sequence: 4 })).toBe(false);
    expect(state.acceptSnapshot({ documents: [document(b, 1)], total: 2, sequence: 5 })).toBe(true);
    expect(state.acceptSnapshot({ documents: [document(b, 3)], total: 2, sequence: 5 })).toBe(true);
    ...
    expect(state.acceptSnapshot({ documents: [document(b, 3)], total: 1, sequence: 6 })).toBe(true);
    ...
    expect(state.acceptSnapshot({ documents: [document(a, 4)], total: 2, sequence: 6 })).toBe(false);
    expect(state.acceptSnapshot({ documents: [], total: 0, sequence: 5 })).toBe(false);
  });
});
