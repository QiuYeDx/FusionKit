...
} from "../../../electron/main/subtitle-studio/transcription/native/resource-manifest";
import {
  resolveLocalSubtitleRuntimeRoot,
  type LocalSubtitleResourceEnvironment,
} from "../../../electron/main/subtitle-studio/transcription/native/resource-path";
...
export function createRuntimeManifest(
  ...
  return {
    ...
    manifestId: `subtitle-studio-runtime-${target.id}-fixture`,
...
export async function createRuntimeFixture(options: {
  ...
  const tempRoot = await mkdtemp(
    path.join(os.tmpdir(), "fusionkit-subtitle-studio-runtime-"),
  );
