...
} from "../../../src/subtitle-studio/transcription/ipc-contract";
...
describe("local subtitle fixed IPC surface", () => {
  it("keeps public, preload-internal, and event channels exact and disjoint", () => {
    ...
    expect(allChannels.every((channel) => channel.startsWith("subtitle-studio:transcription:")))
      .toBe(true);
...
describe("local subtitle module ownership", () => {
  it("does not import or extend the remote audio contracts", () => {
    for (const file of ["domain.ts", "ipc-contract.ts"]) {
      const source = readFileSync(
        path.join(process.cwd(), "src", "subtitle-studio", "transcription", file),
        "utf8",
      );
