  ...
  it('keeps legacy preferences and entry points while localizing the workspace, export, and recovery', async () => {
    ...
    for (const language of languages) {
      ...
      const openExport = async () => {
        await page.getByRole('button', { name: labels.batch.download_single, exact: true }).click();
        await page.getByTestId('studio-download-menu').getByRole('menuitem', { name: labels.export.action, exact: true }).click();
        const exporting = page.getByRole('dialog');
        await uiExpect(exporting).toHaveAccessibleName(labels.export.action);
        return exporting;
      };
      const emptyExport = await openExport();
      ...
      await uiExpect(page.locator('.studio-cue-table tbody tr')).toHaveCount(100);
      const readerPagination = page.locator('.studio-reader-footer .studio-pagination');
      const jump = readerPagination.getByRole('spinbutton', { name: labels.page_number, exact: true });
      await jump.fill('2'); await jump.press('Enter');
      await uiExpect(page.locator('.studio-cue-number').first()).toHaveText('101');
      await uiExpect(readerPagination.getByRole('button', { name: labels.next, exact: true })).toBeDisabled();
      ...
      const exporting = await openExport();
      await exporting.getByRole('combobox', { name: labels.export.destination, exact: true }).click();
      await page.getByRole('option', { name: labels.export.destination_choose, exact: true }).click();
      ...
      await uiExpect(exporting).toHaveAccessibleName(labels.export.review_title);
      await uiExpect(exporting.locator('[data-issue="missing_end"]')).toBeVisible();
      await exporting.getByRole('button', { name: labels.export.review_back, exact: true }).click();
      await exporting.getByRole('checkbox', { name: labels.export.estimate_end, exact: true }).check();
      await exporting.getByRole('button', { name: labels.export.prepare, exact: true }).click();
      const confirmExport = exporting.getByRole('button', { name: labels.export.review_confirm.replace('{{count}}', '1'), exact: true });
      await uiExpect(confirmExport).toBeEnabled();
      ...
      await uiExpect(confirmExport).toBeEnabled();
      await page.keyboard.press('Escape');
      await uiExpect(page.getByRole('button', { name: labels.batch.download_single, exact: true })).toBeFocused();
