...
async function settle(dialog: Locator) {
  ...
  await uiExpect(dialog.locator('[data-dialog-exiting="true"]')).toHaveCount(0);
...
describe.runIf(process.env.FUSIONKIT_KNOWLEDGE_E2E === '1')('translation material record dialogs in native Electron', () => {
  it('keeps five record types readable and separates editing, review and maintenance actions', async () => {
    ...
    try {
      ...
      await nativeWindow.evaluate(win => win.webContents.setBackgroundThrottling(false));
      let labels = zh;
      // Dialog handoffs retain an inert outgoing tree until its exit finishes.
      // Interactions target the live tree; closure assertions still check every dialog.
      const live = page.locator(':not([data-dialog-exiting="true"], [data-dialog-exiting="true"] *)');
      const byTestId = (id: string) => page.getByTestId(id).and(live);
      ...
      const ready = async () => {
        await uiExpect(byTestId('knowledge-import')).toBeEnabled();
      ...
      const allDialogs = () => page.getByRole('dialog', { includeHidden: true });
      const dialog = () => allDialogs().and(live);
      const closeRecord = async () => {
        await byTestId('knowledge-record-close').click();
      };
      ...
      const capture = async (name: string, record = true) => {
        ...
        const geometry = await dialog().evaluate(element => {
          ...
          return {
            ...
            disclosureGroups: [...element.querySelectorAll('.knowledge-editor-disclosures, .knowledge-entry-secondary')].map(group => {
              const bounds = group.getBoundingClientRect();
              return { radius: Number.parseFloat(getComputedStyle(group).borderTopLeftRadius), rows: [...group.querySelectorAll(':scope > [data-slot="knowledge-disclosure"]')].map(row => {
                const trigger = row.querySelector('[data-slot="accordion-trigger"]')!;
                const box = trigger.getBoundingClientRect(), style = getComputedStyle(trigger);
        ...
        for (const group of geometry.disclosureGroups) {
          expect(group.radius, name).toBeGreaterThanOrEqual(8);
          for (const row of group.rows) {
            expect(row.left, name).toBeCloseTo(1, 0); expect(row.right, name).toBeCloseTo(1, 0);
            expect(row.paddingLeft, name).toBeGreaterThanOrEqual(12); expect(row.paddingRight, name).toBeGreaterThanOrEqual(12);
          }
        }
        if (record) {
      ...
      const assertFirstScreen = async (entry: Entry) => {
        await settle(dialog());
        const text = byTestId('knowledge-entry-detail').getByText(primaryText(entry), { exact: true }).first();
      ...
      const openTerm = async () => {
        await byTestId('knowledge-all').click();
        await byTestId('knowledge-views').getByRole('tab', { name: labels.workspace.terms, exact: true }).click();
        await byTestId(`knowledge-entry-details-${term.id}`).click();
        await uiExpect(byTestId('knowledge-entry-detail')).toBeVisible();
      };
      const openActions = async () => {
        await byTestId('knowledge-entry-actions').click();
      ...
      await byTestId('knowledge-import').click();
      ...
      for (const locale of ['zh', 'en'] as const) {
        ...
        await byTestId('knowledge-review').click();
        for (const entry of fixture.entries) {
          await byTestId(`knowledge-entry-details-${entry.id}`).click();
          await uiExpect(byTestId('knowledge-entry-adopt')).toBeVisible();
          await assertFirstScreen(entry);
          await capture(`detail-${entry.kind}-${mode}`);
          if (entry.kind === 'term') {
            await uiExpect(byTestId('knowledge-entry-scope')).toBeVisible();
            for (const section of ['sources', 'all-fields']) {
              const trigger = byTestId(`knowledge-entry-${section}`).locator('[data-slot="accordion-trigger"]');
              await uiExpect(trigger).toHaveAttribute('aria-expanded', 'false');
              await trigger.click();
              await uiExpect(trigger).toHaveAttribute('aria-expanded', 'true');
              await trigger.click();
            }
            await byTestId('knowledge-entry-sources').locator('[data-slot="accordion-trigger"]').click();
            await settle(dialog());
            const footer = byTestId('knowledge-record-primary-actions').locator('..');
          ...
          await byTestId('knowledge-entry-edit').click();
          await uiExpect(byTestId('knowledge-entry-editor')).toBeVisible();
          await capture(`editor-${entry.kind}-${mode}`);
          if (entry.kind === 'term') {
            const tabs = dialog().getByRole('tablist', { name: labels.record.settings });
            const panel = byTestId('knowledge-entry-wording');
            ...
            for (const value of ['language', 'scope', 'metadata', 'wording'] as const) {
              ...
              const activePanel = byTestId(`knowledge-entry-${value}`);
              ...
              if (value === 'metadata') {
                ...
                const footer = byTestId('knowledge-record-primary-actions');
                await settle(dialog());
                const before = await footer.boundingBox();
      ...
      await uiExpect(page.getByRole('menu')).toHaveCount(0);
      await uiExpect(byTestId('knowledge-entry-detail')).toBeVisible();
      await uiExpect(byTestId('knowledge-entry-actions')).toBeFocused();
      expect(new URL(page.url()).hash).toBe('#/tools/translation-knowledge');
      await byTestId('knowledge-entry-adopt').click();
      await uiExpect(allDialogs()).toHaveCount(0);
      ...
      await openTerm();
      await uiExpect(byTestId('knowledge-entry-adopt')).toHaveCount(0);
      ...
      await byTestId('knowledge-save-draft').click();
      const saveError = byTestId('knowledge-entry-editor').getByRole('alert');
      ...
      await byTestId('knowledge-save-apply').click();
      await uiExpect(byTestId('knowledge-entry-detail')).toBeVisible();
      ...
      await openActions();
      await byTestId('knowledge-entry-copy').click();
      await uiExpect(byTestId('knowledge-entry-editor')).toBeVisible();
      await closeRecord();
      ...
      await byTestId('knowledge-review').click();
      await byTestId(`knowledge-entry-details-${context.id}`).click();
      await openActions();
      await byTestId('knowledge-entry-reject').click();
      await expectWorkspaceInteractive();
      ...
      await openTerm();
      await openActions();
      await byTestId('knowledge-entry-maintenance').click();
      await uiExpect(byTestId('knowledge-maintenance-confirm')).toBeEnabled();
      const beforeArchive = await read();
      await capture('entry-archive-preview-en-dark-narrow', false);
      await closePreview();
      await expectWorkspaceInteractive();
      await uiExpect(byTestId(`knowledge-entry-details-${term.id}`)).toBeFocused();
      expect(await read()).toEqual(beforeArchive);
      await openTerm();
      await openActions();
      await byTestId('knowledge-entry-maintenance').click();
      await byTestId('knowledge-maintenance-confirm').click();
      await expectWorkspaceInteractive();
      expect((await read()).data.entries.find(entry => entry.id === term.id)?.state).toBe('archived');
      await byTestId('knowledge-archive').click();
      await byTestId(`knowledge-entry-details-${term.id}`).click();
      await uiExpect(byTestId('knowledge-entry-adopt')).toHaveCount(0);
      await capture('detail-term-archived-en-dark-narrow');
      await openActions();
      await uiExpect(byTestId('knowledge-entry-purge')).toHaveAttribute('role', 'menuitem');
      await page.keyboard.press('Escape');
      await byTestId('knowledge-entry-maintenance').click();
      ...
      for (const locale of ['zh', 'en'] as const) {
        ...
        await byTestId('knowledge-edit-collection').click();
        await uiExpect(byTestId('knowledge-catalog-editor')).toBeVisible();
        await capture(`catalog-collection-${mode}`);
        await dialog().locator('.knowledge-editor-disclosures [data-slot="accordion-trigger"]').first().click();
        await capture(`catalog-collection-options-${mode}`);
        await byTestId('knowledge-catalog-more').click();
        await uiExpect(byTestId('knowledge-catalog-maintenance')).toHaveAttribute('role', 'menuitem');
        await uiExpect(byTestId('knowledge-catalog-purge')).toHaveAttribute('role', 'menuitem');
        await page.keyboard.press('Escape');
        await uiExpect(page.getByRole('menu')).toHaveCount(0);
        await uiExpect(byTestId('knowledge-catalog-editor')).toBeVisible();
        await closeRecord();
        ...
        const disclosure = dialog().locator('.knowledge-editor-disclosures [data-slot="accordion-trigger"]').first();
        await disclosure.click();
        await dialog().locator('.knowledge-editor-disclosures').evaluate(element => element.scrollIntoView({ block: 'start' }));
        await capture(`catalog-plan-options-${mode}`);
      ...
      // Preserve the live geometry before finally closes the native window.
      if (failurePage) {
        try {
          const geometry = await failurePage.evaluate(() => {
            const rect = (element: Element) => {
              const value = element.getBoundingClientRect();
              return { x: value.x, y: value.y, width: value.width, height: value.height };
            };
            return [...document.querySelectorAll('[role="dialog"]')].map(dialog => ({
              exiting: dialog.getAttribute('data-dialog-exiting'), rect: rect(dialog),
              viewports: [...dialog.querySelectorAll('[data-slot="scroll-area-viewport"]')].map(viewport => ({
                rect: rect(viewport), scrollTop: viewport.scrollTop, scrollHeight: viewport.scrollHeight, clientHeight: viewport.clientHeight,
              })),
              alerts: [...dialog.querySelectorAll('[role="alert"]')].map(alert => ({
                text: alert.textContent, rect: rect(alert), focused: document.activeElement === alert,
                exiting: Boolean(alert.closest('[data-dialog-exiting="true"]')),
                flow: alert.closest('[data-flow-motion="true"]')?.getAttribute('data-flow-animating'),
              })),
            }));
          });
          await writeFile(path.join(screenshots, 'failure.json'), JSON.stringify({ error: String(failure), geometry }, null, 2));
          await failurePage.screenshot({ path: path.join(screenshots, 'failure.png') });
        } catch { /* Keep the original assertion if the failing window is already closed. */ }
      }
      throw failure;
    } finally {
