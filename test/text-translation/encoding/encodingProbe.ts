...
export type EncodingProbeResult =
  | {
      status: "accepted";
      encoding: SupportedTextEncoding;
      text: string;
...
const SUPPORTED_ENCODINGS: SupportedTextEncoding[] = [
  "utf-8",
  "utf-16le",
  "utf-16be",
  "gb18030",
...
const MIN_DETECTOR_CONFIDENCE = 0.55;
const MIN_DETECTOR_GAP = 0.15;
const MIN_QUALITY_SCORE = 0.82;
const HIGH_DETECTOR_CONFIDENCE = 0.9;

function normalizeEncodingName(
  name: string,
...
function tryDecodeStrictUtf8(buffer: Uint8Array): string | undefined {
  try {
    ...
    return calculateTextQuality(text) >= MIN_QUALITY_SCORE ? text : undefined;
  } catch {
    return undefined;
  }
}
...
function buildDetectorCandidates(
  buffer: Uint8Array,
): EncodingCandidateScore[] {
...
export function probeTextEncoding(buffer: Uint8Array): EncodingProbeResult {
  if (buffer.length === 0) {
    return {
      status: "rejected",
      hasBom: false,
      reason: "empty_input",
      candidates: [],
      manualOverrideOptions: [...SUPPORTED_ENCODINGS],
    };
  }
  ...
  if (utf16Heuristic) {
    const text = decodeWithIconv(buffer, utf16Heuristic.encoding);
    const qualityScore = calculateTextQuality(text);
    if (qualityScore >= MIN_QUALITY_SCORE) {
      return {
        status: "accepted",
  ...
  if (
    best &&
    best.detectorConfidence >= MIN_DETECTOR_CONFIDENCE &&
    best.qualityScore >= MIN_QUALITY_SCORE &&
    (best.detectorConfidence >= HIGH_DETECTOR_CONFIDENCE ||
      detectorGap >= MIN_DETECTOR_GAP)
  ) {
    return {
      status: "accepted",
      encoding: best.encoding,
      text: decodeWithIconv(buffer, best.encoding),
      hasBom: false,
