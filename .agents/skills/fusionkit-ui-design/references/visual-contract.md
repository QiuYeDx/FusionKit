...
翻译用量概览复用 `ToolStatBar`：标题承载就绪状态，三项指标共处一个紧凑表面；弹窗内长标签自然换行，数字底部对齐。翻译检查计划显示在独立检查弹窗中，与配置表单分开；弹窗正文首个计划区域不叠加顶部边线或额外留白。批量计划/结果清单共用 `StudioBatchItems`；渐变位于滚动视口的同级固定外壳，观察视口及内容尺寸，首尾边缘和不溢出状态必须按真实滚动位置显隐，保留滚动条和指针穿透。
