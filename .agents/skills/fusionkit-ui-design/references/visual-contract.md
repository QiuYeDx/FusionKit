...
| 配置折叠/分隔 | 复用 `ToolConfigDisclosure`（字幕 AI 翻译“定时开始”）；`-mx-3`，内容 `px-3` | 与父级12px内边距配对；仅作为末项时补偿底部留白，后有错误/计划时保留分组间隔 |
...
文档库选择状态采用单一行表面：搜索框与行外框同为12px边距，行内8px，复选框/文字起点20px/44px。li统一承载hover、当前预览和多选背景，子button透明；当前预览仍有勾号指示。批量操作与分页复用37px单行固定footer（24px图标、上下6px），选择前后不增高或挪动列表。更多菜单承载继续、取消、删除和清空；数字跳页保留。搜索控件28px高，筛选摘要与选择栏同排；翻译任务统计横排，实际token总量常驻、明细悬停或在详情查看。行内翻译批次与状态在元数据行同排。原始顶部批量卡片不再作为本页参照。

批量配置弹窗的已选范围预览共用 `StudioSelectedDocuments`：默认折叠，紧凑标题配数量标记；展开后按序号、单行名称和格式排列，复用完整名称Tooltip。标题文字与名称起点对齐，列表最多约五行后内滚动，单份按内容收缩。折叠不改变表单配置，键盘焦点保留且与表面圆角协调；不要回退到裸summary加无层次长列表。删除/恢复确认使用同组件的静态清单，直接展示受影响范围，保留内部滚动及取消默认焦点。
翻译用量概览复用 `ToolStatBar`：标题承载就绪状态，三项指标共处一个紧凑表面；弹窗内长标签自然换行，数字底部对齐。计划分隔线与配置折叠区共用通栏边界，紧邻时不要再叠加正文gap。批量计划/结果清单共用 `StudioBatchItems`；渐变位于滚动视口的同级固定外壳，观察视口及内容尺寸，首尾边缘和不溢出状态必须按真实滚动位置显隐，保留滚动条和指针穿透。
