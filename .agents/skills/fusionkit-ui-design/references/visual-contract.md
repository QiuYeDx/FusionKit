...
翻译资料条目弹窗（2026-09-21）：详情、编辑、方案编辑及通用维护预览共用局部 KnowledgeRecordDialog；Header 52px、外围 12px、分组 16px，正文滚动，底部操作固定。详情先展示身份、核心内容和适用范围，来源与原始字段折叠；术语与参考译文使用双列对照，窄窗可换为单列。右侧仅当前主要操作与关闭/取消，最多三个按钮；复制、拒绝及归档/清除入口集中到左侧更多菜单，危险动作先展示影响预览。条目编辑先呈现核心字段，字段标签承担层级，移除重复小标题和通栏分割线；使用设置改为 ClipPathTabs（rounded / sm），译法、语言、范围、来源共享等高面板，Tab 切换不移动导航与底栏。非活动页保持挂载但隐藏并 inert，各页保留原始输入与滚动位置。浅色设置表面采用明确可辨的浅灰背景，输入框使用 background 色；上下 24px 渐变与表面同色、固定在滚动视口同级，首尾按实际滚动状态显隐，不覆盖滚动条；同时观察视口与内容尺寸变化；保存草稿与保存并启用分开，方案维护菜单说明针对已保存版本。保存失败滚动并聚焦错误且保留草稿；维护取消回到原列表入口。已有资料集归档/删除专用弹窗保持其既有约定。
