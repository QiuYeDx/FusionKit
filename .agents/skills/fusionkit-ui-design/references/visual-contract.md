...
| 工具页顶部 | 共用 40px 标题栏占位 + 24px 标题内距；视口高度 ≤650px 时内距 20px | 所有工具详情与字幕工作台一致，通过 `--tool-page-header-inset` 同步；路由壳层不再叠加 `pt-10` |
...
文档库选择状态采用单一行表面：搜索框与行外框同为12px边距，行内8px，复选框/文字起点20px/44px。li统一承载hover、当前预览和多选背景，子button透明；当前预览仍有勾号指示。批量操作与分页复用37px单行固定footer（24px图标、上下6px），选择前后不增高或挪动列表。更多菜单承载继续、取消、删除和清空；数字跳页保留。搜索控件28px高，筛选摘要与选择栏同排；翻译任务统计横排，实际token总量常驻、明细悬停查看；翻译任务弹窗只展示任务列表，不展示Token卡片。行内翻译批次与状态在元数据行同排。原始顶部批量卡片不再作为本页参照。
...
翻译用量概览复用 `ToolStatBar`：标题承载就绪状态，三项指标共处一个紧凑表面；弹窗内长标签自然换行，数字底部对齐。翻译检查计划显示在独立检查弹窗中，与配置表单分开；弹窗正文首个计划区域不叠加顶部边线或额外留白。「检查与用量」不显示「查看文件详情」及逐文件清单；保留就绪数量、用量估计与队列说明。操作结果的详情继续由 `StudioOperationResult` 承载。单文件与批量「导出字幕」均不提供「编码与换行」配置，统一沿用 UTF-8、无 BOM、LF 默认值；原始文件下载仍保留原始字节。
...
工具详情页的 Esc 与底部返回按钮共用返回工具列表操作，直接进入详情页时也可返回。输入框编辑、输入法组合、弹窗/菜单/选择器/提示/引导与文件拖拽优先消费 Esc；同一次按键不能在关闭局部交互后继续返回。页面级监听只在工具详情路由启用，路由退出时清理监听和待执行回调。

翻译资料的删除资料集弹窗使用紧凑留白：Header、正文横向与 Footer 内边距均为 16px，局部间距不超过 16px；检查叠加后的分组留白。关闭按钮使用独立布局列，详情清单通栏展开，不用大幅 padding 为按钮占位或缩进内容。头部与分组之间不加分割线；Footer 仅保留一行取消/删除按钮，点击删除即确认已展示的删除及历史清理范围，无额外确认复选框；引用拦截仍有效。
可展开行的 hover / 键盘焦点背景与内容保持 8px 内距，背景向两侧扩展，保留原有内容对齐线。紧邻高亮行的上下分割线与背景同步使用 150ms 过渡，移开后恢复；展开详情隔开的下方分割线不受标题悬停影响。分割线显隐不改变布局，减少动态效果偏好下取消过渡。

归档与删除资料集共用 `CollectionActionDialog`、`CollectionImpactRow` 及其样式，保持同一套 600px 宽度、16px 外围留白、固定 Header/Footer 和默认取消焦点。归档使用非破坏性主按钮与归档图标，分为“将移入归档”“将继续保留”，详情默认折叠；明确内容/来源/历史保留、已提交任务与译文不受影响，底部提示恢复后需重新确认。无额外确认复选框；恢复提示与 Footer 之间留有间距，不能紧贴边线。实际操作仍由维护预览与提交逻辑校验。
...
- 工具页只有一个活动导入入口时，整个工具内容区接收文件；多个入口同时可见时，各自限定范围，不猜测导入目标。复用 `ToolFileDropScope` 与 `useToolFileDropTarget`；`ToolFileDropZone` 已自动注册。
- `disabled` 保留入口计数但拒绝接收，避免忙碌入口的文件被转交别处。非活动模式设置 `enabled: false`；隐藏 Tab 与打开的 modal 不接收工具页拖拽。
- 窄窗口可能隐藏上传框，但导入能力仍有效：将注册 marker 放在对应的语义工作区，不能依赖会隐藏的 picker。字幕工作台文档模式以文档 Tab 容器为准。
- 业务回调必须在原始 drop 事件内同步调用原生 File 捕获桥，之后才能等待授权或排队；保留现有文件类型、busy 与权限校验。详见 FK-PIT-0148。
- 整页提示复用共享浮层，保持指针穿透，使用与项目一致的圆角/平滑圆角；检查实际圆角 token 与 Electron 渲染，不能只看 `rounded-*` 类名（FK-PIT-0118）。
...
翻译任务弹窗与转写队列共用 `StudioTaskQueue` / `StudioTaskQueueRow`：行默认透明、无边框和阴影，8px行内留白、4px条目间隔，hover使用muted背景；状态图标与名称同排，元数据缩进23px对齐名称，操作复用28px ghost图标按钮。仅进行中/排队任务展示4px进度条，已结束任务通过状态与批次信息呈现。

翻译配置弹窗（2026-09-21）：Header仅标题，52px高、12px留白、关闭按钮28px热区，标题与按钮同一中心线。文件摘要与复用设置移入正文首行。资料卡片标题与摘要共享左列、顶部12px/阅读间距8px，操作独立顶对齐，窄卡片操作换行。检查与用量、片段检查及试译结果进入独立560px ScrollableDialog，点击立即展示检查中，返回保留原表单并恢复入口焦点，不在表单底部追加。用量三列标签自然换行、数字底对齐；检查错误由检查弹窗就地反馈，不额外触发全局错误提示。已完成的试译可重新查看而不发请求，重新试译是明确的独立动作；检查阶段返回需撤销试译意图。

翻译资料条目弹窗（2026-09-21）：详情、编辑、方案编辑及通用维护预览共用局部 KnowledgeRecordDialog；Header 52px、外围 12px、分组 16px，正文滚动，底部操作固定。详情先展示身份、核心内容和适用范围，来源与原始字段折叠；术语与参考译文使用双列对照，窄窗可换为单列。右侧仅当前主要操作与关闭/取消，最多三个按钮；复制、拒绝及归档/清除入口集中到左侧更多菜单，危险动作先展示影响预览。条目编辑先呈现核心字段，字段标签承担层级，移除重复小标题和通栏分割线；使用设置改为 ClipPathTabs（rounded / sm），译法、语言、范围、来源直接放在弹窗正文层级，按内容自然高度排版，不设置灰色外壳、固定高度或独立滚动。非活动页保持挂载但 display:none 并 inert，保留原始输入；仅弹窗正文统一滚动，头尾固定。编辑弹窗不显示“仅用于之后选用本资料的新翻译”提示（2026-09-22 用户反馈）；保存草稿与保存并启用分开，方案维护菜单说明针对已保存版本。保存失败滚动并聚焦错误且保留草稿；维护取消回到原列表入口。已有资料集归档/删除专用弹窗保持其既有约定。

资料条目编辑的适用范围（2026-09-23 用户反馈）：采用平直的分组标题、对象选择行与条件字段，去掉分组外卡片和对象行内卡片的多层边框、底色及重复 padding。复用 shadcn Checkbox / Select；角色选择与对应对象对齐且保持独立于 checkbox label，不能操作下拉框时误触勾选。仅由真实表单控件表达边界，分组通过留白和必要分隔组织。区分大小写仍使用 ToolSwitchRow。Tab 切换保持表单子树；隐藏测量不得覆盖自然高度，并由当前 Tab 身份控制切换期间的尺寸补间，详见 FK-PIT-0173。

弹窗折叠区与列表（2026-09-23 用户反馈）：直角折叠分区必须延伸到所属表面两端，标题、图标及正文保留 12px 内距；内缩分组使用圆角与合理外部留白，禁止内缩直角分隔线加贴边内容的组合。Motion 裁剪层应与通栏分区同宽，不用负 margin 穿过裁剪层（FK-PIT-0174）。列表短行优先用横向空间，长正文自然换行；选项、列表及操作区保持同类间距，滚动渐变依真实视口首尾变化。双语整理的两个布尔选项用 Switch；本地资源名称/元数据与右侧操作组成紧凑行，删除不得孤立占最后一行。
