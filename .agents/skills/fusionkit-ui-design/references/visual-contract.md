...
批量配置弹窗的已选范围预览共用 `StudioSelectedDocuments`：默认折叠，紧凑标题配数量标记；展开后按序号、单行名称和格式排列，复用完整名称Tooltip。标题文字与名称起点对齐，列表最多约五行后内滚动，单份按内容收缩。折叠不改变表单配置，键盘焦点保留且与表面圆角协调；不要回退到裸summary加无层次长列表。
## 文件名与 Tooltip
