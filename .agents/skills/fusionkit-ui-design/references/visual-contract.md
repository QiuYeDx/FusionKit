...
翻译资料条目弹窗（2026-09-21）：详情、编辑、方案编辑及通用维护预览共用局部 KnowledgeRecordDialog；Header 52px、外围 12px、分组 16px，正文滚动，底部操作固定。详情先展示身份、核心内容和适用范围，来源与原始字段折叠；术语与参考译文使用双列对照，窄窗可换为单列。右侧仅当前主要操作与关闭/取消，最多三个按钮；复制、拒绝及归档/清除入口集中到左侧更多菜单，危险动作先展示影响预览。编辑先呈现核心字段，进阶参数使用带摘要的 Accordion；保存草稿与保存并启用分开，方案维护菜单说明针对已保存版本。保存失败滚动并聚焦错误且保留草稿；维护取消回到原列表入口。已有资料集归档/删除专用弹窗保持其既有约定。
