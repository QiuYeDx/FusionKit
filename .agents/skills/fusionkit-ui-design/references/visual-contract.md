...
资料条目编辑的对象选择卡片（2026-09-22）：以字幕 AI 翻译 Thinking 配置行作为视觉标准。复用 ToolToggleRow 的边框、rounded-lg、12px 留白、左侧标题与说明、右侧控件以及 hover；仅将 Switch 替换为 Checkbox，不使用整卡选中灰底、深边框或外圈。区分大小写直接使用 ToolSwitchRow。角色选择等附属交互放在同卡片但 label 之外，避免误触复选框。
