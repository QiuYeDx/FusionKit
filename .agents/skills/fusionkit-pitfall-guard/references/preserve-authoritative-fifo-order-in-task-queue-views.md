# FK-PIT-0094: Preserve authoritative FIFO order in task queue views
...
task queue, FIFO, session snapshot, enqueue receipt, optimistic batch, createdAt, UUID, display order
...
The main-process session snapshot and enqueue receipt already carry insertion/FIFO order. Re-sorting those arrays in the renderer by `createdAt` replaces that authority, while opaque UUIDs are identities rather than sequence numbers. Prepending optimistic receipts introduces a second, incompatible order.
