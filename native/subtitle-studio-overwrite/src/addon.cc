...
#error "subtitle_studio_overwrite is supported only on macOS"
#endif

#if !defined(__arm64__) && !defined(__aarch64__)
#error "subtitle_studio_overwrite is supported only on macOS arm64"
#endif
...
std::string PartialLeafForTransactionId(const std::string &transaction_id) {
  return ".fusionkit-subtitle-studio-" + transaction_id + ".partial";
}
...
JournalNames DeriveJournalNames(const std::string &transaction_id) {
  const std::string base =
      PartialLeafForTransactionId(transaction_id) + ".fusionkit-subtitle-studio-overwrite";
